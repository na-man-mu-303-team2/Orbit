...
- Context: 기존 KDH 전용 프로젝트는 외부 Unsplash 사진을 첫 슬라이드 배경으로 사용해 홈이 일반 비즈니스 자료 목록처럼 보였고, 외부 제공자의 가용성과 정책에도 의존했다. 사용자는 PPT 제작 서비스임이 바로 드러나는 크리에이티브 표지 10종을 `main` 대상 PR로 적용하도록 승인했다.
- Options considered:
...
- Follow-up review notes: PR 검증에서 `kdh@orbit.com`의 10개 표지와 편집 화면을 확인하고, 다른 계정에서 목록·직접 project ID·access request가 계속 차단되는지 검증한다. PR 병합과 운영 배포는 별도 승인 범위로 둔다.
