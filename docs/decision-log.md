...
- Follow-up review notes: 실제 AWS 적용 전 Change Set에서 기존 `AWS::RDS::DBInstance`, `AWS::S3::Bucket`, `AWS::CloudFront::Distribution`, `AWS::EC2::Instance`의 delete/replace가 없는지 확인한다. storage-owning stack을 저장소의 canonical template로 복구하고 SSE-KMS 전환 영향과 rollback을 별도로 검토한다. candidate smoke test, queue/DB active count 0, raw/evidence 및 legacy audio read, Socket.IO multi-task presence를 확인한 뒤 ECS traffic을 5→25→50→100%로 올린다.
