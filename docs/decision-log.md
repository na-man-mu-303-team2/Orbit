...
## ORBIT production S3 purpose routing and private audio retention

- Context: AWS production은 Static Web, project Assets, Private Audio의 세 S3 bucket으로 분리됐지만 애플리케이션은 `S3_BUCKET` 하나만 사용했다. Private Audio IAM과 lifecycle은 `raw/`, `evidence/` prefix만 허용하는데 신규 rehearsal/focused-practice/Q&A audio key는 해당 prefix를 사용하지 않았다. Live Private Audio stack의 Evidence Clip lifecycle은 14일이었지만 `docs/contracts.md`는 고정 7일 보관을 요구한다. 또한 browser presigned upload CORS에는 CloudFront 기본 domain만 있고 운영 custom domain이 빠져 있었다.
- Options considered:
  - 모든 object를 기존 Assets bucket에 계속 저장하고 AWS의 세 번째 bucket은 사용하지 않는다.
  - StoragePort 호출부마다 bucket을 선택하는 별도 port와 분기 로직을 추가한다.
  - StoragePort 계약은 유지하고 `raw/`, `evidence/` prefix를 private bucket으로 보내는 router를 API와 worker가 공통 사용한다.
- Final decision: `S3_BUCKET`은 project asset, export, transcript artifact의 기본 bucket으로 유지하고 `S3_PRIVATE_AUDIO_BUCKET`을 추가한다. production S3에서는 두 bucket이 반드시 존재하고 달라야 한다. 신규 private audio는 `raw/` 아래에 저장하며 `raw/`와 `evidence/`는 Private Audio bucket으로 routing한다. raw audio는 terminal path 즉시 삭제를 기본으로 하고 14일 lifecycle을 안전망으로 유지한다. Evidence Clip lifecycle은 권위 계약에 맞춰 7일로 수정한다. Private Audio CORS에는 CloudFront 기본 domain과 `tryorbit.site`, `www.tryorbit.site`를 허용한다. Assets bucket versioning 선언은 live 상태인 `Suspended`와 일치시킨다.
- Rationale: 기존 StoragePort와 DB schema를 바꾸지 않고도 민감 음성이 Assets bucket으로 유출되는 경로를 차단한다. prefix와 FilePurpose 불일치를 write 시 거부해 설정 실수를 조기에 발견한다. Evidence Clip을 계약보다 오래 보관하지 않아 개인정보 최소 보관 경계를 유지하고, local MinIO는 단일 bucket 개발 경험을 보존한다.
- Affected files: `packages/storage/src/index.ts`, `packages/config/src/index.ts`, `apps/api/src/files/*`, `apps/api/src/config/env.schema.spec.ts`, `apps/worker/src/storage*`, `.env*.example`, `infra/aws/main-production-bootstrap.yaml`, `infra/aws/main-production-storage.yaml`, `infra/aws/ec2-production.env.example`, `infra/scripts/check-env.mjs`, `infra/scripts/deploy-aws-ec2.sh`, `docs/deployment.md`, `docs/conventions/environment.md`, `docs/runbooks/aws-main-auto-deployment.md`, `docs/decision-log.md`.
- Follow-up review notes: merge 전 CloudFormation validation과 change set에서 기존 Private Audio bucket 교체가 없고 CORS, Evidence lifecycle, tag update만 발생하는지 확인한다. production env에는 stack output의 `PrivateAudioBucketName`만 넣고 값을 로그에 남기지 않는다. 배포 후 raw/rehearsal upload와 worker read/delete가 private bucket을 사용하고 transcript JSON/text는 Assets bucket에 남는지 검증한다.
