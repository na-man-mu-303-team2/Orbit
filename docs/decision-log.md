...
- Final decision: repository variable `AWS_CLOUDFORMATION_EXECUTION_ROLE_ARN`을 필수 계약으로 추가하고 Plan workflow의 `create-change-set --role-arn`으로 고정한다. `DescribeChangeSet` 응답에는 execution role ARN이 없으므로 artifact parser에서 존재하지 않는 값을 검사하지 않는다. 대신 Plan role의 `iam:PassRole`을 execution role ARN 하나와 `iam:PassedToService=cloudformation.amazonaws.com`으로 제한하고, Apply role에는 Change Set 생성 권한과 `iam:PassRole`을 부여하지 않는다. `AWS_INFRA_PLAN_ROLE_ARN`, `AWS_INFRA_APPLY_ROLE_ARN`, `AWS_ECR_PUBLISH_ROLE_ARN`은 repository variable로 관리하되 역할은 서로 재사용하지 않는다. Pull request 정적 검증 job에는 OIDC token 권한을 주지 않고, Plan job은 `refs/heads/main` dispatch로 분리한다. Plan role trust는 `aud=sts.amazonaws.com`과 exact main-ref subject만 허용한다. Apply job은 `production` environment 승인을 유지하고 Apply role trust는 해당 environment의 OIDC subject만 허용한다. Apply의 Change Set ARN과 region은 job environment로 전달하고 형식을 검증한 뒤 shell에서만 참조한다.
...
## ORBIT personal staging decommission

- Context: `develop`을 배포하던 개인 서버를 소유 회사에 반납하고 대체 staging 인프라를 두지 않기로 했다. 서버 runner와 runtime token을 먼저 제거한 상태에서 기존 workflow와 GitHub/Doppler 자격증명을 유지하면 `develop` push가 존재하지 않는 runner를 기다리거나 불필요한 secret 변경 권한을 계속 보유한다.
- Options considered:
  - workflow와 자격증명을 유지해 같은 서버를 다시 받을 때 재사용한다.
  - runner만 제거하고 Doppler sync 및 GitHub Environment는 남긴다.
  - 개인 서버 실행 경로와 자격증명 소비 지점을 제거하고, 정적 staging 환경 계약만 남긴다.
- Final decision: 세 번째 안을 채택한다. `Environment Contract CI`는 환경 파일과 Compose 계약만 검증하고 Doppler sync 또는 self-hosted runner 배포를 실행하지 않는다. 수동 personal staging workflow, develop 전용 GHCR build, 개인 서버 deploy/preflight/wrapper 및 Doppler sync script를 제거한다. `.env.staging.example`, `docker-compose.staging.yml`, `infra/env/personal-staging-env-policy.json`은 새 인프라를 암시하지 않는 정적 계약 자료로 유지한다. 저장소 변경이 `develop`에 병합된 뒤 GitHub `personal-staging` Environment, 개인 서버 deploy key, staging 전용 GHCR PAT와 Doppler read/write service token을 폐기한다.
- Rationale: 존재하지 않는 서버를 대상으로 job이 대기하거나 실패하는 것을 막고, 사용하지 않는 write credential의 수명을 즉시 끝낸다. staging 예시와 Compose 계약은 향후 별도 승인된 인프라 설계에서 재사용할 수 있지만 기존 개인 서버 권한·자동화는 복원하지 않는다.
- Affected files: `.github/workflows/environment-contract-ci.yml`, `.github/workflows/deploy-personal-staging.yml`, `.github/workflows/build-images.yml`, `package.json`, `infra/scripts/check-personal-staging-env.sh`, `infra/scripts/deploy-personal-server.sh`, `infra/scripts/orbit-deploy-personal-staging-wrapper.sh`, `infra/scripts/sync-personal-staging-doppler.mjs`, `infra/scripts/personal-staging-env.mjs`, `infra/scripts/personal-staging-env.test.mjs`, `infra/scripts/personal-staging-deploy.test.mjs`, `infra/scripts/personal-staging-wrapper.test.mjs`, `docs/contracts.md`, `docs/conventions/environment.md`, `docs/testing/test-matrix.md`, `docs/runbooks/personal-server-deployment.md`, `docs/runbooks/deploy-image-registry-migration.md`, `docs/decision-log.md`.
- Follow-up review notes: PR merge 후 GitHub runner가 0개인지, `personal-staging` Environment와 `orbit-personal-server-deploy` key가 없는지, Doppler webhook과 개인 staging service token이 없는지 확인한다. 서버 디스크의 최종 secure erase 또는 재이미징은 장비 소유 회사가 수행하고 완료 사실을 확인한다.
