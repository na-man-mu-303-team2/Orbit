...
`S3_BUCKET`은 기존 assets bucket 호환 key다. 신규 배포는 `S3_ASSETS_BUCKET`을 일반 asset bucket으로, `S3_PRIVATE_AUDIO_BUCKET`을 private-audio bucket으로 사용한다. private key prefix로 새 audio를 전용 bucket에 저장하고, prefix 없는 기존 audio는 기존 assets bucket에서 읽는다. `S3_PRIVATE_AUDIO_BUCKET`이 비어 있으면 dedicated bucket을 아직 활성화하지 않고 기존 bucket만 사용한다.
