...
개인 서버의 임시 HTTP demo처럼 TLS가 아직 없는 staging 검증 경로에서만 다음 값을 사용할 수 있다.

```txt
AUTH_COOKIE_SECURE=false
```

이 값은 개인 서버 develop demo 전용 예외다. production에서는 `AUTH_COOKIE_SECURE=false`를 사용할 수 없고, 일반 staging도 HTTPS를 우선한다.
