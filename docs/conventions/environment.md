...
ORBIT_PPTX_OOXML_VECTOR_IMPORT=false
```
...
`ORBIT_PPTX_OOXML_VECTOR_IMPORT=true | false`는 PPTX import에서 OOXML XML 직접 파서 기반 visual tree 추출 경로를 켠다. 기본값 `false`는 기존 `python-pptx` 기반 importer를 유지한다.
