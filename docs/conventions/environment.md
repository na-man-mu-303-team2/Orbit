...
`S3_BUCKET`은 project asset, export, transcript artifact를 저장하는 기본 bucket이다. `S3_PRIVATE_AUDIO_BUCKET`은 `raw/`와 `evidence/` prefix 전용 private bucket이다. `APP_ENV=production`과 `STORAGE_DRIVER=s3` 조합에서는 두 값이 모두 존재하고 서로 달라야 한다. 로컬 MinIO는 두 값을 같은 `orbit-local` bucket으로 둘 수 있으며 prefix routing만 동일하게 검증한다.
