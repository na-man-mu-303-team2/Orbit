...
| 구분 | 계획 범위 | 구현 시작 조건 |
| --- | --- | --- |
| P0 계약 선행 | 별도 RehearsalFocusProfile, CPM·STT Quality Gate·pause v2, 문장 Target, 12초 Audio Clip·기본 14일 보관·Owner-only Evidence API | 제품 문서 범위 승인과 Schema·Migration·지표 버전·보관·삭제·권한 정책 PR 승인 |
...
16. Clip은 프로젝트 Owner 전용으로 기본 14일 보관하고 조기 삭제·만료·저장 실패·삭제 재시도를 지원한다. 최대 30일 연장은 별도 제품·개인정보 정책 승인 뒤에만 허용하며 저장 URL이나 Signed URL은 결과·로그에 넣지 않는다.
...
- Clip은 12초를 넘지 않고 기본 14일 만료·조기 삭제·삭제 재시도 시험이 통과한다. 30일 연장은 별도 승인 없이는 사용할 수 없다.
...
제품 문서에서 P0 확장 범위를 승인한 뒤, P0 계약 PR에서 RehearsalFocusProfile, CPM·WPM 호환, STT Quality Gate, pause v2, 문장 Target, 최대 12초 Clip·기본 14일 보관·Owner-only 접근을 먼저 고정한다.
...
- Clip은 기본 14일 뒤 삭제되고 조기 삭제·삭제 재시도가 동작하며 전체 raw audio는 분석 뒤 삭제된다. 최대 30일 연장은 별도 제품·개인정보 정책 승인 뒤에만 허용한다.
...
- 현재 전체 리허설 음성 원본은 분석 뒤 삭제하며 12초 Clip은 기본 14일 보관 계약이 있지만 생성·재생 흐름 통합은 미완성이다.
...
따라서 제품 문서에서 P0 확장 범위를 먼저 승인하고, Phase 0에서 P0 계약 확장과 Migration·결정 기록을 병합한 뒤 네 명이 Fixture를 이용해 P0를 병렬 구현한다. P0에는 별도 RehearsalFocusProfile, 전체 경과시간 기준 CPM·STT Quality Gate·근거가 없으면 `unknown`인 pause v2, 문장 Target, 최대 12초 Clip·기본 14일 보관·Owner-only Evidence Player, 전체 대본을 숨기는 실전 Prompter가 포함된다. P0 완료 뒤에는 저장소 제품 문서의 Next 후보를 검토하고, 제품 문서와 계약 PR에서 승인된 기능만 P1으로 구현한다. 신뢰할 수 있는 발음 평가 등 Later 후보는 Provider 검증과 별도 승인이 필요하다.
