...
```text
feature/ORBIT-XXX-p0-core-contract
```

최종 담당자는 임재환이며 네 명 모두 검토한다.

`ORBIT-XXX`는 연결할 Jira 이슈가 있을 때 실제 이슈 키로 바꾼다. Jira 이슈가 없으면 `AGENTS.md`와 `docs/git-rules.md`에 따라 목적이 드러나는 브랜치명을 사용한다.
...
```text
feature/ORBIT-XXX-p0-core-evaluator
```
...
```text
feature/ORBIT-XXX-p0-core-speech-evidence
```
...
```text
feature/ORBIT-XXX-p0-core-focused-verification
```
...
```text
feature/ORBIT-XXX-p0-core-report-integration
```
...
19. 기존 `storage-deletion-reconciler.ts`의 삭제 outbox 처리를 재사용하고, durable coaching Dispatcher·오래 멈춘 attempt·미삭제 private audio 복구만 별도 Reconciler로 추가한다. 같은 삭제 작업을 수행하는 두 번째 주기 루프를 만들지 않는다.
