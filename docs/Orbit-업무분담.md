...
> **P1(피원)**: P0가 안정적으로 동작한 다음 분석 정확도와 사용 편의성을 높이는 후속 기능 범위
...
> P0에서는 기존 공통 계약과 데이터베이스 구조를 변경하지 않는다. 현재 계약에 없는 짧은 음성 재생·CPM·정교한 멈춤 분류·별도 PracticeIntent는 P1 제안으로 남기며, 별도의 계약·개인정보·데이터베이스 검토가 승인된 뒤 구현한다.
...
| 구분         | 이번 문서에서 다루는 기능                                                                                                                                   | 구현 시작 조건                                      |
| ------------ | ----------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------- |
| P0 즉시 구현 | 기존 Brief 목표 확인, 공통 평가, Top 3, 허용된 시간 범위·의미 기준·문제 참조 근거, WPM, 멈춤 v1, 집중 연습, 전체 재검증, 5개 추세, 다음 연습 계획, 실제 E2E | 현재 공통 계약과 Fixture 확인 후 병렬 시작          |
| P1 선행 검토 | 12초 음성 30일 보관·재생, CPM, 멈춤 v2, 별도 PracticeIntent                                                                                                 | 계약·개인정보·권한·Migration·지표 버전 승인 후 시작 |
| P1 고도화    | Lens별 평가표, 낭독·언어 분석, 고급 음성·발음, 맞춤 후속 질문, Evidence Timeline, 문장 교정, Prompter, 대본 축약                                            | P0 E2E 통과 후 기능별 작은 PR로 진행                |

P1 기능은 삭제하거나 포기한 항목이 아니다. 현재 공식 규칙과 충돌하지 않도록 구현 순서만 뒤로 둔 것이다.
...
| 말하기 속도            | `wordsPerMinute`     | P0에서는 현재 공식 계약대로 말한 낱말 수 ÷ 발표 시간(분)을 사용 | 김동현        |
| 시간 균형              | `timing-balance`     | P0 계약대로 통과한 시간 기준 수 ÷ 측정된 전체 시간 기준 수      | 최영빈·김동현 |
...
- P0 WPM 목표 범위는 현재 화면 기본값인 85~130을 Phase 0의 v1 정책으로 결정 기록에 고정한다. 실행 당시 Snapshot에 별도 속도 범위가 있으면 그 값이 우선한다.
- P0 말하기 속도는 현재 공식 계약의 WPM을 유지한다.
- CPM은 기존 값을 바꾸는 대체 지표가 아니라 P1의 추가 지표로 검토한다.
- P0에서 신뢰할 수 있는 시간 구간이나 STT 자료가 없으면 기존 이유 코드인 `NO_MEASUREMENT`(측정 자료 없음) 또는 `EVALUATION_UNAVAILABLE`(분석 기능 사용 불가)로 측정 불가 처리한다.
- 새 `LOW_TRANSCRIPTION_CONFIDENCE` 이유 코드와 CPM을 사용하려면 공통 계약과 `metricDefinitionVersion`(지표 계산 규칙 번호)을 먼저 확장한다.
...
- 결과 보고서에는 음성 원본, 저장 주소, 파일 ID, Signed URL을 넣지 않는다.
- Owner를 포함한 누구에게도 P0 결과 화면에서 음성 원본 재생 기능을 제공하지 않는다.
- 문제 근거는 현재 계약이 허용한 시간 범위·Semantic Cue(의미 기준)·Issue(문제) 참조와 관측값으로 제공한다.
- 자료가 부족하면 존재하지 않는 음성 구간이나 문장을 만들어 내지 않고 근거 사용 불가로 표시한다.

### P1 제안 — 문제 구간 음성 저장·재생

다음 기능은 삭제하지 않고 P1 선행 검토 항목으로 남긴다.

- 문제 한 건당 최대 12초 Audio Clip 추출
- 생성 시각부터 30일 보관
- 기본적으로 Owner만 접근하며, Editor 접근은 별도 권한 정책이 승인된 뒤에만 허용
- 짧은 만료시간의 Signed URL 발급
- Clip 만료·삭제·접근권한 관리
- Evidence Player에서 정확한 구간 재생
- 동시에 하나의 Clip만 재생
- 재생 불가 시 글·시간·수치 근거로 Fallback

이 기능은 현재 공식 보관·권한 정책과 다르므로 P0에서 구현하지 않는다. 다음 선행 결정을 별도 PR로 승인받은 뒤 P1을 시작한다.

1. 최대 12초와 30일 보관이 필요한 이유와 최소 보관기간
2. 현재 Owner-only 경계를 Editor까지 넓힐지 여부
...
6. 공통 Schema, 데이터베이스 Migration, `docs/contracts.md`, `docs/decision-log.md` 변경

P1이 승인되더라도 전체 음성 원본은 분석 직후 삭제하며 Clip 생성 실패가 전체 보고서 실패로 이어지지 않게 한다.
...
- P0 시험은 음성 Clip이나 Signed URL이 있다고 가정하지 않는다.

---

## 결정 7. 리허설 전 사용자 목표 설정

### P0 — 기존 PresentationBrief 사용

P0에서는 별도의 PracticeIntent 저장소를 만들지 않는다. 기존 PresentationBrief(발표 목표 설명)의 다음 정보를 사용자가 리허설 전에 확인하고 수정하게 한다.
...
### P1 제안 — 별도 PracticeIntent

다음 기능은 기존 Brief만으로 부족하다는 사용자 검증이 끝난 뒤 추가한다.

- 사용자가 습관어·시간 균형·핵심 내용·발음·목소리 중 최대 세 개 선택
- `intentId`, Revision, 선택 출처, 적용 시각, 측정 가능 여부 저장
- 별도 목표 저장 API와 선택 UI
- 사용자 목표를 먼저 배치하고 자동 Top 3가 빈 자리만 채우는 병합

별도 PracticeIntent는 새로운 공식 데이터 원본이 되므로 공통 Schema, API 계약, 데이터베이스 Migration, Brief와의 우선순위, 중복 제거 기준, Run Snapshot 연결을 먼저 결정해야 한다. 이 결정 전에는 이름만 같은 자체 JSON을 앱마다 만들지 않는다.
...
- P0 공식 말하기 속도는 현재 계약의 WPM이다.
- STT가 제공한 최종 시간 구간과 전체 시간을 사용하며 Transcript 글자 길이만 보고 시간을 추정하지 않는다.
- 시간 구간·언어·전체 시간처럼 계산에 필요한 자료가 없거나 분석 기능을 사용할 수 없으면 기존 이유 코드로 측정 불가 처리한다.
- Provider가 confidence(음성 인식 확실도)를 제공하지 않았는데 임의 숫자를 만들어 신뢰할 수 있다고 표시하지 않는다.
- 측정 불가 결과는 실패·0점·Top 3 문제로 바꾸지 않는다.

### P0 멈춤 판정

- 현재 공식 `pause v1`은 연속된 음성 구간 사이가 1초 이상 비면 긴 멈춤 한 번으로 센다.
- 음성 구간 시간이 없으면 긴 멈춤 0회가 아니라 측정 불가로 처리한다.
- P0에서는 멈춤의 의도를 추측해 새로운 상태를 만들지 않는다.
- 이 규칙의 결과에는 현재 지표 계산 규칙 번호를 붙여 다른 규칙의 결과와 섞지 않는다.
...
├─ scope: 전체 발표·슬라이드·슬라이드 범위·시작/마무리 중 평가 범위
├─ measurementState: 측정 여부
├─ value: 시간·개수·분당 낱말 수·비율·의미 결과 중 실제 관측값
...
### P1 제안 — CPM·STT 품질 코드·멈춤 v2

다음 기능은 계약과 지표 버전을 먼저 확장한 뒤 추가한다.
...
P1의 멈춤 분류는 기존 1초 규칙을 몰래 바꾸지 않고 `pause v2`로 추가한다. v1과 v2 결과는 `METRIC_DEFINITION_CHANGED`(계산 규칙 변경)로 비교 불가 처리한다. CPM도 새 단위와 목표 범위, Fixture, `metricDefinitionVersion`, `docs/decision-log.md`가 확정되기 전에는 공식 추세로 표시하지 않는다.
...
P1은 P0 핵심 흐름과 실제 E2E가 통과한 뒤 별도 PR로 진행한다. P1 때문에 P0 병합을 막지는 않지만 문서에서 담당과 완료조건을 없애지 않는다.

| P1 기능                  | 쉬운 설명                                                           | 담당          |
| ------------------------ | ------------------------------------------------------------------- | ------------- |
| 별도 PracticeIntent      | 사용자가 최대 세 가지 연습 목표를 직접 골라 별도로 저장             | 최영빈·임재환 |
| 한국어 CPM               | 한국어 말하기 속도를 공백 제외 1분당 글자 수로 추가 측정            | 김동현        |
| 멈춤 분류 v2             | 의도한 멈춤·말막힘·판단 불가와 문장 위치를 구분                     | 김동현        |
| 12초 음성 근거 재생      | 승인된 보관·권한 정책에 따라 문제 음성을 제한적으로 재생            | 김동현·임재환 |
...
| 개인화 Prompter          | 전체 대본 대신 남은 시간·핵심 단어·반복 문제만 표시                 | 임재환        |
...
| 담당   | 바로 시작할 작업                                                                    |
| ------ | ----------------------------------------------------------------------------------- |
| 최영빈 | Brief 참조·평가계획 Snapshot·평가 결과·Top 3 Fixture와 통과·실패 경계값 작성        |
| 김동현 | TypeScript→Python DTO, WPM·습관어·멈춤 v1·시간 근거, HTTP 422 시험 작성             |
| 이창원 | 기존 집중 연습·Q&A 임시 판정 조사, Target·안정화·전체 재검증 Fixture 작성           |
| 임재환 | 목표 확인 화면, 결과 보고서, 다음 연습 계획 Fixture와 실제 전체 흐름 시험 목록 작성 |
...
- 모든 Fixture가 기존 공통 Schema를 통과한다.
- P0는 현재 WPM·멈춤 v1·Owner-only·음성 원본 즉시 삭제 정책을 사용한다.
- CPM·멈춤 v2·12초 음성 재생·별도 PracticeIntent를 P0 코드나 Fixture에 몰래 넣지 않는다.
- 통과·부분 전달·실패·측정 불가·비교 불가 사례가 준비되어 있다.
- Brief Revision 충돌과 `briefRef`·`evaluationPlan` Snapshot 고정 사례가 준비되어 있다.
- 습관어 1회/2회 경계 시험이 있다.
- 긴 멈춤 0회/1회 경계 시험이 있다.
- 시간 근거 있음·없음과 마지막 슬라이드 종료 시각 있음·없음 사례가 있다.
- 실패 → 통과 → 통과 안정화 시험이 있다.
- 보고서 Fixture에 Top 3, 평가 결과, 위치·시간 근거, 최대 4단계 다음 연습 계획이 있다.
- TypeScript 요청을 Python이 정상적으로 받는다.
- 잘못된 요청은 Python이 HTTP 422로 거부한다.
- 민감한 정보가 Fixture와 로그에 없다.
- 파일별 최종 수정 담당자가 한 명으로 정해져 있다.
- P0에서는 `packages/shared`, 데이터베이스 Migration, `docs/contracts.md`를 변경하지 않는다.
- P1 제안은 별도 선행 결정과 계약 PR 없이는 구현을 시작하지 않는다.
...
2. Brief ID와 Revision, 그 Brief로 만든 평가계획이 발표 시작 당시 Snapshot에 고정됐는지 검증한다.
3. 시간·습관어·멈춤 v1·의미 전달·질문 답변을 공통 판정 함수로 보낸다.
4. 전체 발표·집중 연습·질문 답변이 같은 Criterion(평가 기준)과 Evidence(근거)를 넣으면 같은 결과가 나오게 한다.
5. 측정 자료가 없거나 분석할 수 없는 경우를 기존 이유 코드의 측정 불가로 만들고 실패로 바꾸는 임시 처리를 제거한다.
6. 실패 또는 부분 전달 결과만 Top 3 후보로 만든다.
7. Brief 영향도, Lens 우선순위, 문제 심각도, 근거 신뢰도, 반복 여부, 집중 연습 가능성, 슬라이드 순서로 정렬한다.
8. 사용자 목표와 같은 Brief 기준을 먼저 고려하고 남은 자리만 다른 자동 문제로 채운다.
9. 같은 평가 기준과 범위의 중복 문제를 하나로 합친다.
10. 같은 입력에는 항상 같은 세 가지가 나오도록 정렬 마지막 기준까지 고정한다.
11. 각 Top 3에 관측 사실·청중 영향·바꿀 행동·연습 범위·성공 조건·Observation 참조를 넣는다.
12. 과거 결과를 수정하지 않고 새 분석 Revision의 목표 묶음으로 저장한다.
13. 실제로 비교할 수 있는 전체 발표만 사용해 반복 기록을 계산한다.
...
- 별도 PracticeIntent가 승인되면 사용자 선택 목표와 자동 Top 3의 순수 병합·중복 제거 규칙을 구현한다.
- Lens별 Rubric이 승인되면 우선순위뿐 아니라 평가 항목과 성공 조건도 관점별로 바꾼다.
...
5. Provider가 confidence를 주지 않았으면 확실도 숫자를 추측하지 않는다.
6. 자료가 충분할 때만 전체 발표 시간과 현재 공식 지표인 WPM을 계산한다.
7. 기존 습관어 목록과 단어·여러 낱말 표현 구분 방식을 사용해 단순 글자 포함 검사 없이 습관어를 계산한다.
8. 음성 구간 사이 1초 이상의 빈 구간을 현재 공식 `pause v1` 긴 멈춤으로 계산한다.
9. 기존 숫자 DTO는 호환을 위해 유지한다. 시간 근거가 없을 때 나온 WPM·멈춤 숫자는 측정값으로 사용하지 않고, 최영빈의 평가기와 임재환의 조립기가 기존 `durationSeconds`, `speedSamples`, 구간 존재 여부를 확인해 측정 불가 처리하게 한다.
10. 슬라이드 이동 기록으로 목표시간과 실제시간을 계산한다.
11. 실제 발표 종료 시각을 받은 경우에만 마지막 슬라이드 종료 시각으로 사용하고, 없으면 추정하지 않는다.
12. 신뢰할 수 있는 실제 구간이 있는 문제만 현재 계약이 허용한 `scope`, 관측값, `time-range` Evidence 참조의 원천값을 만든다.
13. 음성 변환 원문은 분석 중 Memory에서만 사용하고 결과·로그·작업 데이터에 넣지 않는다.
...
- `pauseCount`와 `pauseDetails`: pause v1의 긴 멈춤 개수와 실제 구간
- `durationSeconds`: 전체 발표 시간
- `wordsPerMinute`: 현재 공식 말하기 속도
- `slideTimings`: 슬라이드별 목표시간과 실제시간
- 위치를 확인할 수 있는 문제의 평가 범위·시작/종료 시간 참조·관측값
- 평가기가 측정 가능 여부를 판단하는 데 필요한 기존 시간 근거의 존재 여부
...
- CPM 계약이 승인되면 한국어 공백 제외 글자 수와 실제 발표 시간으로 `charactersPerMinute`를 추가한다. WPM을 삭제하거나 과거 결과를 CPM으로 다시 쓰지 않는다.
- `LOW_TRANSCRIPTION_CONFIDENCE`가 승인되면 Provider별 최소 확실도·미제공 처리·Fixture를 지표 버전과 함께 추가한다.
- `pause v2`가 승인되면 의도한 멈춤·말막힘·판단 불가와 문장 위치를 분류하고 v1 결과와 비교 불가 처리한다.
- 습관어 발생별 시각 계약이 승인되면 슬라이드·전환·문장 시작 위치 패턴을 추가한다.
- Audio Clip 정책이 승인되면 전체 음성을 삭제하기 전에 문제별 최대 12초 Clip을 만들고, 저장·삭제 재시도까지 담당한다.
...
4. P0에서는 기존 `slide`, `slide-range`, `opening`, `closing` Target(연습 대상)만 사용한다.
5. 각 연습에 원래 전체 발표 ID, 목표 묶음 Revision, Criterion Revision, Target 범위를 고정한다.
6. 발표 자료가 바뀌어 Target과 현재 자료가 맞지 않으면 오래된 연습으로 표시하고 자동 실행하지 않는다.
7. 발화 글자 수나 `"음"` 개수로 평가하는 임시 규칙을 제거한다.
8. 최영빈의 공통 평가기와 김동현이 제공한 실제 측정값을 사용한다.
9. 원래 전체 발표의 값, 현재 시도의 값, 변화량, 통과 기준, 상태, 다음 행동 하나를 같은 단위로 만든다.
10. 측정 가능한 같은 목표의 인접한 두 시도가 모두 통과했을 때만 안정화로 표시한다.
11. 집중 연습 통과는 `전체 발표 검증 대기`로 저장하고 공식 해결로 바꾸지 않는다.
12. 다음 전체 발표에서 같은 Criterion을 다시 평가해 해결·반복·측정 불가·비교 불가를 확정한다.
13. 기존 질문의 출처와 관련 슬라이드를 유지한 채 답변 결과를 공통 CriterionResult로 변환한다.
14. 질문 답변 원문은 분석 중에만 사용하고 공통 보고서·로그에는 넣지 않는다.
...
- 문장 Target 계약이 승인되면 슬라이드 ID, 문장 순서, 문장 복사본 Hash(내용 확인용 짧은 값)를 저장하고 자료 변경 시 오래된 상태로 만든다.
...
1. 리허설 시작 전에 기존 PresentationBrief와 여기서 만들어진 평가 목표를 확인·수정하는 화면을 만든다.
2. 수정 시 기존 Revision과 CAS를 사용하고 HTTP 409 충돌을 사용자가 이해할 문장으로 보여 준다.
...
11. P0에는 음성 재생 버튼이나 Signed URL을 넣지 않는다.
12. 다섯 추세를 최근 최대 5회의 전체 발표로 조립하고 P0 말하기 속도는 WPM으로 표시한다.
13. 측정 불가와 비교 불가를 0으로 그리지 않고 집중 연습 시도를 장기 추세에서 제외한다.
14. 보고서 마지막에 최대 네 단계의 다음 연습 계획과 바로 시작하는 CTA를 표시한다.
15. 기존 결과 보고서에서 음성 변환 원문 펼치기·내려받기 코드를 제거한다.
16. 일부 데이터만 있으면 있는 부분을 표시하고, 새 결과가 없는 과거 기록은 기존 결과 보고서로 보여 준다.
17. 작업 보내기 장치와 상태 복구 장치를 구현한다.
18. 가짜 API 응답을 사용하지 않는 실제 전체 흐름 시험을 작성한다.
...
- 목표 확인·수정·Revision 충돌·실행 당시 `briefRef`와 평가계획 Snapshot 고정 흐름이 동작한다.
...
- 민감한 정보가 화면에도 없다.
- P0 화면에는 Audio Clip과 Signed URL이 없다.
- 작업 등록 실패가 자동으로 복구된다.
...
- 별도 PracticeIntent 계약이 승인되면 목표 선택·수정 화면과 API 연결을 구현하고, 최영빈이 만든 목표 병합 결과를 화면에 표시한다.
- Audio Clip 정책이 승인되면 Owner 권한을 매 요청마다 확인하는 Evidence API와 Evidence Player를 구현한다. Editor 확대가 승인되기 전에는 Owner-only를 유지한다.
- Player는 정확한 구간만 재생하고 동시에 하나만 재생하며, 만료·삭제·권한 거부 시 글·시간·수치 근거로 돌아간다.
- Evidence Timeline에서 의미 누락·과속·습관어·멈춤·전환·낭독·잘 전달한 내용을 시간순으로 보여 준다.
- Report-to-Deck은 제안 문장을 자동 적용하지 않고 사용자가 승인한 뒤 `baseVersion` 충돌을 확인해 발표자 메모에 반영하며 되돌리기를 제공한다.
- 개인화 Prompter는 전체 대본을 기본 숨기고 남은 시간·핵심 단어 최대 세 개·반복 문제 한 개만 표시한다.
...
네 명이 동시에 Fixture와 전달 규칙을 준비한다.

이 단계가 끝나기 전에는 각자 다른 평가식이나 다른 데이터 모양을 만들면 안 된다.

## Phase 1 — 네 명 병렬 작업

- 최영빈: Brief 기반 목표·공통 평가기·우선 개선 3개와 CoachingAction
- 김동현: WPM·습관어·멈춤 v1·위치·시간 근거와 DTO
- 이창원: Python 파일 분리, 기존 Target 집중 연습, 질문 답변, 안정화·전체 재검증
- 임재환: 목표 확인 화면, 결과 보고서 API·화면, 다음 연습 계획과 복구 장치
...
- API Mock 없이 실제 데이터베이스·저장소·작업 대기 줄·Worker·Python Worker를 사용한다.
- 접근성·작은 화면·일부 결과·측정 불가·비교 불가·기존 보고서 Fallback을 함께 확인한다.
- 실패가 계산 문제면 최영빈·김동현·이창원에게, 화면·조립·이동 문제면 임재환에게 되돌린다.

## Phase 4 — P1 선행 결정과 후속 구현

1. 기능별 사용자 필요성과 개인정보 위험을 검토한다.
2. 공통 계약·보관 정책·지표 버전·Migration을 별도 PR로 승인받는다.
3. 12초 음성 재생, CPM, 멈춤 v2, 별도 PracticeIntent를 서로 독립된 작은 PR로 구현한다.
4. Lens·언어 분석·Adaptive Q&A·Timeline·문장 교정·Prompter·대본 축약을 담당별로 병렬 진행한다.
5. P1 결과가 기존 P0 결과를 소급 변경하지 않는지 기존 기능 확인 시험을 실행한다.
...
따라서 Phase 0을 먼저 병합하면 네 명이 현재 계약을 바꾸지 않고 Fixture를 이용해 P0를 병렬 구현할 수 있다. CPM·멈춤 v2·12초 음성 재생·별도 PracticeIntent는 선행 결정과 계약 PR이 승인된 뒤 P1로 시작한다.
