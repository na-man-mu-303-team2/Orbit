...
> **P1(피원)**: P0가 안정적으로 동작한 뒤 저장소 제품 문서의 Next·Later 후보 중 승인된 항목을 구현하는 고도화 범위
> **저장소(Repository)**: 프로젝트의 코드와 코드 변경 기록을 함께 보관하는 공간
> **검증일**: 2026년 7월 13일
> **검증 기준**: `origin/develop@1edda998`
> **공통 계약(Common Contract)**: 여러 프로그램이 같은 데이터 이름·형식·상태값을 사용하기로 정한 약속
> P0는 이 문서에서 제안하는 완료 대상이다. 현재 제품 문서와 다른 P0 확장은 제품 문서와 계약 PR에서 승인된 뒤 확정한다. P1은 저장소 제품 문서에서 Next·Later로 분류된 후보이며, 제품 문서와 계약 PR에서 승인된 항목만 Phase 4 이후 구현한다.
...
1. `AGENTS.md`
2. `docs/contracts.md`, `packages/shared`의 현재 공통 계약
3. `docs/product/adaptive-rehearsal-coach-direction.md`의 승인된 제품 범위
4. 담당자별 구현계획과 현재 코드의 구현 상태

현재 계약과 새 요구가 다르면 구현부터 시작하지 않는다. `현재 상태`, `승인된 목표 범위`, `필요한 선행 계약`을 분리하고 제품 문서와 계약 PR을 먼저 승인받는다. 타입·권한·보관 정책은 승인된 계약이 병합되기 전까지 기존 보안 경계를 유지한다.
...
| RehearsalFocusProfile(연습 의도 정보)      | 다음 연습에서 무엇을, 왜, 어느 부분까지 연습할지 정리한 데이터              |
...
| 구분 | 계획 범위 | 구현 시작 조건 |
| --- | --- | --- |
| P0 계약 선행 | 별도 RehearsalFocusProfile, CPM·STT Quality Gate·pause v2, 문장 Target, 12초 Audio Clip·기본 7일 보관·Owner-only Evidence API | 제품 문서 범위 승인과 Schema·Migration·지표 버전·보관·삭제·권한 정책 PR 승인 |
...
| P1 Next 후보 | Lens별 Rubric·개선 이력, 언어 구조·음량/에너지, Adaptive Q&A, Timeline, Report-to-Deck, Prompter 개인화·대본 단계, Editor의 Clip 접근 | P0 실제 E2E 통과와 제품 문서·계약 PR 승인 |
| P1 Later 후보 | 신뢰할 수 있는 발음 평가 등 검증 비용이 큰 기능 | Provider 검증·개인정보 검토·제품 문서 승인 |

P1 후보는 자동 확정 범위가 아니다. 저장소 제품 문서와 계약 PR에서 승인된 기능만 담당자·입출력·완료조건·시험을 지정해 구현하며, 승인되지 않은 Later 기능은 P0 완료를 막지 않는다.
...
| 담당   | 바로 시작할 작업                                                                    |
| ------ | ----------------------------------------------------------------------------------- |
| 최영빈 | CriterionResult 입출력·semantic 매핑·Top 3·RehearsalFocusProfile·CoachingAction 계약과 경계 Fixture 작성 |
| 김동현 | TypeScript→Python DTO, CPM/WPM·STT Quality Gate·filler·pause v2 capability·Clip 보관/삭제/권한 계약과 HTTP 422 시험 작성 |
...
- 현재 `origin/develop`의 C0 공통 계약을 기준으로 P0에 필요한 RehearsalFocusProfile·CPM·STT Quality Gate·pause v2·문장 Target·Audio Clip 확장을 additive Schema로 정의한다.
- 공통 Schema, API 계약, Migration, 보관·삭제·Owner-only 권한 정책, 지표 버전을 기능 구현 전에 작은 선행 PR로 병합한다.
- 모든 Fixture가 선행 계약 PR의 공통 Schema를 통과한다.
- P0는 CPM canonical·WPM 호환·근거가 없으면 `unknown`인 pause v2·Owner-only Clip·음성 원본 즉시 삭제 정책을 사용한다.
- 통과·부분 전달·실패·측정 불가·비교 불가 사례가 준비되어 있다.
- Brief·RehearsalFocusProfile Revision 충돌과 `briefRef`·RehearsalFocusProfile Revision·`evaluationPlan` Snapshot 고정 사례가 준비되어 있다.
- 습관어 1회/2회 경계 시험이 있다.
...
- P1 후보도 제품 문서와 기능별 선행 계약 PR이 병합되기 전에는 앱별 자체 enum·payload·저장 구조를 만들지 않는다.
...
2. Brief ID와 Revision, 그 Brief로 만든 평가계획, RehearsalFocusProfile Revision이 발표 시작 당시 Snapshot에 고정됐는지 검증한다.
...
2. Python 요청·응답 DTO는 이창원이 `main.py`의 Pydantic 경계에 반영하고, 김동현은 필드·단위·Fixture를 제공한다.
...
7. 자료가 충분할 때 한국어 공백 제외 글자 수를 녹음의 실제 전체 경과시간 `durationSeconds`로 나눠 공식 `charactersPerMinute`를 계산한다. 전체 시간이 없을 때만 첫 유효 STT 구간 시작부터 마지막 구간 종료까지를 fallback으로 사용하며, 기존 `wordsPerMinute`와 서로 환산하거나 과거 결과를 다시 쓰지 않는다.
8. 기존 습관어 목록과 단어·여러 낱말 표현 구분 방식을 사용해 단순 글자 포함 검사 없이 습관어를 계산하고 확인 가능한 발생 위치를 Evidence 원천값으로 만든다.
9. 기존 1초 이상 빈 구간인 `pause v1`을 유지하면서 `pause v2`에는 문장 사이·문장 중간·슬라이드 전환·판단 불가 위치를 기록한다. v1과 v2는 지표 버전이 다르면 비교하지 않는다.
10. pause v2의 의도한 멈춤·말막힘 분류는 승인된 Provider 근거가 있을 때만 기록하고, 근거가 없으면 실패로 추정하지 않고 `unknown`으로 둔다.
...
16. Clip은 프로젝트 Owner 전용으로 기본 7일 보관하고 조기 삭제·만료·저장 실패·삭제 재시도를 지원한다. 최대 30일 연장은 별도 제품·개인정보 정책 승인 뒤에만 허용하며 저장 URL이나 Signed URL은 결과·로그에 넣지 않는다.
17. 음성 변환 원문은 분석 중 Memory와 비영속 Private Evidence Redis에서만 사용한다. Private Redis의 cache key에는 원문을 넣지 않고 원문 값은 최대 30분 TTL로 저장하며, 데이터베이스·분석 결과·로그·Job payload/result에는 넣지 않는다.
...
- pause v2 분류 Provider 근거가 없을 때 의도·말막힘을 추정하지 않고 `unknown`으로 유지한다.
- Clip은 12초를 넘지 않고 기본 7일 만료·조기 삭제·삭제 재시도 시험이 통과한다. 30일 연장은 별도 승인 없이는 사용할 수 없다.
...
1. 리허설 시작 전에 기존 PresentationBrief와 별도 RehearsalFocusProfile의 사용자 목표 최대 세 개를 확인·수정하는 화면을 만든다.
...
17. 전체 리허설 전사 원문은 데이터베이스와 `report_json`에 저장하지 않고 `PRIVATE_EVIDENCE_REDIS_URL`의 비영속 Redis에 저장 시점부터 최대 30분 TTL로만 보관한다.
18. Report API는 매 요청마다 프로젝트 Owner 권한을 확인하고 Redis TTL이 살아 있을 때만 `transcriptRetained=true`와 전사 원문을 반환한다. cache miss·만료·Redis 오류에서는 `transcriptRetained=false`, `transcript=null`을 반환한다.
19. 결과 보고서 화면은 Owner에게만 전사본 펼쳐보기와 명시적인 DOCX 다운로드를 제공하고 TTL이 만료되면 전사본 섹션과 다운로드 CTA를 숨긴다. DOCX는 사용자가 요청한 시점에 브라우저에서 생성하고 Object URL을 즉시 해제하며 서버·Object Storage·데이터베이스에는 파일 사본이나 다운로드 URL을 저장하지 않는다. Editor·Viewer·다른 프로젝트 사용자는 전사 원문을 조회하거나 내려받을 수 없다.
20. 전사 원문은 React Query의 장기 cache, `localStorage`, URL, 오류 telemetry와 업무 로그에 넣지 않는다. 전사 cache 조회 실패나 DOCX 생성 실패는 보고서 전체 실패로 바꾸지 않고 해당 열람·다운로드 기능만 사용할 수 없는 상태로 표시한다.
21. 일부 데이터만 있으면 있는 부분을 표시하고, 새 결과가 없는 과거 기록은 기존 결과 보고서로 보여 준다.
22. 기존 `storage-deletion-reconciler.ts`의 삭제 outbox 처리를 재사용하고, durable coaching Dispatcher·오래 멈춘 attempt·미삭제 private audio 복구만 별도 Reconciler로 추가한다. 같은 삭제 작업을 수행하는 두 번째 주기 루프를 만들지 않는다.
23. 가짜 API 응답을 사용하지 않는 실제 전체 흐름 시험을 작성한다.
...
- 목표 확인·수정·Revision 충돌·실행 당시 `briefRef`, RehearsalFocusProfile Revision, 평가계획 Snapshot 고정 흐름이 동작한다.
...
- 프로젝트 Owner는 전사 원문이 Private Evidence Redis에 저장된 뒤 최대 30분 동안 결과 보고서 화면에서 펼쳐보고 DOCX로 내려받을 수 있다.
- Redis TTL 만료·cache miss·조회 실패 이후에는 Report API가 `transcriptRetained=false`, `transcript=null`을 반환하고 화면은 전사본 섹션과 다운로드 CTA를 숨긴다.
- 전사 원문은 Owner 전용 TTL Report API 응답과 사용자가 명시적으로 내려받은 로컬 DOCX를 제외하고 데이터베이스·`report_json`·Job payload/result·로그·청중 API에 포함되지 않는다.
- DOCX는 브라우저에서 생성하고 Object URL을 즉시 해제하며 서버·Object Storage·데이터베이스에 파일 사본이나 다운로드 URL을 남기지 않는다.
- Editor·Viewer·다른 프로젝트 사용자는 전사 원문을 조회하거나 내려받을 수 없다.
- 전사 원문 외 민감한 정보가 API 응답과 화면에 없다.
...
제품 문서에서 P0 확장 범위를 승인한 뒤, P0 계약 PR에서 RehearsalFocusProfile, CPM·WPM 호환, STT Quality Gate, pause v2, 문장 Target, 최대 12초 Clip·기본 7일 보관·Owner-only 접근을 먼저 고정한다.

네 명은 동시에 Fixture와 전달 규칙을 준비하되 계약 PR이 병합되기 전에는 앱 내부에 임시 Schema나 독자적인 평가식을 만들면 안 된다.

## Phase 1 — P0 네 명 병렬 작업

- 최영빈: Brief·RehearsalFocusProfile 기반 목표, 공통 평가기, 비교 불가 판정, 실제 문제 Top 3와 CoachingAction
- 김동현: CPM·WPM 호환, STT Quality Gate, 습관어, pause v1·v2 capability, 위치·시간 근거, 최대 12초 Clip과 DTO
...
Phase 5에는 Phase 4에서 제품 문서와 계약 PR이 승인된 P1 기능만 들어간다. 승인된 기능은 작은 PR로 나누고 담당별 완료조건과 통합 시험까지 끝낸다.

## Phase 6 — 승인된 P1 통합·회귀·보안 검증
...
- 리허설 전에 기존 Brief와 별도 RehearsalFocusProfile의 사용자 목표 최대 세 개를 확인하고 수정할 수 있다.
- Revision 충돌이 발생하면 다른 사람의 변경을 덮어쓰지 않는다.
- 실행 당시 `briefRef`, RehearsalFocusProfile Revision, 그 Brief로 만든 평가계획이 Snapshot에 고정된다.
- 첫 전체 발표가 완료된다.
...
- pause v1과 pause v2는 지표 버전을 구분하고, pause v2의 의도·말막힘은 승인된 Provider 근거가 있을 때만 표시하며 그렇지 않으면 `unknown`이다.
- 질문·답변 결과가 표시된다.
...
- Clip은 기본 7일 뒤 삭제되고 조기 삭제·삭제 재시도가 동작하며 전체 raw audio는 분석 뒤 삭제된다. 최대 30일 연장은 별도 제품·개인정보 정책 승인 뒤에만 허용한다.
- 음성 변환 원문은 Owner 전용 Report API와 화면에서 Private Evidence Redis TTL이 살아 있는 최대 30분 동안만 열람·DOCX 다운로드로 제공된다.
- 음성 변환 원문은 사용자가 명시적으로 내려받은 로컬 DOCX를 제외하고 데이터베이스·`report_json`·Job payload/result·로그·청중 API·브라우저 영구 저장소에 남지 않는다. DOCX 파일과 다운로드 URL도 서버에 저장하지 않고, 전체 음성 원본과 장기 Signed URL도 API·화면·로그에 노출되지 않는다.
- 실전 Prompter는 전체 대본을 기본 숨기고 남은 시간·핵심 단어 최대 세 개·미해결 반복 문제 최대 한 개만 표시한다.
- 기존 결과 보고서와 기존 리허설 기능이 계속 동작한다.

## 4-4. P1 계약 및 완료 검사

Phase 4에서 승인된 P1 기능에 대해서만 다음 질문에 모두 `예`라고 답한 뒤 구현한다. Later 후보는 별도 제품 문서 승인이 없으면 완료 범위에 포함하지 않는다.
...
최신 `origin/develop@1edda998`를 Static Inspection(정적 확인: 프로그램을 실행하지 않고 코드와 설정을 읽어 확인하는 방식)으로 검토한 결과는 다음과 같다.
...
- 기존 결과 보고서 API는 음성 변환 원문을 항상 `null`로 바꿔서 반환하므로 Owner의 30분 전사본 열람이 현재 동작하지 않는다.
- 웹에는 전사본 펼쳐보기와 DOCX 내려받기 코드가 남아 있지만 API가 원문을 반환하지 않아 도달할 수 없다. P0 정책에 맞춰 Owner 전용 30분 TTL API를 연결하고 두 기능을 모두 활성화해야 한다.
...
- 현재 전체 리허설 음성 원본은 분석 뒤 삭제하며 12초 Clip을 기본 7일 보관하는 계약은 없다.
- 현재 음성 근거 접근 정책은 Owner-only이며 Editor 접근 확대는 확정되지 않았다.
- 별도 RehearsalFocusProfile 계약은 없고 기존 PresentationBrief Revision을 가리키는 `briefRef`와 실행 당시 평가계획 Snapshot을 사용한다.
- 기존 공통 코칭 계약에는 제한된 시간 범위·의미 기준·문제 참조, 정해진 연습 이동 대상, 최대 네 단계의 다음 연습 계획을 담을 수 있다.
- 기존 반복 기록 계산은 실제로 비교 가능한 전체 발표만 정확히 골라내지 못한다.

따라서 제품 문서에서 P0 확장 범위를 먼저 승인하고, Phase 0에서 P0 계약 확장과 Migration·결정 기록을 병합한 뒤 네 명이 Fixture를 이용해 P0를 병렬 구현한다. P0에는 별도 RehearsalFocusProfile, 전체 경과시간 기준 CPM·STT Quality Gate·근거가 없으면 `unknown`인 pause v2, 문장 Target, 최대 12초 Clip·기본 7일 보관·Owner-only Evidence Player, 전체 대본을 숨기는 실전 Prompter가 포함된다. P0 완료 뒤에는 저장소 제품 문서의 Next 후보를 검토하고, 제품 문서와 계약 PR에서 승인된 기능만 P1으로 구현한다. 신뢰할 수 있는 발음 평가 등 Later 후보는 Provider 검증과 별도 승인이 필요하다.
