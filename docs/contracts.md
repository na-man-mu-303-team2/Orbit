...
`briefDraft`와 `briefExtraction`은 새 PPTX 가져오기 화면의 검토 입력이다. 이미 저장된
legacy Job result에는 두 필드가 없을 수 있으며 이 경우 각각 `null`로 파싱한다. Brief
추출 실패는 PPTX import를 실패시키지 않고 `fallback` draft를 반환한다.
...
- `presentation-brief.schema.ts`: Brief CAS, requirement server revision, approved reference hash, identity-free import draft.
...
`PresentationBrief.origin`은 `ai-generation`, `pptx-import`, `manual` 중 하나이며 최초
생성 후에는 변경할 수 없다. 기존 `content_json`에 값이 없으면 `manual`로 해석한다.
