...
- capability reader는 rolling deployment 호환을 위해 `version: "1" | "2"`를 수용한다. API가 발행하는 `designAgentCapabilities` 상수는 `version: "2"`, `addableElementTypes: ["text", "rect", "ellipse", "line", "polygon", "image", "chart", "table"]`, `canGenerateImages: true`를 사용한다. version `1` 요청은 계속 읽을 수 있지만 신규 발행자는 장식용 ellipse/line/polygon 추가, 기존 image 재배치, 비동기 slide-redesign Job의 이미지 생성을 v2로 광고한다.
- capability v2 배포 순서는 (1) Python/shared의 `1 | 2` tolerant reader 배포, (2) v2 필드를 실제로 지원하는 API 발행자 배포, (3) 안정화 확인 순이다. `version: "1"` reader 제거는 별도 호환성 정리 작업이며 이번 슬라이드 리디자인 Goal 범위 밖이다. 발행자를 reader보다 먼저 배포하거나 API와 Python의 배포 순서를 뒤집지 않는다.
- 슬라이드 리디자인 palette 선택은 `selectedPaletteOptionId`의 세 상태를 구분한다. 필드가 없으면 기존 동기 proposal 흐름을 유지하고, `null`이면 proposal 없이 `paletteOptions` 3개를 반환하며, 문자열이면 같은 사용자·프로젝트·`sessionId`에 저장된 option을 선택해 최종 proposal을 생성한다. 이 구분은 palette UI가 배포되기 전 기존 Web 요청에 새 2단계 흐름을 강제하지 않기 위한 호환성 계약이다.
- `paletteOptions`는 정확히 3개이며 첫 항목만 `isCurrentTheme=true`여야 한다. 각 option은 고유한 `optionId`와 `dominant`, `surface`, `text`, `focal`, `secondary` 색을 포함하고, `text`와 `dominant`의 contrast ratio는 4.5 이상이어야 한다. 첫 항목의 `focal`은 현재 theme의 `accentColor`를 유지한다.
- API는 배색 3안을 성공한 assistant message의 `context_json`에 저장한다. 선택 요청은 actor·project·session 범위 안에서만 이 값을 읽고, 존재하지 않거나 다른 session의 `selectedPaletteOptionId`는 Python worker 호출 전에 `BadRequestException`으로 거부한다. 선택 전 응답에는 proposal을 저장하지 않는다.
- palette 선택이 끝난 비동기 리디자인 시작점은 `POST /api/v1/projects/:projectId/design-agent/slide-redesign-jobs`다. 요청은 `sessionId`, `content`, 문자열 `selectedPaletteOptionId`, 현재 `context`를 사용하며 `null` palette 선택은 받지 않는다. 기존 `POST .../design-agent/messages` 동기 경로는 이 endpoint로 자동 전환하지 않는다.
- `slide-redesign`은 active internal Job type이며 generic `POST /jobs`와 `POST /api/v1/jobs`의 `publicCreatableJobTypeSchema`에는 포함하지 않는다. API는 현재 Deck의 `deckId`, `baseVersion`, `slideId`를 Job이나 request message 생성 전에 검증하고 stale `baseVersion`을 `ConflictException`으로 거부한다.
- API는 선택한 palette를 같은 actor·project·session의 성공 message에서만 해석한다. 전용 Job의 BullMQ payload는 `jobId`, `projectId`, `userId`, `requestMessageId`, `sessionId`, 질문, 현재 context, 최근 성공 history, capability manifest, 검증된 palette option을 shared schema로 검증한다. generic Job DB payload에는 session/message/deck/slide/version/palette option 식별자만 저장하며 질문 원문과 slide JSON을 복제하지 않는다.
- enqueue 성공과 실패 업무 로그에는 `jobId`, project/deck/slide/session/message ID, `baseVersion`, palette option ID만 기록한다. 사용자 질문, slide content, speaker notes는 로그에 기록하지 않는다. enqueue 실패 시 Job과 pending request message를 모두 `SLIDE_REDESIGN_ENQUEUE_FAILED`의 bounded error로 종료한다.
- slide redesign 진행 단계는 `interpreting`, `composing`, `coloring`, `ornamenting`, `illustrating`, `verifying` 순서다. progress payload는 `jobId`, `projectId`, `sessionId`, 현재 `stage`, 중복 없이 순서대로 완료된 `completedStages`, 선택적 `previewProposal`을 사용한다. 이미지 슬롯이 없으면 `illustrating`은 생략할 수 있다.
- slide redesign progress는 `roomId`, `sessionId`, `userId: "system"`, `payload`, `sentAt` 공통 realtime envelope을 사용한다. 별도 payload 내부 transport discriminator나 `websocketEventTypeSchema` 신규 값을 만들지 않으며, `previewProposal`은 적용 가능한 최종 Deck이 아니라 읽기 전용 중간 결과다.
- Web은 realtime progress를 `projectId`, `sessionId`, `jobId`가 현재 요청과 모두 일치할 때만 소비한다. realtime 연결이 끊기거나 terminal event를 받지 못하면 인증된 Job 조회를 polling fallback으로 사용하며, 다른 project/session/job의 event로 현재 preview나 terminal 상태를 갱신하지 않는다.
- intermediate `previewProposal`은 inline progress와 modal 모두 읽기 전용이며 apply action을 노출하지 않는다. `verifying`을 완료하고 shared proposal schema를 통과한 final proposal만 공통 apply 경계를 사용할 수 있고, 한 번의 apply는 editor history에 한 항목만 추가해 undo 1회로 원래 Deck 상태를 복구해야 한다.
- Python stage 응답은 optional `alignment`를 `exclude_none`으로 생략할 수 있다. shared reader는 생략을 `null`로 정규화하며, 다른 필드의 누락이나 구조 오류까지 허용하지 않는다.
- Worker가 raw SQL로 `design_agent_proposals`에 기록하는 `operations`, `interpretedIntent`, 영향·원본 element ID 배열, warning 배열은 PostgreSQL JSONB parameter로 명시적으로 JSON 직렬화한다. Job DB payload/result에는 prompt나 전체 slide JSON을 복제하지 않는다.
- Python compose artifact의 `imageRequests[]`는 `placeholderElementId`, `assetRole`, `needsGeneration: true`, bounded `prompt`, `alt`를 사용한다. 이 artifact는 Worker 내부 stage transport용이며 Job DB payload/result나 로그에 prompt를 복제하지 않는다. 요청이 없으면 `illustrating` stage를 생략한다.
- Worker는 한 슬라이드에서 첫 `needsGeneration` 요청만 해석해 AI 생성 상한을 1장으로 유지한다. 추가 슬롯은 styled rect placeholder로 남기며 layout·배색·장식 operations를 보존한다.
- `assetRole: atmosphere`만 generated image provider를 사용할 수 있다. prompt에는 text·logo·watermark 금지, 중앙부 가독성, 선택 palette를 포함하고 Worker 자체 timeout은 25초다. full-bleed atmosphere 결과는 `slideStylePatchSchema.backgroundImage`, 그 외 결과는 editable image `add_element`로 최종 proposal에 연결한다.
- `assetRole: evidence`는 license와 source provenance를 검증하는 공개 이미지 검색 provider만 사용한다. 검색 결과가 없거나 실패해도 AI 생성으로 대체하지 않는다. `assetRole: decoration`은 이미지 provider를 호출하지 않고 editable shape/placeholder를 유지한다.
- 이미지 생성 budget 초과, provider 예외·timeout, 검색 실패, storage 실패는 slide-redesign Job의 terminal failure가 아니다. 최종 proposal은 원래 layout operations와 bounded warning을 유지하고 성공하며, raw provider 오류는 warning·Job result·로그에 넣지 않는다.
- 성공한 이미지는 기존 `StoragePort` 경계에서 `purpose: "design-asset"`으로 저장하고 `project_assets`의 provider, source URL, author, license, 확인 시각, generation prompt, 사용자 budget scope를 기록한다. fallback 업무 로그는 bounded reason code/provider/request ID만 기록한다.
...
### Motion 추천 eligibility 계약

- `recommend-animation`은 일반 Design Agent 변경과 분리된 fail-closed 경로다. Web, API 요청, Python Worker, API apply는 같은 eligibility 정책을 각 경계에서 다시 평가한다.
- `activity`, `activity-results`, `snapshot` slide와 `importedMainSequenceCoverage = partial | unknown`인 imported slide는 `refused-unsafe`다. 보이는 본문 대상이 없는 authored slide는 `not-needed`다.
- imported slide는 `importRenderMode = editable | hybrid`, 유일한 `ooxmlSourceSlidePart`, `absent | complete` main-sequence coverage, authoritative TemplateBlueprint의 쓰기 가능한 stable target이 모두 확인될 때만 `applicable`이다. client가 보낸 source, coverage, element 목록은 권위 근거로 사용하지 않는다.
- stable target은 Deck element와 TemplateBlueprint source가 1:1로 대응하고, slide part와 shape locator가 유일하며, element type과 frame capability가 일치해야 한다. group, group child, fallback source, hidden·locked·배경·장식 요소는 대상에서 제외한다.
- API가 Python Worker에 전달하는 `motionImportContext`는 내부 strict 계약이며 `renderMode`, `sourceSlidePartPresent`, `importedMainSequenceCoverage`, `stableTargetElementIds`만 포함한다. 공개 message API나 Deck schema에는 이 필드를 추가하지 않는다.
- eligibility가 `applicable`이 아니면 Python을 호출하거나 proposal을 만들지 않고 bounded 사유만 반환한다. apply 시 Deck 또는 source capability가 바뀌었거나 animation target을 해석할 수 없으면 proposal을 `stale`로 만들고 patch를 한 건도 적용하지 않는다.
- `speakerNotes`는 추천 계산을 위한 bounded transient input으로만 사용할 수 있다. 원문은 `design_agent_messages.context_json`, Job payload/result, proposal metadata, 서버 로그에 저장하지 않으며 저장 context에는 존재 여부만 남길 수 있다.

### Semantic Motion Plan v2 계약

- Motion extractor의 신호 우선순위는 approved+current `semanticCues`, `aiNotes.compositionPlan`, 최대 4,000자의 transient `speakerNotes`, element role·group·geometry·z-order 순이다. suggested, excluded, stale cue와 eligibility allowlist 밖 target은 의미 근거로 사용하지 않는다.
- text 위계는 `normalizeRichTextProps()` 이후 Web renderer와 같은 `fontScale`·`lineSpaceReduction`을 적용한 effective typography를 사용한다. 명시적 `role`은 raw/effective font size보다 우선한다.
- LLM Structured Output은 strict `{ schemaVersion: 2, pattern, pacing, beats }` Semantic Motion Plan만 허용한다. beat는 `{ beatId, purpose, trigger, relation, targets }`, target은 `{ elementId, motionIntent }`만 가지며 `operations`, patch, `animationId`, effect/type, duration/delay/easing, 좌표, CSS, OOXML 필드는 금지한다.
- `motionIntent`는 `introduce`, `reveal`, `focus`, `support`, `compare`, `connect`, `conclude`만 허용한다. `pacing`은 `deliberate`, `balanced`, `brisk`만 허용하며 구체적인 effect와 시간값은 compiler가 결정한다.
- plan은 beat 최대 6개, click beat 최대 4개, entry beat 최대 1개, 전체 unique target 최대 8개, beat당 target 최대 4개, entry target 최대 2개다. 빈 beat, 중복 target, eligibility allowlist 밖 ID, unknown key, cap 초과가 있으면 응답 전체를 폐기한다.
- `on` mode의 provider unavailable, timeout, empty response, schema·allowlist 오류, compile 오류는 한 번만 재시도하고 성공하지 못하면 proposal과 assistant message를 만들지 않는다. silent fallback은 금지한다.
- 성공한 `on` proposal은 `source`, pinned `model`, `attemptCount`, `compilerVersion`, plan을 `motionPlan`에 저장한다. notes/excerpt, prompt, 사용자 text, provider raw output은 proposal metadata, assistant message, Job payload/result, 저장 context, 로그에 영속화하지 않는다. 기존 proposal은 `motionPlan`이 없을 수 있다.

### Semantic Motion Plan v3 계약

- v3는 작성 장표의 복합 대상을 `element`, `explicit-group`, `spatial-cluster` 의미 단위로 계획한다. 기존 v2 proposal은 같은 JSONB 컬럼에서 조회·미리보기·적용 호환을 유지하며 DB migration은 추가하지 않는다.
- 저장 metadata는 strict `units[]`와 `{ schemaVersion: 3, pattern, pacing, beats }` plan으로 구성한다. unit은 `unitId`, `kind`, `animationElementIds`, `memberElementIds`, `semanticRole`, `readingOrder`만 포함하고 beat target은 `{ unitId, motionIntent }`만 포함한다.
- plan은 의미 단위 최대 8개, beat당 단위 최대 4개, entry 단위 최대 2개, click beat 최대 5개, 전체 beat 최대 6개다. 단위 하나의 animation element는 최대 4개, 전체 compile 대상은 중복 없이 최대 32개다.
- LLM은 allowlist된 `unitId`, pacing, beat 의미만 선택한다. unit 구성원과 pattern은 서버가 결정론적으로 hydrate하며 member 누락·중복, 존재하지 않는 unit, 순차 단위 누락·역전은 응답 전체를 폐기한다.
- 작성 장표는 visible·unlocked 명시적 `group`을 우선한다. group이 없으면 canvas 35% 이하의 `rect | ellipse | polygon | star | ring | customShape` 중 자식 중심점이 내부에 있고 자식 면적의 80% 이상이 겹치며 자식 `zIndex`가 더 높은 최소 면적 컨테이너를 선택한다. `background | footer`, connector, 빈 decoration은 독립 단위가 될 수 없다.
- `timeline`, `process-vertical-rail`, `feature-comparison`, `diagram-hub`, `diagram-orbit` 작성 장표는 composition resolver registry가 stable ID family, role, geometry를 함께 검증한다. `timeline`은 marker·marker label·index·body를, `process-vertical-rail`은 rail/rule을 제외한 marker·marker label·body를 `timeline` family 원자 단위로 만든다. comparison은 field 또는 rule·index·body를, `diagram-hub`은 hub field·hub text와 각 node field·index·body를, `diagram-orbit`은 connector를 제외한 hub field·hub text와 각 node field·node body를 `diagram-hub` family 원자 단위로 만든다. vertical rail step과 orbit node는 3~6개이며 stable ID ordinal이 1부터 연속이어야 한다. 지원 composition의 필수 요소가 누락되거나 중복·모호하게 매칭되면 generic 추출로 부분 fallback하지 않고 `MOTION_AI_COMPILE_UNSAFE`로 종료한다. `MotionStructureFamily`와 외부 Motion Plan v3 wire schema는 이 composition 추가로 확장하지 않는다.
- slide type은 `aiNotes.visualPlan.visualType`과 composition ID를 우선 구조 신호로 사용한다. `process-horizontal | process-vertical-rail | timeline`은 `process`, `diagram-hub | diagram-orbit`은 `architecture`로 fallback한다. pattern은 서버가 `process → stepwise-process`, `feature-grid | architecture → cluster-reveal`, `comparison → paired-comparison`, `data | chart → evidence-then-insight`로 결정한다.
- process의 주요 카드 단위는 reading order대로 정확히 한 번 포함한다. 1~5단계는 제목을 entry에 두고 카드마다 click beat 하나를 사용하며 마지막 카드 뒤 결론 단위를 같은 final click의 `sequence` follower로 둔다. 6단계는 제목과 1단계를 entry에 두고 나머지 5단계를 click beat로 공개한다.
- v3 compiler version은 `motion-compiler-v3`다. 명시적 group은 group element 하나를 대상으로 하고 spatial cluster는 첫 요소를 root, 나머지를 `with-previous`로 compile해 단위 내부를 atomic하게 유지한다.

### Semantic Motion compiler 계약

- LLM plan은 animation patch로 직접 저장하지 않는다. `motion-compiler-v2`가 target role, `motionIntent`, `pacing`, beat 관계를 해석해 기존 animation operation으로 결정론적으로 compile한다.
- compiler가 새로 생성하는 effect는 `appear`, `fade-in`, `zoom-in`뿐이다. effect는 `motionIntent`와 semantic role의 고정 매핑으로, duration은 `pacing`의 고정 테이블로 결정하며 easing은 `ease-out`, delay는 0이다.
- v3 복합 단위의 구성원은 모두 같은 `appear | fade-in` 효과를 사용한다. 카드의 번호나 아이콘만 `zoom-in`으로 분리하지 않으며, 단위 내부 duration은 실제 animation element 수와 관계없이 한 번만 계산한다.
- `entry` beat root는 `on-slide-enter`, `click` beat root는 `on-click`이다. `together` follower는 `with-previous`, `sequence` follower는 `after-previous`를 사용한다. LLM이 effect, duration, start mode, easing, delay를 결정할 수 없다.
- 새 `animationId`는 compiler version, `deckId`, `slideId`, `baseVersion`, `elementId`, beat index의 stable hash로 생성한다. 같은 입력은 byte-identical operations를 반환하고 random UUID·현재 시각·model ID를 사용하지 않는다.
- entry 자동 재생은 900ms, click step 하나는 1,200ms, 전체 예상 motion은 6,000ms를 넘을 수 없다. compiler는 cap 초과 plan을 자르거나 일부 operation만 반환하지 않고 전체를 거부한다.
- `shadow`는 plan과 compiler를 실행해도 기존 proposal을 유지한다. `on`만 compiler proposal을 사용하며 eligibility, context, compile 실패는 기존 heuristic으로 우회하지 않고 `operations=[]`로 닫는다.

### Motion 병합 및 최종 안전 검증 계약

- compiler 결과는 기존 `slide.animations`를 대체하지 않는다. 기존 `animationId`, 지원하지 않는 기존 effect, 같은 slide의 `play-animation` action 참조를 그대로 보존하고 새 target만 추가한다. 이미 안전한 ORBIT effect를 조정할 때도 기존 `animationId`를 유지하는 `update_animation`만 허용한다.
- Design Agent Motion 경로는 Web/API 요청/Python Worker의 eligibility 검증에 더해 proposal 생성 직전과 apply 직전에 최신 authoritative Deck으로 animation graph와 canonical timeline을 다시 검증한다. duplicate animation ID/order, 존재하지 않는 target, dangling action reference, target 변경, allowlist 밖 target은 전체 proposal을 거부한다.
- 자동 생성 또는 수정 가능한 effect는 `appear`, `fade-in`, `zoom-in`뿐이다. 사용자가 명시적으로 기존 Motion 전체 교체를 요청하지 않은 `delete_animation`은 금지하며, 명시적 교체에서도 `play-animation` action이 참조하는 animation은 삭제할 수 없다.
- 병합 후에는 entry 900ms, click step 1,200ms, click root 5개, 전체 6,000ms의 canonical timeline budget을 다시 적용한다. v3 apply는 저장한 unit의 `animationElementIds`와 실제 add/update operation target을 정확히 한 번씩 대조하고 저장된 click beat 수와 canonical timeline root 수도 재검증한다. patch dry-run, graph, timeline, Deck schema 중 하나라도 실패하면 부분 operation을 저장하거나 적용하지 않고 `operations=[]` 또는 stale proposal로 fail-closed한다.
- undo/redo는 공통 Deck patch inverse를 사용하며 add→delete→add 왕복 뒤 기존 animation 배열과 action 참조가 byte-equivalent하게 복원되어야 한다. 공개 API와 Deck schema에는 Motion Planner 전용 metadata를 추가하지 않는다.
