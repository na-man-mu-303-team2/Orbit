...
- 프로젝트 목록의 각 항목은 `tags`와 최신 활성 `ai-deck-generation` 작업의 `generation` 요약을 포함한다. 활성 작업이 없으면 `generation`은 `null`이다.
...
- `timeline`, `feature-comparison`, `diagram-hub` 작성 장표는 composition resolver registry가 stable ID family, role, geometry를 함께 검증한다. timeline은 marker·marker label·index·body, comparison은 field 또는 rule·index·body, hub는 hub field·hub text와 각 node field·index·body를 원자 단위로 만든다. 지원 composition의 필수 요소가 누락되거나 중복·모호하게 매칭되면 generic 추출로 부분 fallback하지 않고 `MOTION_AI_COMPILE_UNSAFE`로 종료한다.
- slide type은 `aiNotes.visualPlan.visualType`과 `compositionId = process-horizontal | timeline`을 우선 구조 신호로 사용한다. pattern은 서버가 `process → stepwise-process`, `feature-grid | architecture → cluster-reveal`, `comparison → paired-comparison`, `data | chart → evidence-then-insight`로 결정한다.
