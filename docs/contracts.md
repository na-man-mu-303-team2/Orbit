...
- `POST /api/v1/projects/:projectId/slide-practice-analyses`
  ...
  - 신규 client는 `contentHashVersion: "slide-text-v1"`와 `slideContentHash`를 함께 보낸다. API는 현재 slide에서 hash를 재계산하며 불일치는 `409 SLIDE_PRACTICE_CONTENT_HASH_MISMATCH`로 거부한다. 필드가 없는 과거 client도 API가 server hash를 계산해 analysis row에 저장한다.
