...
- Report STT transcript 원문은 Python worker 응답에서 TypeScript worker 메모리로만 전달한다. API response, Job payload/result, DB, report, 로그에 저장하지 않는다.
...
- AI 코칭은 현재 slide의 `speakerNotes` 최대 6,000자, 습관어 집계, 말 속도, 쉼 비율, pitch 폭, 음량, versioned issue code만 transient input으로 사용한다. raw audio와 Report STT transcript는 코칭 provider에 보내지 않으며 generic Job payload/result와 로그에도 넣지 않는다.
- 저장 코칭은 최대 2개 개선 item, 30초 practice plan, model/prompt/policy version만 허용한다. 대본 수정이 필요한 경우 creator-private report에 실제 `speakerNotes`와 일치하는 원문·추천문을 item당 최대 1,000자로 저장할 수 있으며 worker가 원문 포함 여부를 재검증한다.
...
- `POST /api/v1/projects/:projectId/slide-practice-analyses`
  ...
  - API는 현재 deck ID/version과 slide ID/order를 다시 확인한 뒤에만 private upload를 만든다.
...
- `GET /api/v1/projects/:projectId/slide-practice-reports`
  ...
  - `deckId`, `slideId`, `cursor`, `limit` 필터를 지원한다.
...
- 신규 `reportVersion: 2`는 1초 단위 `loudnessSamples` 최대 300개와 5초 단위 `speedSamples` 최대 60개를 저장한다. 음량 sample은 `dBFS`, 속도 sample은 `syllablesPerSecond`만 포함한다. `reportVersion: 1` 기록은 sample과 코칭이 없는 상태로 계속 읽는다.
...
- worker는 생성 대상인 현재 slide를 `targetSlideId`로 고정하고, 같은 deck version의 모든 slide에서 제목·화면 텍스트·`speakerNotes`를 bounded transient context로 구성한다. 이 전체 deck context는 Python worker의 생성 입력에만 사용하며 generic Job payload/result, 로그, 별도 DB snapshot에 복제하지 않는다.
...
- web citation은 검색 응답의 cited excerpt를 별도 strict source-vetting 호출로 검증하고, 해당 주제를 책임지는 정부·학교·회사·표준기관·프로그램 운영 주체의 official source만 generation allowlist에 포함한다. cited excerpt는 생성 중 메모리에서만 사용하고 저장하거나 로그로 출력하지 않는다.
...
- 응답 지연을 제한하기 위해 official web search는 한 번만 시도하고 `search_context_size: low`를 사용한다. official citation vetting은 생성 전에 유지하며, 검색 실패 시 추가 검색 재시도 없이 전체 deck·대본·승인 참고자료로 생성을 계속한다.
