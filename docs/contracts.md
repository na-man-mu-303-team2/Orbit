...
- capability reader는 rolling deployment 호환을 위해 `version: "1" | "2"`를 수용한다. API가 발행하는 `designAgentCapabilities` 상수는 `version: "2"`, `addableElementTypes: ["text", "rect", "ellipse", "line", "polygon", "image", "chart", "table"]`, `canGenerateImages: false`를 사용한다. version `1` 요청은 계속 읽을 수 있지만 신규 발행자는 장식용 ellipse/line/polygon 추가와 기존 image 재배치를 v2로 광고한다.
