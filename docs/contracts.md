...
- 병합 후에는 entry 900ms, click step 1,200ms, click root 4개, 전체 6,000ms의 canonical timeline budget을 다시 적용한다. patch dry-run, graph, timeline, Deck schema 중 하나라도 실패하면 부분 operation을 저장하거나 적용하지 않고 `operations=[]` 또는 stale proposal로 fail-closed한다.
