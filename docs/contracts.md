...
- capability reader는 rolling deployment 호환을 위해 `version: "1" | "2"`를 수용한다. API가 발행하는 `designAgentCapabilities` 상수는 `version: "2"`, `addableElementTypes: ["text", "rect", "ellipse", "line", "polygon", "chart", "table"]`, `canGenerateImages: false`를 사용한다. version `1` 요청은 계속 읽을 수 있지만 신규 발행자는 장식용 ellipse/line/polygon 추가 가능성을 v2로 광고한다.
