...
`liveTranscript`는 발표 제어와 진단용이며 최종 습관어 통계의 authoritative source로
사용하지 않는다. 최종 습관어 원천 우선순위는 성공한 Realtime OOB 축어 전사,
`gpt-4o-mini-transcribe` 축어 전사, legacy report transcript 순이다. 축어 원천이 없으면
...
`verbatimCoachingSource`와 `verbatimCoachingTelemetry`를 제공한다. `verbatimCoachingSource`는 `mode`, `state`, `model`,
`promptVersion`, `classifierVersion`, `completedUtterances`, `totalUtterances`를 포함한다.
`verbatimCoachingTelemetry`는 transcript 없이 OOB 응답 성공/실패 수, 총/최대 latency,
input/output token 수와 mini fallback 발화 수만 집계한다.
...
- API runtime config 경로는 `LIVE_STT_ENGINE`과 secret이 아닌 `FILLER_TRANSCRIPTION_MODE`만 노출한다. web은 engine 값을 presenter localStorage의 `sttEngine`보다 우선하며, `web-speech`가 미지원이면 OpenAI로 자동 fallback하지 않는다.
...
- `GET /api/v1/runtime-config`
  ...
  - response: `{ "liveSttEngine": "openai-realtime", "fillerTranscriptionMode": "mini" }`
...
- `POST /api/v1/projects/:projectId/realtime-transcription/client-secret`
  ...
  - response: `{ "clientSecret": "ek_...", "expiresAt": 1790000000, "model": "gpt-realtime-whisper", "delay": "xhigh" }`
  - 서버 로그에는 OpenAI API key, client secret, raw audio, transcript 원문을 남기지 않는다.
- `POST /api/v1/projects/:projectId/realtime-transcription/oob-client-secret`
  - `FILLER_TRANSCRIPTION_MODE=realtime-oob`일 때만 발급하며 같은 인증·project read 권한을 요구한다.
  - response: `{ "clientSecret": "ek_...", "expiresAt": 1790000000, "model": "gpt-realtime-2.1" }`
  - 별도 precision session은 local VAD commit마다 latest audio item을 `conversation=none`인 text-only `response.create`로 전사하고, 12초 timeout 또는 연결 실패는 mini fallback으로 격리한다.
...
- `POST /api/v1/rehearsals/:runId/audio/complete`
  - request: `{ "fileId": "file_audio_1", "recordingDurationSeconds": 90.25, "utteranceBoundaries": [{ "utteranceId": "utterance_1", "sequence": 1, "startMs": 700, "endMs": 2500, "commitReason": "silence", "slideId": "slide_1", "deckRevision": 7 }], "oobVerbatimResults": [] }`
  ...
  - `oobVerbatimResults`는 optional이며 생략 시 빈 배열이다. 성공 항목은 `utteranceId`, `fragmentSequence`, `responseId`, 축어 `transcript`, `latencyMs`, 선택 token 수를 포함한다. 실패 항목은 transcript 대신 `connection | response-error | timeout`을 포함한다. API/Worker 로그에는 이 transcript를 남기지 않는다.
...
- Report STT transcript 원문, timestamp segment 원문, 습관어 축어 transcript 원문은 Python worker 응답에서 TypeScript worker 메모리로만 전달한다. API response, Job payload/result, DB, report, 로그에 저장하지 않는다.
...
- 말 속도와 timestamp 기반 대본 정렬은 기존 Report STT transcript를 authoritative source로 사용한다. `syllablesPerSecond`는 이 전사의 음절 수를 `activeSpeechMs`로 나누며 transcript 자체는 report에 넣지 않는다.
- 습관어 집계는 기존 Report STT와 병렬 실행한 `gpt-4o-mini-transcribe` 축어 결과만 authoritative source로 사용한다. Worker는 내부 Python 요청에 bounded `{ model, prompt, promptVersion: "korean-filler-verbatim-v1" }` 설정을 전달하며, prompt는 `음`, `어`, 반복, 말더듬, 재시작을 보존하고 존재하지 않는 습관어를 만들지 않도록 versioned 관리한다. Python worker는 오디오를 한 번만 읽고 두 전사를 실행한다.
- 주 Report STT가 실패하면 기존처럼 전체 분석을 실패시킨다. 습관어 축어 STT만 실패하거나 rolling compatibility 요청에 적용되지 않았으면 다른 음성 지표 분석은 성공시키고 습관어만 `unmeasured`로 반환한다. provider 로그에는 성공 여부, model, prompt version, 지연만 남기며 transcript와 raw audio를 남기지 않는다.
- 신규 리포트의 optional `fillers.measurement`는 `metricDefinitionVersion: 2`, `state`, `reasonCode`, `source.mode`, `source.model`, `source.promptVersion`만 저장한다. 성공은 `state: measured`, `reasonCode: null`이고 축어 실패는 `state: unmeasured`, `reasonCode: FILLER_VERBATIM_UNAVAILABLE`이다. 필드가 없는 기존 v1~v3 리포트는 `FILLER_VERBATIM_NOT_APPLIED`인 legacy `unmeasured`로 해석한다.
- `fillers.measurement.state: measured`인 기록만 습관어 추세, 무습관어 축하, GREAT 판정, `filler-use` 코칭 후보에 사용한다. legacy 0회는 축어 전사가 적용되지 않아 측정 불가로 표시하고, legacy 양수는 기존 전사에서 감지된 최소 횟수 참고 정보로만 표시한다.
...
`realtime-oob` 모드에서는 성공한 OOB fragment를 `utteranceId`와
`fragmentSequence` 순서로 결합해 authoritative source로 사용한다. 하나라도 실패한
발화와 OOB 결과가 없는 발화만 private 원본에서 clip을 추출해 mini로 보완한다. OOB와
mini가 모두 실패한 발화가 있으면 source는 `degraded`, 모두 실패하면 `unavailable`이다.
