...
`fit: contain` authored picture는 음수 `a:srcRect` letterbox와 ORBIT extension marker를 함께 기록한다. reimport는 marker 또는 음수 `srcRect`를 contain으로 복원하며, frame/source 변경 시 원본 media 크기로 `srcRect`를 다시 계산한다. media를 읽지 못하면 `IMAGE_CONTAIN_SOURCE_UNAVAILABLE`로 fail-closed한다.
...
- writable authored plain text/frame과 image source를 동기화한다. imported text는 importer가 `richText=full`을 증명하기 전까지 content 교체를 허용하지 않는다. 새로 추가된 plain text/rect/image 요소는 실제 OOXML `shapeId`, `elementType`, writable authored source mapping을 만들며, plain text의 font/size/bold/italic/underline/color/alignment/line height/body inset을 OOXML에 기록한다. image에는 alt description, relationship, media part를 함께 생성해 후속 편집도 같은 요소를 갱신한다. 내부 image asset은 같은 project의 uploaded image인지 검증한 뒤 Python worker에 전달하고, Python은 대상 picture relationship과 media part만 교체한다. advanced rich text, crop, table, unsupported element는 capability와 sync 양쪽에서 fail-closed다.
