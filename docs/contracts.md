...
- LLM Structured Output은 strict `{ schemaVersion: 1, pattern, beats }` Narrative Motion Plan만 허용한다. beat는 `{ beatId, purpose, trigger, targetElementIds, relation }`만 가지며 `operations`, patch, `animationId`, effect/type, duration/delay/easing, 좌표, CSS, OOXML 필드는 금지한다.
...
- provider unavailable, timeout, schema 오류, cap 위반은 같은 slide type·role·approved cue·reading order 입력에서 결정론적으로 같은 fallback Narrative Motion Plan을 만든다. fallback도 좌표 y/x만으로 위계를 정하거나 raw animation operation을 만들지 않는다.
- Narrative Motion Plan과 notes/excerpt는 proposal metadata, assistant message, Job payload/result, 저장 context, 로그에 영속화하지 않는다. `shadow` mode는 기존 proposal을 변경하지 않고 원문 없는 bounded 집계만 비교한다.

### Semantic Motion compiler 계약

- LLM plan은 animation patch로 직접 저장하지 않는다. `motion-compiler-v1`이 target role과 beat 관계를 해석해 기존 `add_animation` operation으로만 결정론적으로 compile한다.
- compiler가 새로 생성하는 effect는 `appear`, `fade-in`, `zoom-in`뿐이다. title/subtitle은 `fade-in` 400ms, body/label/supporting은 `appear` 300ms, focal/media/data는 `zoom-in` 450ms이며 easing은 `ease-out`, delay는 기본 0이다.
