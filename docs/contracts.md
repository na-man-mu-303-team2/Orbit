...
- `validation.designIssues`는 overflow, contrast, collision, safe area, density, placeholder media 같은 advisory warning을 담는다. 이 warning이 있어도 `validation.passed`가 true이면 worker는 deck을 저장한다.
