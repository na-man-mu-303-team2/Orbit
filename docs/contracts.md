...
- `layout-compile` 전에는 승인된 `content-planning` artifact에서 `order`, `title`, `message`만 projection해 `outline`과 `deck=null`을 반환한다. layout 이후에는 `generateDeckResponseSchema`를 통과한 Deck을 사용하고 성공 checkpoint에 연결된 `image-slide` artifact만 같은 `slideId`에 병합한다. 성공한 `semantic-quality` 또는 `rendered-visual-quality` artifact가 있으면 가장 최근 검증 Deck을 우선한다.
