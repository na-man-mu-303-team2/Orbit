...
- `text.props`는 `text`, `runs`, `paragraphs`, `bodyInset`, `fontFamily`, `fontSize`, `fontWeight`, `italic`, `underline`, `color`, `align`, `verticalAlign`, `lineHeight`, `bullet`을 사용한다. paragraph와 run도 `italic`, `underline`을 optional boolean으로 보존한다. 세 계층의 두 필드에는 schema default를 두지 않으므로 생략된 기존 Deck을 parse할 때 `false`를 materialize하거나 Deck version을 증가시키지 않는다. `runs`는 기존 단일 paragraph 호환 field이고, `paragraphs`는 PPTX OOXML import에서 paragraph별 run/font/color/spacing/indent/bullet을 보존하기 위한 optional field다. `bodyInset`은 PPT text box 내부 여백을 px 단위로 보존한다.
...
- AI PPT wizard의 입력 UI는 `발표 주제`, `발표 내용`, `청중`, 복수 첨부파일만 받는다. `prompt`는 발표 내용, `brief.audienceText`는 청중을 사용하고 `targetDurationMinutes=10`, `slideCountRange={ min: 5, max: 8 }`, `mediaPolicy=minimal`, 한글 추천 폰트 첫 항목, generic `general-novice` coaching context를 내부 기본값으로 사용한다.
- 첨부파일이 있으면 `references-first`, 없으면 `user-input-only`를 `referencePolicy`, `brief.referencePolicy`, `design.referencePolicy`에 동일하게 사용한다. Web은 별도 reference extraction Job을 시작하지 않고 업로드된 `referenceFileIds`를 GenerateDeck request에 직접 전달하며 staged coordinator가 OCR fan-out을 소유한다.
...
- bounded repair 이후 rendered Visual QA가 `passed=false`여도 남은 issue가 `BALANCE_WEAK`, `LAYOUT_REPETITIVE`, `BACKGROUND_RHYTHM_FLAT`, `CARD_OVERUSED`로만 구성되면 영향 slide 수와 관계없이 advisory warning과 함께 발행한다. 이때 `visualQaStatus="advisory"`, `GENERATE_DECK_VISUAL_ADVISORY`, issue code와 영향 slide order를 diagnostics에 기록한다. allowlist 밖의 visual issue, blocking issue, unresolved placeholder 또는 schema 위반은 기존처럼 terminal이며 `GENERATE_DECK_VISUAL_QUALITY_GATE_FAILED`를 사용한다. 렌더 또는 Vision provider를 사용할 수 없더라도 semantic/deterministic validation에 blocking issue와 unresolved placeholder가 모두 없으면 `GENERATE_DECK_VISUAL_QA_UNAVAILABLE` warning과 `visualQaStatus="unavailable"`을 남기고 발행한다. terminal failure에서는 후보 Deck과 validation을 `jobs.result`에 보존하고 기존 project Deck은 변경하지 않는다. semantic/deterministic validation의 non-blocking issue만 남고 rendered Visual QA가 통과한 경우도 warning과 함께 발행한다.
...
- `slides[].sourceSlidePart`: presentation order와 물리 part 이름이 다를 수 있으므로 `ppt/slides/slideN.xml` 형식의 실제 writable slide part를 기록한다. operation routing은 이 값을 우선 사용한다. legacy blueprint에서 누락된 경우 writable `elementSources[].slidePart`의 안전한 고유값이 정확히 하나일 때만 파생하며, 0개이거나 여러 개면 fail-closed한다.
- `slides[].elementSources[]`
- `slides[].ooxmlOrigin`
- `slides[].ooxmlMotionCapabilities`
...
imported Deck의 full PUT은 저장 version을 `current + 1`로 정규화하고, 변경된 요소를 `add_element`, `delete_element`, `update_element_frame`, `update_element_props` 형태의 synthetic patch로 기록한다. transition 차이는 `update_slide_transition`, animation 차이는 `add_animation`, `update_animation`, `delete_animation`으로 기록한다. slide add/delete/reorder와 theme/style처럼 현재 synthetic OOXML operation으로 완전히 표현하지 못하는 구조 변경은 저장 전에 `OOXML_CHANGE_UNSUPPORTED`로 거부한다. 일반 Deck의 full replacement와 snapshot 동작은 기존 계약을 유지한다.
...
`update_element_props`는 authored canonical text와 authoritative source mapping이 일치하는 imported text, `imageSource=true` picture의 `src`/`alt`, 그리고 `crop=picture|picture-fill` image의 `crop` 교체·초기화를 허용한다. text serializer의 top-level 지원 field는 `text`, `runs`, `paragraphs`, `bodyInset`, `fontFamily`, `fontSize`, `fontWeight`, `italic`, `underline`, `color`, `align`, `verticalAlign`, `writingMode`, `lineHeight`, `bullet`이며 targeted sync의 `fontWeight`는 lossless round-trip이 가능한 `normal | bold`만 허용한다. 실제 `elementType`과 OOXML shape kind가 일치해야 하며 `src`와 `crop`이 같은 operation에 있으면 둘 다 누적 적용한다. `update_element_frame`은 geometry와 `zIndex`만 허용하며 `zIndex`는 slide shape tree의 실제 XML 순서를 결정적으로 바꾼다. 미지원 필드나 operation은 fail-closed 대상이다.

table의 imported targeted sync는 authoritative source mapping의 `tableCellText=true`, complete row-major `tableCellLocators`, live fingerprint, direct unmerged rectangular `p:graphicFrame/a:tbl`을 모두 다시 검증한다. API는 정확히 한 cell의 `text`만 달라지고 기존 paragraph 수를 유지하는 `{ rows }` patch만 저장하고, Python은 target text node와 필요한 `xml:space`만 바꿔 기존 `a:tcPr`, paragraph/run property, 다른 cell, `a:tblPr`, `a:tblGrid`, frame transform을 보존한다. 빈 paragraph에 처음 문구를 넣을 때는 existing `a:endParaRPr`를 `a:rPr` template으로 복제한다. 두 cell 이상 변경, newline에 의한 paragraph 수 변경, style/span/track/row/column 변경, locator·fingerprint drift는 각각 `TABLE_CELL_CAPABILITY_UNSAFE` 또는 `TABLE_STRUCTURE_UNSUPPORTED`로 package 전체를 fail-closed한다.

imported Deck에서 ORBIT가 새로 만든 `ooxmlOrigin=authored` table은 rectangular unmerged modeled table만 지원한다. add 시 `p:graphicFrame/a:tbl`과 authored source mapping을 만들고, 후속 cell 또는 row/column 변경은 owned `a:tbl` subtree만 재생성한 뒤 모든 `tableCellLocators`를 갱신한다. imported table의 row/column 구조 변경은 비활성화하며 generic/manual Deck의 table export도 같은 serializer support matrix를 통과해야 한다.

### OOXML provenance와 편집 capability

Deck과 TemplateBlueprint는 다음 optional 필드로 OOXML 보존 가능 범위를 전달한다. optional인 이유는 legacy/non-import Deck을 계속 읽기 위해서이며, `metadata.sourceType === "import"`인 item에서 필드 누락은 지원으로 추정하지 않는다.

- element/slide `ooxmlOrigin`: `imported | authored`
- element `ooxmlEditCapabilities.richText`: `none | style-only | full`
- element `ooxmlEditCapabilities.crop`: `none | picture | picture-fill`. writable raster image의 locator가 relationship과 일치하는 direct `p:pic/p:blipFill/a:blip`이면 `picture`, direct `p:sp/p:spPr/a:blipFill/a:blip`이면 `picture-fill`이다. layout/master/locked/fallback source, SVG, locator·relationship 불일치는 `none`이다.
- element `ooxmlEditCapabilities.tableCellText`: boolean
- element `ooxmlEditCapabilities.frame`: boolean. 단일 `(slidePart, shapeId)` source이며 group-local 역변환 없이 현재 serializer로 geometry/order를 쓸 수 있을 때만 `true`다.
- element `ooxmlEditCapabilities.delete`: boolean. `frame` 안전성에 더해 원본 main sequence가 `absent`여서 dangling motion target을 만들지 않을 때만 `true`다.
- element `ooxmlEditCapabilities.imageSource`: boolean. writable `p:pic` relationship/media를 안전하게 교체할 수 있을 때만 `true`다. `frame`/`delete`에서 추론하지 않는다.
- slide `ooxmlSourceSlidePart`: 원본 physical slide와 연결되는 stable `ppt/slides/slide*.xml` locator. slide reorder 뒤에도 바뀌지 않으며, 누락되거나 TemplateBlueprint에서 유일하게 일치하지 않으면 motion 편집을 fail-closed한다.
...
`importedMainSequenceCoverage`는 현재 package의 가변 편집 상태가 아니라 importer가 원본 main sequence를 이해한 정도를 기록한 불변 provenance다. `absent`는 원본 sequence가 없었다는 안전 증거이므로 ORBIT가 sequence를 새로 만든 뒤에도 writable 판정에 사용할 수 있다. `unknown`과 `partial`은 fail-closed다.

OOXML motion sync는 patch payload 자체를 serializer 입력으로 사용하지 않는다. Worker가 unsynced patch에서 transition/animation이 닿은 slide를 식별한 뒤 최신 persisted Deck의 transition과 animations full-state를 `slide_motion`으로 전달한다. transition scope는 `transitionWritable: true`, animation scope는 `importedMainSequenceCoverage: absent | complete`에서만 쓸 수 있다. transition-only sync는 기존 `p:timing` subtree를 변경하지 않고, animation sync는 supported main entrance sequence만 원자적으로 교체하며 interactive/media sibling branch를 보존한다. Python이 scope를 거부하거나 applied acknowledgement가 불완전하면 package와 synced version은 갱신하지 않는다.

PPTX import quality report는 필요할 때 optional `motionDiagnostics`를 포함한다. 구조는 `{ total, unsupported, downgraded, unresolved, excluded, details }`이며 `total`은 네 category 합과 같아야 한다. `details`는 최대 500개의 `{ slideIndex, code, count }`만 허용하고 detail count 합도 `total`과 같아야 한다. 500개를 넘는 경우 aggregate count는 유지하되 details를 비워 bounded contract를 지킨다. 상세 code는 shared enum의 content-free 값만 사용하고 원문 text, 발표자 script, raw media는 diagnostics에 포함하지 않는다.

Python importer는 imported slide와 source mapping에 capability를 기록하고, Worker는 이를 검증해 Deck slide/element로 전달한다. Imported Deck에서 ORBIT가 추가하거나 복제한 item은 `ooxmlOrigin: authored`로 바꾸고 imported capability/source mapping을 재사용하지 않는다. `authored` 자체는 지원 증거가 아니며 현재 add/update serializer support matrix를 통과해야 한다.

API는 client가 제출한 provenance/capability를 권한 정보로 신뢰하지 않는다. 기존 item의 필드는 저장된 서버 값으로 복원하고, 신규 `add_element`/`add_slide`와 full PUT 신규 item은 재귀적으로 `authored`로 지정하면서 imported capability를 제거한다. Imported Deck의 `metadata.sourceType`도 client full PUT으로 바꿀 수 없다.

Web은 공용 `resolveOoxmlEditCapability` 결과로 control의 enabled 상태와 한국어 disabled reason을 함께 결정한다. manual/AI Deck은 generic exporter support matrix를 사용한다. Imported Deck은 imported item의 explicit capability 또는 authored serializer matrix를 사용하며, provenance/capability가 없는 legacy item은 비활성화한다.

하나의 native `p:sp`가 fill/text 등 여러 ORBIT element로 분리되면 같은 `(slidePart, shapeId)`의 shared-shape cohort로 취급한다. frame/delete는 cohort 전체가 같은 operation과 같은 frame으로 완전한 round를 이룰 때만 Python에서 원자적으로 한 번 적용하며, 부분·불일치 round는 `SHARED_SHAPE_COHORT_UNSAFE`로 package 전체를 보존한다. Web의 단일 요소 control은 이 cohort capability를 `false`로 표시한다.

`fit: contain` authored picture는 명시적 crop이 없을 때 음수 `a:srcRect` letterbox와 ORBIT extension marker를 함께 기록한다. reimport는 marker 또는 음수 `srcRect`를 contain으로 복원한다. 양수 명시적 `crop`은 `a:srcRect`를 전적으로 소유하며 frame/source 변경에도 유지되고, `crop: null`은 direct `srcRect`를 제거한다. 명시적 crop이 없는 contain image의 frame/source 변경만 원본 media 크기로 `srcRect`를 다시 계산하며, media를 읽지 못하면 `IMAGE_CONTAIN_SOURCE_UNAVAILABLE`로 fail-closed한다.

### Fail-closed sync 응답

Python sync response는 다음 bounded 배열을 반환한다.
...
각 배열은 최대 500개다. 500개가 넘는 unsynced visual operation은 Worker가 순서를 유지한 여러 batch로 실행하되 shared-shape complete round는 batch 경계에서 분할하지 않는다. Python은 한 batch에서 unsupported operation이 하나라도 있으면 package bytes와 source mapping을 원본 상태로 반환한다. Worker는 전송한 visual operation의 순서·type·`slideId`·`elementId`가 `appliedOperations`와 정확히 일치하는지 확인하며, expected operation이 0개인 경우의 추가 acknowledgement도 거부한다. 성공한 delete locator는 다음 batch와 최종 TemplateBlueprint에서 제거하여 numeric `shapeId` 재사용과 충돌하지 않게 한다. `unsupportedOperations`가 하나라도 있거나 응답 확인이 불완전하면 `PPTX_OOXML_SYNC_UNSUPPORTED_OPERATION` retry 불가 오류로 종료한다. 이때 새 asset, `currentPackageFileId`, `ooxmlSyncedDeckVersion`, patch compaction을 commit하지 않는다. DB transaction 뒤쪽에서 실패하면 이번 시도에서 새로 쓴 storage object를 보상 삭제하며, 삭제 실패는 원래 sync 오류를 가리지 않고 안전한 ID/count만 기록한다. Job error와 log에는 operation type, reason code, target ID만 포함하고 text, image bytes, signed URL은 포함하지 않는다.

단순 fidelity warning은 저장이 안전하고 원본 데이터가 유지되는 경우에만 허용한다. serializer가 적용하지 못한 사용자 편집은 warning으로 성공 처리하지 않는다.
...
- writable authored canonical rich text/frame, image source, image crop, supported authored table을 동기화한다. imported text는 writable direct text shape와 고유 authoritative source mapping을 검사해 `richText=full | style-only | none`을 기록한다. `full`은 canonical paragraph/run content와 style을 동기화하고, `style-only`는 semantic text 및 기존 paragraph 경계를 유지하면서 modeled style만 반영한다. 이때 기존 run 경계를 target 경계와 합쳐 서로 다른 hyperlink relationship을 유지하고, `a:rPr`의 unknown attribute/child는 template 복제 또는 부분 갱신으로 보존한다. field 또는 안전하게 reconcile할 수 없는 구조는 `none`이며 `RICH_TEXT_CAPABILITY_UNSAFE`로 package와 synced version을 갱신하지 않는다. 새로 추가된 text/rect/image/table 요소는 원본 locator를 재사용하지 않고 실제 OOXML `shapeId`, `elementType`, writable authored source mapping을 만들며, authored text는 canonical paragraph/run, bullet, spacing, indent, body inset과 writing mode를 기록하고 `richText=full`을 사용한다. image에는 alt description, relationship, media part와 `crop: picture` capability를 함께 생성해 후속 편집도 같은 요소를 갱신한다. imported `p:pic`과 shape picture-fill은 각각 direct `a:srcRect`만 수정하며 안전한 locator가 없으면 `CROP_CAPABILITY_UNSAFE`로 fail-closed한다. 내부 image asset은 같은 project의 uploaded image인지 검증한 뒤 Python worker에 전달하고, Python은 대상 picture relationship과 media part만 교체한다. imported table cell text는 authoritative locator/fingerprint가 일치할 때만 갱신하고, imported table structure와 그 밖의 unsupported element는 capability와 sync 양쪽에서 fail-closed다.
- group 내부 child의 frame은 group-local 좌표 역변환을 지원하기 전까지 `GROUPED_FRAME_UNSUPPORTED`로 실패하며 원본 package bytes와 synced version을 보존한다. grouped child의 plain text/image source 동기화는 계속 허용한다.
...
- `ai_deck_execution_artifacts`는 `(pipeline_job_id, stage, shard_key)` UNIQUE이며 `image-slide`, `semantic-quality`, `rendered-visual-quality`, `publication`의 shared schema 검증 결과만 저장한다. `(pipeline_job_id, project_id)`는 부모 Job, `(pipeline_job_id, stage, shard_key)`는 해당 checkpoint를 `ON DELETE CASCADE`로 참조한다. legacy image artifact는 한 slide와 warning을 저장한다. v2 completed slide artifact는 strict `{ artifactVersion:2, sourceOrder, order, slideId, slide, warnings, validation }`이며 identity가 manifest와 일치해야 한다. quality artifact는 검증된 worker payload만, publication artifact는 최종 Job result만 저장하며 locator UUID는 같은 stage/shard upsert에서 유지한다.
...
- legacy `layout-compile` artifact는 검증된 worker payload와 visual requirements를 유지한다. v2 `layout-compile`은 전체 Deck을 미리 만들지 않고 strict `{ artifactVersion:2, deckShell, slides, warnings }` manifest를 저장한다. v2 manifest의 모든 slide는 `001-slide_1` 형식의 zero-padded `shardKey`로 기존 `image-slide` checkpoint에 fan-out한다. 각 shard가 Story 고정 필드 검증, slide 상세 생성, layout compile, asset resolution과 bounded QA를 끝낸 뒤에만 completed slide artifact를 저장한다.
...
- 조회와 변경 endpoint는 각각 `GET /api/v1/projects/:projectId/jobs/:jobId/story-plan`, `POST .../story-plan/edit`, `POST .../story-plan/regenerate`, `POST .../story-plan/approve`, `POST .../story-plan/cancel`이다. 모두 기존 인증과 project write 권한 검사를 재사용한다. edit는 strict discriminated union `{ kind: "reorder", expectedRevision, orders } | { kind: "speaker-notes", expectedRevision, order, speakerNotes }`, regenerate는 strict `{ expectedRevision, instruction? }`, approve는 하위 호환되는 strict `{ expectedRevision, slides?, designSelection? }`이며 cancel body는 사용하지 않는다. `slides`의 각 항목은 `sourceOrder`, `title`, `message`만 받고 배열 순서가 승인할 최종 순서다. `designSelection`은 `paletteOptionId`, 전체 `paletteOverride`, strict `fontOverride`만 받는다. instruction은 trim 후 최대 240자, speakerNotes는 trim 후 1~5000자다.
- `StoryPlanReviewResponse`는 strict `{ jobId, projectId, status, styleContext, plan, error }`다. `styleContext`는 후속 Style & Color 화면에 필요한 안전한 `{ topic, tone }`만 제공한다. `status`는 `planning | review-pending | regenerating | approved | failed | cancelled`다. 공개 plan에는 revision, 재제안 횟수와 한도 5회, outline, 예상 시간, slide 수, 생성 시각, 품질 warning, repair reason과 slide별 order/type/title/message/speaker notes/예상 시간/source 상태·안전한 metadata만 포함한다. raw source·OCR·provider 응답·URL·내부 prompt는 노출하지 않는다.
...
- edit는 review와 `content-planning` artifact row를 같은 transaction에서 잠그고 `expectedRevision`을 검증한다. reorder는 현재 slide order 전체를 중복 없이 받아 artifact의 `slidePlans` 배열과 각 order를 함께 재정렬하고 outline 제목 순서도 갱신한다. Web Story Review는 제목·핵심 메시지·순서만 편집하며 대본 탭을 노출하지 않는다. 하위 호환 speaker-notes API로 직접 저장된 non-empty 대본은 slide 상세 생성보다 우선한다. 성공 시 review revision을 증가시키며, 이 artifact가 approve 이후 `design-planning`과 `layout-compile` 입력으로 그대로 사용되므로 편집 결과가 최종 Deck 순서와 speaker notes에 반영된다. stale revision, 누락·중복 order, review 대기 상태가 아닌 요청은 `409`다.
- approve는 Job, review와 `content-planning` artifact row를 잠근 뒤 `expectedRevision`을 검증한다. `slides`가 있으면 `sourceOrder`의 중복·누락을 검증하고 배열 순서와 title/message 및 outline slide title만 교체하며 `slideType`, `speakerNotes`, `targetSeconds`, `sourceRefs`는 보존한다. `designSelection`이 있으면 선택한 palette/font를 Job payload와 artifact의 `rawInput.design`에 반영한다. artifact 수정, review 승인, `design-planning` checkpoint enqueue는 같은 transaction에서 처리한다. 동일 revision의 중복 approve는 멱등이며 stale revision, 잘못된 slide 집합, 승인할 수 없는 상태는 `409`다.
- 승인된 Story Review의 `content-planning` artifact는 사용자 확정 콘텐츠로 불변 취급한다. 후속 `design-planning`은 각 slide의 확정 `slideType`으로 composition을 선택하고 media intent와 디자인·배치만 결정하며 slide 순서, title, message, content items, speaker notes를 축약하거나 첫·마지막 slide 역할 또는 CTA로 다시 쓰지 않는다.
...
- regeneration instruction과 이전 outline 제목은 Python `content-planning` input의 선택 context로만 전달한다. instruction은 근거가 아니며 source 제약, 사실 경계와 reference policy를 덮어쓸 수 없다. staged v2 `content-planning`은 deck title과 slide별 title/message/slideType/sourceRefs만 한 번의 strict LLM 호출로 만든다. speaker notes, contentItems, keywords, visual/media intent는 `/internal/ai/deck-generation/slide-compose`에서 해당 slide가 참조하는 source excerpt만 사용해 생성한다. monolith full content 경로는 유지한다.
...
- Web은 내용 입력에서 생성 Job을 시작하고 `storyReviewRequired=true` 응답이면 generic project route보다 먼저 매칭되는 `/project/:projectId/story-plan/:jobId`로 이동한다. review UI는 `목차`와 `대본` 두 탭만 제공하고 두 탭은 동일한 slide 순서를 사용한다. 각 slide는 draggable card와 keyboard/mobile용 위·아래 이동 버튼을 제공하고 title/message를 직접 편집할 수 있으며, 대본은 직접 편집·저장하거나 기존 AI 재제안 instruction으로 다시 생성할 수 있다. 승인 전 Story 초안은 Style & Color handoff 시 현재 탭의 `sessionStorage`에만 저장하고 서버 autosave나 별도 draft table은 사용하지 않는다. `스타일 선택` action은 아직 approve하지 않고 `/project/:projectId/style-color/:jobId`로 이동한다. Style & Color에서 폰트, 9+1 팔레트와 live preview를 확인한 뒤 Story 초안과 디자인을 함께 approve하고 `/project/:projectId/generation/:jobId`로 즉시 이동한다. `topic`처럼 사용자 입력인 source는 대본 근거로 반복 표시하지 않고, `web | uploaded` source의 안전한 title/type만 해당 대본 아래에 작게 표시한다. 사실이 “검증됨”이라고 표시하거나 Visual QA 결과를 미리 예측하지 않으며, `repairReasonCodes`는 AI가 자동 조정한 내용으로만 안내하고 정확한 token 절감량은 표시하지 않는다.

### AI Deck Progressive Preview

- `GET /api/v1/projects/:projectId/jobs/:jobId/deck-preview`는 project read 권한과 `type='ai-deck-generation'` Job identity를 확인하고 strict `AiDeckPreviewResponse`를 반환한다. 응답은 `{ jobId, projectId, status, progress, editable:false, outline, deck, completedSlideIds, pendingSlideIds, updatedAt, error }`만 포함하며 raw source, OCR, prompt, provider response, 내부 layout/visual requirement는 노출하지 않는다.
...
- `apps/api/src/database/migrations/2026071504000-CreateAiDeckReferenceExtractionArtifacts.ts`
...
- `sourceElementIds`는 현재 선택된 visible 요소의 부분집합이어야 한다. 지원 프리셋이 없는 layout type/항목 수 조합은 다른 크기의 프리셋으로 대체하거나 항목을 누락하지 않고 요청을 실패시킨다.
