...
- 승인된 Story Review의 `content-planning` artifact는 사용자 확정 콘텐츠로 불변 취급한다. 후속 `design-planning`은 media intent와 디자인·배치만 결정하며 slide 순서, title, message, content items, speaker notes를 축약하거나 CTA로 다시 쓰지 않는다.
