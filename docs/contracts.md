...
- bounded repair 이후 rendered Visual QA가 `passed=false`여도 남은 issue가 `BALANCE_WEAK`, `LAYOUT_REPETITIVE`, `BACKGROUND_RHYTHM_FLAT`, `CARD_OVERUSED`로만 구성되고 영향 slide 수가 `max(1, floor(slideCount * 0.2))` 이하면 advisory warning과 함께 발행한다. 이 기준을 넘으면 `GENERATE_DECK_VISUAL_QUALITY_GATE_FAILED`, 렌더 또는 Vision provider를 사용할 수 없으면 `GENERATE_DECK_VISUAL_QA_UNAVAILABLE`로 실패한다. semantic/deterministic validation의 non-blocking issue만 남고 rendered Visual QA가 통과한 경우도 warning과 함께 발행한다. terminal failure에서는 후보 Deck과 validation을 `jobs.result`에 보존하고 기존 project Deck은 변경하지 않는다. Visual QA unavailable을 warning으로 낮추고 실제 blocking visual issue만 terminal로 유지하는 정책 변경은 #338에서 적용한다.
...
- generate-deck 응답과 Job result의 `diagnostics`에는 `referencePolicy`, `uploadedSourceCount`, `webSourceCount`, `researchAttempts`, `relevantWebSourceCount`, `officialWebSourceCount`, `repairAttempted`, `repairReasons`, `uniqueCoreLayoutCount`, `validationIssueCount`를 저장하고, rendered QA를 실행한 경우 `visualQaStatus`, `visualReviewAttempts`, `visualRepairAttempts`, `visualIssueCodes`를 추가할 수 있다. 참고자료 원문, 검색 결과 원문, 발표 대본은 진단 정보에 포함하지 않는다.
