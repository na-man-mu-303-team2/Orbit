...
- 조회와 변경 endpoint는 각각 `GET /api/v1/projects/:projectId/jobs/:jobId/story-plan`, `POST .../story-plan/edit`, `POST .../story-plan/regenerate`, `POST .../story-plan/approve`, `POST .../story-plan/cancel`이다. 모두 기존 인증과 project write 권한 검사를 재사용한다. edit는 strict discriminated union `{ kind: "reorder", expectedRevision, orders } | { kind: "speaker-notes", expectedRevision, order, speakerNotes }`, regenerate는 strict `{ expectedRevision, instruction? }`, approve는 strict `{ expectedRevision }`이며 cancel body는 사용하지 않는다. instruction은 trim 후 최대 240자, speakerNotes는 trim 후 1~5000자다.
...
- Web은 `storyReviewRequired=true` 응답이면 generic project route보다 먼저 매칭되는 `/project/:projectId/story-plan/:jobId`로 이동한다. 한 화면에서 planning, review-pending, regenerating, regeneration error, approved, cancelled와 failed를 처리하고 승인 뒤 기존 Job endpoint를 polling해 성공 시 editor로 이동한다. review UI는 `목차`와 `대본` 두 탭만 제공하고 두 탭은 동일한 slide 순서를 사용한다. 각 slide는 draggable card와 keyboard/mobile용 위·아래 이동 버튼을 제공하며 대본은 직접 편집·저장하거나 기존 AI 재제안 instruction으로 다시 생성할 수 있다. `topic`처럼 사용자 입력인 source는 대본 근거로 반복 표시하지 않고, `web | uploaded` source의 안전한 title/type만 해당 대본 아래에 작게 표시한다. 사실이 “검증됨”이라고 표시하거나 Visual QA 결과를 미리 예측하지 않으며, `repairReasonCodes`는 AI가 자동 조정한 내용으로만 안내하고 정확한 token 절감량은 표시하지 않는다.
