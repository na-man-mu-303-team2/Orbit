...
- 로컬 기본 `AI_DECK_EXECUTION_MODE`는 `bullmq`다. staged BullMQ는 파일별 OCR, planning, slide별 image fan-out, semantic quality, rendered visual quality와 publication까지 실행해 완전한 Deck을 발행한다. `sqs`는 같은 stage/checkpoint pipeline의 다섯 stage queue만 SQS로 운반하고 ID-only coordinator와 AI Deck 이외 Job은 계속 BullMQ를 사용한다. `monolith`는 338-5 제거 전 회귀·rollback용 opt-in 호환 경로로 유지한다. staging·production 예제의 명시적 `monolith` 값과 `develop` 자동 배포 규칙은 이 adapter 추가로 전환하지 않는다.
...
- bounded repair 이후 rendered Visual QA가 `passed=false`여도 남은 issue가 `BALANCE_WEAK`, `LAYOUT_REPETITIVE`, `BACKGROUND_RHYTHM_FLAT`, `CARD_OVERUSED`로만 구성되고 영향 slide 수가 `max(1, floor(slideCount * 0.2))` 이하면 advisory warning과 함께 발행한다. 이 기준을 넘는 실제 visual issue는 `GENERATE_DECK_VISUAL_QUALITY_GATE_FAILED` terminal이다. 렌더 또는 Vision provider를 사용할 수 없더라도 semantic/deterministic validation에 blocking issue와 unresolved placeholder가 모두 없으면 `GENERATE_DECK_VISUAL_QA_UNAVAILABLE` warning과 `visualQaStatus="unavailable"`을 남기고 발행한다. blocking issue 또는 unresolved placeholder가 있으면 terminal이며 후보 Deck과 validation을 `jobs.result`에 보존하고 기존 project Deck은 변경하지 않는다. semantic/deterministic validation의 non-blocking issue만 남고 rendered Visual QA가 통과한 경우도 warning과 함께 발행한다.
...
- staged coordinator message는 `bullmq`와 `sqs` 모두 BullMQ `generate-deck` queue에 strict `{ jobId, projectId }`만 담고 전체 request는 DB의 부모 `jobs.payload`에서 읽는다. `generate-deck` queue는 `job.name`의 `generate-deck`과 `generate-deck-staged-coordinator`, `reference-extract` queue는 `reference-extract`와 `reference-extract-file`을 구분해 기존 monolith/standalone OCR과 staged handler를 함께 안전하게 routing한다. BullMQ stage queue도 stage 이름과 `job.name`이 일치할 때만 실행한다.
- `AI_DECK_WORKER_QUEUE=all|reference-extract|research-content|design-layout|image|qa-finalize`를 `bullmq`와 `sqs`에서 실행할 수 있다. `bullmq`의 `research-content`, `design-layout`, `image`, `qa-finalize`는 각각 대응 BullMQ queue만 소비한다. `sqs`의 동일 role은 대응 SQS queue만 소비하며 `reference-extract` role은 coordinator·standalone OCR용 BullMQ queue와 reference SQS queue를 함께 소비한다. `JOB_QUEUE_DRIVER=sqs`는 전역 adapter가 없으므로 계속 Worker startup에서 거부한다.
...
- SQS queue URL은 `AI_DECK_SQS_REFERENCE_EXTRACT_QUEUE_URL`, `AI_DECK_SQS_RESEARCH_CONTENT_QUEUE_URL`, `AI_DECK_SQS_DESIGN_LAYOUT_QUEUE_URL`, `AI_DECK_SQS_IMAGE_QUEUE_URL`, `AI_DECK_SQS_QA_FINALIZE_QUEUE_URL`이며 `AI_DECK_EXECUTION_MODE=sqs`에서만 다섯 값이 모두 필수다. receive는 한 번에 최대 1개를 20초 long polling하고 초기 visibility를 5분으로 설정한다. 처리 중에는 60초마다 visibility를 다시 5분으로 연장하고 handler가 성공한 뒤에만 receipt handle로 delete한다. validation·handler·delete 실패는 message를 delete하지 않아 redelivery와 후속 DLQ 정책에 맡긴다.
...
- dispatcher는 9개 stage checkpoint를 모두 enqueue한다. BullMQ는 enqueue 후 `getState()`가 `waiting | delayed | prioritized`, SQS는 `SendMessage`가 성공해 `waiting` 결과를 반환할 때만 조회 당시 `attempt`를 대조해 `dispatched_at`을 기록한다. BullMQ의 `active | completed | failed | unknown`과 transport 오류는 mark하지 않으며, 늦은 이전 send가 새 retry generation을 덮지 못한다.
- dispatcher는 매 회차 `listUndispatched` 전에 active parent의 9개 stage 중 `status='queued'`이고 `dispatched_at`이 15분 이상 지난 row를 최대 100개씩 `FOR UPDATE ... SKIP LOCKED`로 복구한다. 이 scan은 `idx_ai_deck_generation_stages_stale_dispatch (dispatched_at, pipeline_job_id, shard_key)` partial index를 사용한다.
