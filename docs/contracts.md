...
- capability reader는 rolling deployment 호환을 위해 `version: "1" | "2"`를 수용한다. 현재 API가 발행하는 `designAgentCapabilities` 상수는 계속 `version: "1"`, `addableElementTypes: ["text", "rect", "chart", "table"]`, `canGenerateImages: false`를 유지하며, reader 확장만으로 사용자 동작이나 생성 가능한 element 범위가 바뀌지 않는다.
