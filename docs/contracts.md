...
- `/ai-ppt` design-pack 요청은 선택적으로 `design.fontOverride`, 확장된 `design.mediaPolicy`, `design.referencePolicy`, `visualPlanPolicy`, `referencePolicy`, `referenceFileIds`, `officialAssetFileIds`를 보낼 수 있다. `officialAssetFileIds`는 일반 참고 자료와 분리된 사용자 제공 공식 이미지를 가리킨다. 생략 시 `generationMode = "legacy"` 기본 경로와 기존 AI 덱 생성 계약은 유지된다.
