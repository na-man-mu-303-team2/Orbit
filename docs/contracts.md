...
- 한 장 바로 연습의 raw audio와 transcript 원문은 브라우저 메모리에서만 처리하며 API, Job, DB, 로그에 저장하지 않는다.
...
- 연습 기록의 기본 보관 기간은 90일, 사용자/기기 voice baseline은 180일이다. project 또는 user 삭제 시 FK cascade로 함께 삭제한다.
...
- `research`는 `{ status: succeeded | unavailable, attempts, officialSourceCount, issueCodes, researchedAt }`만 반환한다. `attempts`는 최대 2, 공식 source는 최대 5개로 제한한다.
...
- 온디바이스 Web Speech가 기본이며, 사용할 수 없으면 별도 체크 없이 OpenAI Realtime 전사로 자동 전환한다. 사용자가 연습 시작을 선택하면 이 전환 정책이 함께 적용된다.
- 전사를 사용할 수 없어도 PCM 기반 음성 측정은 계속할 수 있으며, 응답에는 `quality.state`와 reason을 함께 기록한다.
