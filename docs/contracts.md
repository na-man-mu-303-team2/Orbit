...
- 로컬 기본 `AI_DECK_EXECUTION_MODE`는 `bullmq`다. staged BullMQ는 파일별 OCR, planning, slide별 image fan-out, semantic quality, rendered visual quality와 publication까지 실행해 완전한 Deck을 발행한다. `monolith`는 338-5 제거 전 회귀·rollback용 opt-in 호환 경로로 유지한다. staging·production 예제의 명시적 `monolith` 값과 `develop` 자동 배포 규칙은 이 로컬 기본값 변경으로 전환하지 않는다. `sqs`는 아직 지원하지 않으며 API와 Worker 시작 시 즉시 거부한다.
