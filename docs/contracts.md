...
`update_element_props`는 authored plain `text` 교체, imported `richText=full` target의 text 교체, `imageSource=true` picture의 `src`/`alt` 교체, 그리고 `crop=picture|picture-fill` image의 `crop` 교체·초기화만 허용한다. 실제 `elementType`과 OOXML shape kind가 일치해야 하며 `src`와 `crop`이 같은 operation에 있으면 둘 다 누적 적용한다. `update_element_frame`은 geometry와 `zIndex`만 허용하며 `zIndex`는 slide shape tree의 실제 XML 순서를 결정적으로 바꾼다. 미지원 필드나 operation은 fail-closed 대상이다.
...
- writable authored plain text/frame, image source와 image crop을 동기화한다. imported text는 importer가 `richText=full`을 증명하기 전까지 content 교체를 허용하지 않는다. 새로 추가된 plain text/rect/image 요소는 실제 OOXML `shapeId`, `elementType`, writable authored source mapping을 만들며, plain text의 font/size/bold/italic/underline/color/alignment/line height/body inset을 OOXML에 기록한다. image에는 alt description, relationship, media part와 `crop: picture` capability를 함께 생성해 후속 편집도 같은 요소를 갱신한다. imported `p:pic`과 shape picture-fill은 각각 direct `a:srcRect`만 수정하며 안전한 locator가 없으면 `CROP_CAPABILITY_UNSAFE`로 fail-closed한다. 내부 image asset은 같은 project의 uploaded image인지 검증한 뒤 Python worker에 전달하고, Python은 대상 picture relationship과 media part만 교체한다. advanced rich text, table, unsupported element는 capability와 sync 양쪽에서 fail-closed다.
