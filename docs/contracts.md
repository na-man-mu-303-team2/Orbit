...
- `AI_DECK_EXECUTION_MODE`의 기본값은 `monolith`다. `monolith`만 기존 동기 pipeline을 끝까지 실행해 완전한 Deck을 발행한다. opt-in `bullmq`는 338-2에서 파일별 OCR과 `source-grounding`, `content-planning`, `design-planning`, `layout-compile`까지 실행하고 layout artifact를 보존한 채 338-3의 image·QA·publication 연결을 기다린다. 따라서 현재 `bullmq` 결과를 완전한 Deck 생성 성공으로 취급하지 않는다. `sqs`는 아직 지원하지 않으며 API와 Worker 시작 시 즉시 거부한다.
...
- provider timeout, 제한된 재시도 실패, deck·user 비용 한도 초과가 발생해도 다른 slide의 asset 해소는 계속한다. unresolved optional asset은 `dropOptionalMediaSlideIds`를 통해 호환 가능한 no-media composition으로 전환하며, placeholder와 blocking issue가 남지 않을 때만 발행한다. required asset 실패, no-media fallback 실패 또는 unresolved placeholder 잔존은 terminal failure다. 현재 no-media fallback request 자체의 실패는 `GENERATE_DECK_VISUAL_QA_UNAVAILABLE`로 terminal 처리한다. 338-3에서 rendered Visual QA unavailable을 degraded success로 바꾸기 전에 이 경우를 `GENERATE_DECK_OPTIONAL_IMAGE_FALLBACK_FAILED` terminal error로 분리한다.
...
- bounded repair 이후 rendered Visual QA가 `passed=false`여도 남은 issue가 `BALANCE_WEAK`, `LAYOUT_REPETITIVE`, `BACKGROUND_RHYTHM_FLAT`, `CARD_OVERUSED`로만 구성되고 영향 slide 수가 `max(1, floor(slideCount * 0.2))` 이하면 advisory warning과 함께 발행한다. 이 기준을 넘으면 `GENERATE_DECK_VISUAL_QUALITY_GATE_FAILED`, 렌더 또는 Vision provider를 사용할 수 없으면 현재 `GENERATE_DECK_VISUAL_QA_UNAVAILABLE`로 실패한다. semantic/deterministic validation의 non-blocking issue만 남고 rendered Visual QA가 통과한 경우도 warning과 함께 발행한다. terminal failure에서는 후보 Deck과 validation을 `jobs.result`에 보존하고 기존 project Deck은 변경하지 않는다. Visual QA unavailable을 warning으로 낮추고 실제 blocking visual issue만 terminal로 유지하는 동작 변경은 338-3에서 적용한다.
...
- `retryable`은 부모 Job 실패 후 명시적 retry API를 허용할지 나타낸다. 자동 stage 재시도는 checkpoint의 `attempt < 5`로 별도 관리한다. 부모 Job 수동 retry API와 성공한 upstream checkpoint를 보존하는 재개 동작은 338-3 범위이며 338-2까지는 공개 retry API가 없다.
...
- 338-2의 staged BullMQ 경로는 opt-in OCR·planning slice다. coordinator message는 strict `{ jobId, projectId }`만 담고 전체 request는 DB의 부모 `jobs.payload`에서 읽는다. `generate-deck` queue는 `job.name`의 `generate-deck`과 `generate-deck-staged-coordinator`, `reference-extract` queue는 `reference-extract`와 `reference-extract-file`을 구분해 기존 monolith/standalone OCR과 staged handler를 함께 안전하게 routing한다. planning queue도 stage 이름과 `job.name`이 일치할 때만 실행한다.
- `AI_DECK_WORKER_QUEUE=all|reference-extract|research-content|design-layout`은 338-2에서 실행 가능하다. `research-content`는 `ai-deck-research-content`, `design-layout`은 `ai-deck-design-layout`만 소비한다. `image`, `qa-finalize` role과 `AI_DECK_EXECUTION_MODE=sqs`는 구현된 것처럼 healthy 상태를 보이지 않도록 Worker 시작 시 거부한다.
...
- `source-grounding`의 `input_ref_json`은 `{}`다. `content-planning`, `design-planning`, `layout-compile`의 input은 strict `{ planningArtifactId: UUID }`만 허용하며 각 consumer가 같은 tenant·pipeline과 기대 upstream stage의 artifact인지 다시 검증한다. `result_ref_json`은 기본적으로 `null | {}`이고 `reference-extract-file`은 strict `{ referenceExtractionArtifactId: UUID }`, 네 planning stage는 strict `{ planningArtifactId: UUID }`만 허용한다. 전체 Deck·content·binary/base64·provider raw response는 checkpoint에 저장하지 않는다.
...
- 338-2 dispatcher는 `reference-extract-file`과 네 planning checkpoint만 enqueue한다. enqueue 후 BullMQ `getState()`가 `waiting | delayed | prioritized`일 때만 조회 당시 `attempt`를 대조해 `dispatched_at`을 기록한다. `active | completed | failed | unknown`은 durable dispatch로 인정해 mark하지 않으며, 늦은 이전 send가 새 retry generation을 덮지 못한다. 338-3 이후 stage는 아직 dispatch하지 않는다.
- dispatcher는 매 회차 `listUndispatched` 전에 active parent의 OCR·planning stage 중 `status='queued'`이고 `dispatched_at`이 15분 이상 지난 row를 최대 100개씩 `FOR UPDATE ... SKIP LOCKED`로 복구한다. 이 scan은 같은 5개 stage에 한정된 partial index `idx_ai_deck_generation_stages_stale_dispatch (dispatched_at, pipeline_job_id, stage, shard_key)`를 사용한다.
- retryable failure는 현재 stage/shard만 지수 backoff로 최대 총 5회 시도한다. DB lease는 10분, heartbeat는 60초다. retry release와 expired lease의 1~4번째 복구는 `status='queued'`, `lease_owner=NULL`, `lease_expires_at=NULL`, `dispatched_at=NULL`로 전이하고 기존 `attempt`는 유지한다. OCR의 5번째 종료는 policy join이 부모의 계속/실패를 결정하고, 필수 planning stage의 5번째 실패·expired lease는 checkpoint와 부모를 함께 terminal 처리한다. 부모가 terminal이면 transaction commit 후 반환된 parent Job으로 표준 `job.failed` 업무 로그를 남긴다. 이 DB checkpoint `attempt`는 BullMQ transport의 `attemptsMade`와 별도 재시도 층이다. dispatcher와 expired-lease reconciler는 Worker 시작 직후 한 번 실행한 뒤 주기적으로 실행한다.
...
- 338-2 `bullmq` 부모 Job은 `layout-compile` 성공 후에도 progress 60의 `running`을 유지한다. image/QA/publication과 완전한 Deck 발행은 338-3 범위이고, 그 전까지 production 기본값은 `monolith`다. `WEB_RESEARCH_QUALITY_FAILED`는 usable grounding 또는 사용자 입력이 있으면 `warningCodes`와 warning을 남기고 계속한다. usable grounding이 전혀 없는 strict policy의 `SOURCE_GROUNDING_REQUIRED`와 내부 재시도 후에도 유효하지 않은 Art Director 응답의 `ART_DIRECTOR_INVALID_RESPONSE`는 terminal이다.
...
- generate-deck 응답과 Job result의 `diagnostics`에는 `referencePolicy`, `uploadedSourceCount`, `webSourceCount`, `researchAttempts`, `relevantWebSourceCount`, `officialWebSourceCount`, `repairAttempted`, `repairReasons`, `uniqueCoreLayoutCount`, `validationIssueCount`, `warningCodes`를 저장한다. `warningCodes`는 `^[A-Z][A-Z0-9_]*$` machine-readable code 배열이며 기본값은 `[]`다. rendered QA를 실행한 경우 `visualQaStatus`, `visualReviewAttempts`, `visualRepairAttempts`, `visualIssueCodes`를 추가할 수 있고 `visualQaStatus`는 `not-run | passed | failed | unavailable`을 허용한다. TypeScript Zod와 Python Pydantic mirror는 같은 값을 검증한다. 참고자료 원문, 검색 결과 원문, 발표 대본은 진단 정보에 포함하지 않는다. 338-2는 degraded research에서 `WEB_RESEARCH_QUALITY_FAILED`를 실제 emit하고, Visual QA unavailable warning은 338-3에서 연결한다.
...
- `research-first`는 OpenAI Responses `web_search`를 최초 1회와 최대 2회의 bounded retry로 덱당 최대 3회 실행하고 서로 다른 URL citation 2개 이상을 목표로 한다. 모든 시도 후에도 2개 미만이지만 usable grounding 또는 topic·prompt·Brief 사용자 입력이 있으면 `WEB_RESEARCH_QUALITY_FAILED` warning/degraded success로 계속한다. strict policy에서 usable grounding이 전혀 없을 때만 `SOURCE_GROUNDING_REQUIRED` terminal failure다. 검색 질의는 topic, Brief, 추출 keyword만 사용하며 첨부 원문, 파일명, speaker notes를 포함하지 않는다.
