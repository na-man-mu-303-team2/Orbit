...
- 조회와 변경 endpoint는 각각 `GET /api/v1/projects/:projectId/jobs/:jobId/story-plan`, `POST .../story-plan/edit`, `POST .../story-plan/regenerate`, `POST .../story-plan/approve`, `POST .../story-plan/cancel`이다. 모두 기존 인증과 project write 권한 검사를 재사용한다. edit는 strict discriminated union `{ kind: "reorder", expectedRevision, orders } | { kind: "speaker-notes", expectedRevision, order, speakerNotes }`, regenerate는 strict `{ expectedRevision, instruction? }`, approve는 하위 호환되는 strict `{ expectedRevision, designSelection? }`이며 cancel body는 사용하지 않는다. `designSelection`은 `paletteOptionId`, 전체 `paletteOverride`, strict `fontOverride`만 받는다. instruction은 trim 후 최대 240자, speakerNotes는 trim 후 1~5000자다.
...
- approve는 Job, review와 `content-planning` artifact row를 잠근 뒤 `expectedRevision`을 검증한다. `designSelection`이 있으면 선택한 palette/font를 Job payload와 artifact의 `rawInput.design`에 같은 transaction으로 반영한 후 `design-planning` checkpoint를 `ON CONFLICT DO NOTHING`으로 정확히 하나 만든다. 동일 revision의 중복 approve는 멱등이며 stale revision이나 승인할 수 없는 상태는 `409`다.
