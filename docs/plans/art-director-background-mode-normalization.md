...
> 이 계획이 승인되기 전 GitHub의 기존 댓글은 최신 계약으로 보지 않는다. 승인 후 이 문서 전문을 #341에 댓글로 남겨 이슈와 실행 계획의 기준을 동기화한다.
...
- #338의 `design-planning` Job은 정규화·검증된 `DesignPlan`만 checkpoint에 저장한다.
