...
위 다이어그램은 338-3의 현재 구현 구조다. 338-1은 OCR child의 policy join 또는 OCR skip으로 `source-grounding` checkpoint를 생성하는 지점까지, 338-2는 `layout-compile`까지 병합됐고 338-3이 image·QA·publication을 연결한다.
...
- Worker image는 `AI_DECK_WORKER_QUEUE`에 따라 담당 queue만 소비할 수 있어야 하며 후속 ECS service가 같은 image를 사용한다. 338-3은 `all`, `reference-extract`, `research-content`, `design-layout`, `image`, `qa-finalize` role을 모두 실행한다.
...
- input_ref_json         # source는 '{}', planning/image/semantic은 planning locator, rendered/publication은 execution locator
...
5. binary와 큰 stage 결과는 checkpoint가 아닌 별도 DB 또는 Storage artifact에 둔다. `ai_deck_reference_extraction_artifacts`, `ai_deck_planning_artifacts`, `ai_deck_execution_artifacts`에 stage별 검증 결과를 저장하고 checkpoint에는 각각 strict `{ referenceExtractionArtifactId: uuid }`, `{ planningArtifactId: uuid }`, `{ executionArtifactId: uuid }` locator만 기록한다. 전체 Deck·content·binary/base64·provider raw response는 checkpoint에 저장하지 않는다.
...
- `AI_DECK_EXECUTION_MODE=monolith|bullmq|sqs`로 시작하고 338-5에서 `bullmq|sqs`만 남긴다. 338-3 로컬 기본값은 `bullmq`, staging·production 예제는 명시적 `monolith`이며 `sqs`는 API·Worker startup에서 fail-fast한다.
- `AI_DECK_WORKER_QUEUE=all|reference-extract|research-content|design-layout|image|qa-finalize`를 사용하며 338-3에서 모든 role을 실행한다.
...
필수 stage가 최종 실패하면 부모 Job에 `failedStage`, `retryable`과 안전한 오류 코드만 기록한다. 이전 checkpoint는 보존하며 명시적 재시도 API가 해당 stage부터 다시 시작한다.
