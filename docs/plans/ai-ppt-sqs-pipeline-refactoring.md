...
**상태**: 확정 · #339 완료 · #338-0·338-1 완료·병합 · #338-2 구현·로컬 검증 중
...
338-1은 staged coordinator와 파일별 OCR까지 활성화해 병합됐다. 현재 338-2는 네 planning stage 연결을 구현·검증한다.
...
- 338-2 dispatcher·consumer는 `reference-extract-file`, `source-grounding`, `content-planning`, `design-planning`, `layout-compile`만 처리한다. image 이후 stage는 338-3 전까지 dispatch하지 않는다.
...
- Worker image는 `AI_DECK_WORKER_QUEUE`에 따라 담당 queue만 소비할 수 있어야 하며 후속 ECS service가 같은 image를 사용한다. 338-2는 `all`, `reference-extract`, `research-content`, `design-layout` role을 실행하고 image 이후 role은 owner PR 전까지 startup에서 거부한다.
...
- input_ref_json         # 338-2: source는 '{}', content/design/layout은 strict { planningArtifactId: uuid }
- result_ref_json        # OCR은 { referenceExtractionArtifactId }, planning은 { planningArtifactId }
...
- `AI_DECK_WORKER_QUEUE=all|reference-extract|research-content|design-layout|image|qa-finalize`를 사용한다. 338-2는 `all`, `reference-extract`, `research-content`, `design-layout`을 실행하고 후속 role은 해당 stage owner PR에서 활성화한다.
...
BullMQ transport retry는 위 DB lease `attempt`와 독립적이다. 최종 BullMQ OCR 실패는 DB attempt를 소모하지 않고 stale `dispatched_at`만 해제한다. coordinator가 failed set에 들어가면 정확한 stall/started-limit `failedReason`을 먼저 판별해 `attemptsMade`와 무관하게 coordinator transaction을 멱등 재실행하고, 그 외 `attemptsMade >= opts.attempts`인 entry만 active checkpoint와 부모를 atomic 종료한다. 즉시 recovery가 DB 장애로 실패하면 cap 없이 retained된 entry를 opaque Redis `ZSCAN` cursor maintenance가 full cursor cycle에서 다시 처리한다.
...
- `ai_deck_planning_artifacts`에 stage별 JSON 결과를 저장하고 checkpoint에는 strict `{ planningArtifactId }` locator만 전달한다.
- OCR과 네 planning stage의 stale dispatch, lease expiry, final transport failure를 같은 fencing 규칙으로 복구한다.
