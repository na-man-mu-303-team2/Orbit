...
9. BullMQ 최종 transport attempt 실패는 원 오류를 재throw하기 전에 DB recovery를 await한다. OCR은 queued checkpoint의 `dispatched_at`만 복구하고 coordinator는 active checkpoint와 부모를 `AI_DECK_COORDINATOR_FAILED`, `retryable=true`로 atomic 종료한다. 재시도 소진 또는 stall limit 초과로 failed set에 들어간 coordinator는 `attemptsMade`와 무관하게 transport-terminal로 취급한다. failed entry는 `removeOnFail=false`로 cap 없이 보존하며 maintenance가 기본 25개·최대 100개를 `start`/`nextStart` cursor로 순환해 DB recovery가 확인된 entry만 제거한다. BullMQ transport attempt와 DB checkpoint `attempt`는 서로 다른 재시도 층이다.
...
부모 request에서 non-empty `references`의 `{ fileId }[]`를 우선하고, 비어 있을 때만 `referenceFileIds`를 fallback으로 사용한다. 첫 등장 순서로 중복 제거한 전체 선택 집합은 `selectedReferenceFileIds`, 여기서 `referenceContext`가 covered한 file을 제외한 OCR 대상은 `uncoveredReferenceFileIds`다.
...
BullMQ transport retry는 위 DB lease `attempt`와 독립적이다. 최종 BullMQ OCR 실패는 DB attempt를 소모하지 않고 stale `dispatched_at`만 해제하며, coordinator가 재시도 소진 또는 stall limit 초과로 failed set에 들어가면 `attemptsMade`와 무관하게 active checkpoint와 부모를 atomic 종료한다. 즉시 recovery가 DB 장애로 실패하면 cap 없이 retained된 coordinator failed entry를 bounded cursor maintenance가 다음 회차에 다시 처리한다.
...
- 15분 stale queued dispatch 복구와 partial index, OCR/coordinator 최종 transport recovery, retained coordinator의 bounded cursor reconciliation을 구현한다.
