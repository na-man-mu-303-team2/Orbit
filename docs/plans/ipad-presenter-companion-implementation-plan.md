...
- 예상 전달 단위: browser MVP 7개 PR, 6개 검증 checkpoint + 선택적
  native double-tap 확장 3개 PR, 1개 checkpoint
...
일반 slide navigation UI는 계속 범위 밖이다. 단, browser MVP 이후 선택적
native iPad shell에서 명시적으로 활성화한 Apple Pencil hardware double tap을
기존 desktop `next-step` command로 전달하는 기능은 별도 확장 범위로 둔다.
화면을 Pencil tip으로 두 번 누르는 gesture는 drawing과 충돌하므로 fallback으로
사용하지 않는다.
...
| D15 | 선택적 native iPad shell은 `UIPencilInteraction`으로 hardware double tap을 받고 allowlisted `next-step` intent만 `WKWebView` companion에 전달한다. slide/step 계산은 desktop authority가 기존 `getNextPresenterStepState`로 수행한다. |
| D16 | custom double-tap action은 기본값 off이며 system `ignore` preference를 존중한다. pairing에서 별도 `advance-presentation` scope를 승인한 경우에만 사용할 수 있다. |
...
### 5.1 선택적 native double-tap 경계

Apple Pencil hardware double tap은 browser event가 아니다. Apple의 공식
경로는 native `UIPencilInteraction`의
`pencilInteraction(_:didReceiveTap:)`이고, web Pointer Events는 Pencil
contact, pressure, tilt, button 상태 등만 계약한다. 따라서 browser MVP의
capability detection 결과에 `hardwareDoubleTap: true`를 만들지 않는다.

선택적 native 확장은 기존 companion web UI를 `WKWebView`로 감싼다.

```mermaid
flowchart LR
  P["Supported Apple Pencil"] -->|"hardware double tap"| U["UIPencilInteraction"]
  U -->|"allowlisted next-step intent"| W["WKWebView companion"]
  W -->|"scoped control command"| S["Socket.IO relay"]
  S --> D["Desktop authority"]
  D -->|"existing getNextPresenterStepState"| A["Main audience output"]
  D -->|"accepted/rejected ack"| W
```

native bridge는 URL, cookie, arbitrary JavaScript, slide index를 전달하지 않고
`next-step` intent만 전달한다. web companion과 API는 native bridge가 보낸
slide index를 신뢰하지 않으며 desktop authority가 현재 권위 상태에서 다음
step을 계산한다.
...
- 기본 scope: `view-audience-output`, `write-annotation`
- 선택 scope: presenter가 pairing에서 native double tap을 승인한 경우에만
...
| `presentation:companion:control-command` | iPad -> presenter | scoped `next-step`, command ID, authority/output revision |
| `presentation:companion:control-ack` | presenter -> iPad | command ID, accepted/rejected reason, resulting output revision |
...
### 6.8 선택적 presentation control command

native 확장을 구현할 때
`presentationCompanionControlCommandSchema`를 strict object로 추가한다.

```ts
type PresentationCompanionControlCommand = {
  type: "next-step";
  commandId: string;
  sessionId: string;
  authorityEpochId: string;
...
- `commandId`는 한 companion generation 안에서 unique한 최대 64자
  identifier다.
- server는 `advance-presentation` scope, current pairing generation,
  active authority epoch, payload 크기와 socket session 일치를 확인한 뒤
  relay만 한다.
- desktop authority는 `output.mode === "slide"`, navigation not pending,
  no active Pencil stroke를 확인한 후 기존 `getNextPresenterStepState`를
  호출한다.
- animation step이 남으면 `stepIndex`만 1 증가시키고, 남지 않았으면 다음
  slide의 step `0`으로 이동한다. 마지막 slide의 마지막 step은 accepted
  no-op으로 ack한다.
- 같은 `commandId`의 재전송은 결과를 다시 적용하지 않고 기존 ack를
  반환한다.
- 초기 rate limit은 초당 2개, burst 2개다. timeout/reject는 output을
  변경하지 않는다.
- command payload와 Apple Pencil preference는 로그에 남기지 않고
  accepted/rejected reason code와 latency bucket만 기록한다.
...
- 설명: production 기능 코드에 앞서 staging HTTPS에서 Pointer Events,
  coalesced events, pressure, hover, Apple Pencil side double tap 때 발생하는
  DOM event, Socket.IO RTT, WebRTC receive, sleep/reload, 실전 발표/리허설
  host, 두 멀티 윈도우 모드를 측정한다. test-only spike 화면과 결과 문서를
  만든다.
...
- Acceptance:
  ...
  - hover 지원/미지원 기기에서 touch drawing과 pressure fallback 결과가
    기록된다.
  - supported Apple Pencil side double tap 전후의 모든 Pointer/Mouse/Touch
    event를 기록해 Safari에 지원되는 hardware gesture API가 없음을
    확인한다. 우연히 관찰된 event는 production capability로 채택하지 않는다.
...
### 선택적 Native Apple Pencil Double-Tap 확장

이 확장은 browser MVP의 Checkpoint 5를 통과한 뒤 별도 승인으로 시작한다.
Apple은 native app에서
[`UIPencilInteraction`](https://developer.apple.com/documentation/uikit/uipencilinteraction)으로
hardware double tap을 제공하지만,
[Pointer Events Level 3](https://www.w3.org/TR/pointerevents3/)에는 해당
gesture가 없다. 새 native app의 배포·서명·업데이트 경로도 제품 결정이므로
browser MVP와 release gate를 공유하지 않는다.

#### Task N0. native shell·배포 feasibility

- 설명: `WKWebView`에서 기존 companion route와 pairing cookie가 동작하는
  최소 iPad shell을 만들고 `UIPencilInteraction` event 수신, supported
  hardware, background/foreground, TestFlight 또는 private distribution
  경로를 검증한다.
- 의존성: Task 17, native 배포 방식 승인.
- 예상 파일:
  - `apps/ipad-companion/OrbitCompanion.xcodeproj`
  - `apps/ipad-companion/OrbitCompanion/CompanionWebView.swift`
  - `apps/ipad-companion/OrbitCompanion/PencilInteractionController.swift`
  - `docs/spikes/ipad-presenter-companion-native.md`
- Acceptance:
  - system `ignore` preference에서는 event를 command로 변환하지 않는다.
  - supported Pencil의 hardware double tap은 정확히 한 번의 local
    `next-step` intent를 만들고 Pencil tip double tap은 만들지 않는다.
  - Safari와 native shell이 같은 safe companion payload만 렌더링한다.
  - 앱 종료, background/foreground, session revoke에서 credential과 web
    view가 기존 server lifecycle을 벗어나지 않는다.
- Verification:
  - native unit/UI test
  - supported/unsupported Pencil 물리 기기 QA
  - distribution install/update smoke
- 예상 크기: M

#### Task N1. scoped next-step contract와 desktop authority

- 설명: 별도 feature flag, pairing opt-in, `advance-presentation` scope,
  control command/ack schema, relay와 desktop next-step adapter를 구현한다.
- 의존성: Task N0.
- 예상 파일:
  - `packages/config/src/index.ts`
  - `packages/shared/src/presentation/presentation-companion.schema.ts`
  - `packages/realtime/src/presentation.ts`
  - `apps/api/src/presentation-sessions/presentation-companion.gateway.ts`
  - `apps/web/src/features/presenter-companion/usePresenterCompanionAuthority.ts`
  - `apps/web/src/features/rehearsal/presenter/presenterStepNavigation.ts`
  - `.env.example`
  - `docs/contracts.md`
- Acceptance:
  - `IPAD_PRESENTER_COMPANION_ADVANCE_ENABLED` 기본값은 `false`이고 false면
    scope 발급과 control event relay가 모두 fail-closed한다.
  - presenter pairing UI의 별도 opt-in이 없으면
    `advance-presentation` scope를 발급하지 않는다.
  - animation step이 남아 있으면 step만 진행하고, 없으면 다음 slide의 step
    `0`으로 이동한다.
  - duplicate/stale/rate-limited command와 black/screen-share/navigation
    pending 상태는 output을 바꾸지 않는다.
- Verification:
  - shared schema boundary tests
  - API scope/rate-limit/replay integration tests
  - desktop `getNextPresenterStepState` adapter unit test
  - slide-window/surface swap multi-page control E2E
- 예상 크기: M

#### Task N2. opt-in native gesture bridge와 feedback

- 설명: iPad shell에 off-by-default 설정, `UIPencilInteraction` delegate,
  allowlisted web bridge, ack feedback와 lifecycle cleanup을 구현한다.
- 의존성: Task N1.
- 예상 파일:
  - `apps/ipad-companion/OrbitCompanion/PencilInteractionController.swift`
  - `apps/ipad-companion/OrbitCompanion/CompanionSettingsView.swift`
  - `apps/ipad-companion/OrbitCompanion/CompanionWebView.swift`
  - `apps/web/src/features/presenter-companion/nativeCompanionBridge.ts`
  - `apps/web/src/features/presenter-companion/useCompanionSocket.ts`
  - `docs/qa/ipad-presenter-companion-native.md`
- Acceptance:
  - `Apple Pencil 더블 탭으로 다음 단계 실행`은 기본값 off이며 현재 mode가
    화면에 보인다.
  - native bridge는 allowlisted `next-step` 외의 payload, URL, JavaScript,
    cookie를 주고받지 않는다.
  - active stroke, offline, authority unavailable, scope 없음, ack timeout이면
    command를 보내거나 재적용하지 않고 non-blocking feedback만 표시한다.
  - 한 gesture가 animation/slide를 두 단계 이상 진행시키지 않는다.
- Verification:
  - Swift bridge allowlist/unit tests
  - web native-bridge malformed message tests
  - physical double-tap animation-then-slide E2E
  - rapid gesture, sleep/wake, reconnect, revoke QA
- 예상 크기: M

#### Checkpoint N. native gesture release 승인

- Apple의
  [double-tap design guidance](https://developer.apple.com/design/human-interface-guidelines/apple-pencil-and-scribble#Double-tap)에
  따라 custom action이 명시적 opt-in이고 system `ignore`를 존중한다.
- supported hardware에서 animation step, 다음 slide, 마지막 step no-op을
  각각 검증한다.
- unsupported Pencil과 Safari는 기존 drawing companion으로 안전하게
  degrade한다.
- native 배포, rollback, version compatibility runbook이 승인되어 있다.
...
flowchart TD
  ...
  T16 --> T17
  T17 -. "optional native extension" .-> N0["N0 Native feasibility"]
  N0 --> N1["N1 Scoped next-step"]
  N1 --> N2["N2 Native gesture bridge"]
```
...
| PR N0 | Task N0 | native shell, Pencil event, 배포 방식 go/no-go |
| PR N1 | Task N1 | scoped next-step command와 desktop authority |
| PR N2 | Task N2 | opt-in double-tap bridge와 physical-device release gate |
...
PR N0~N2는 browser MVP와 별도 follow-up이며 native advance flag는 승인 전
계속 `false`로 둔다.
...
- native bridge allowlist, duplicate `next-step`, stale output revision,
  system `ignore`와 per-pairing scope
...
내린다. Apple Pencil hardware double tap은 synthetic `dblclick`이나
PointerEvent로 합격 판정하지 않고 native shell 물리 기기 QA에서만
검증한다.
...
| 입력 | Pencil hover 지원, Pencil hover 미지원, hardware double-tap 지원 native shell, touch-only |
...
7. native 확장은 `IPAD_PRESENTER_COMPANION_ADVANCE_ENABLED=false`를 독립
   rollback으로 사용한다. 이 flag를 내려도 Safari/native drawing,
   mirroring, desktop presenter control은 계속 동작한다.
...
- opt-in native `next-step` 외의 slide navigation UI나 presenter-only
  정보를 iPad에 추가해야 하는 경우
