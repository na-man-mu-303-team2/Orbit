...
> 
> 
> 상태: 1차 완료, 2차 핵심 기능 완료·기본 생성 계약 보강 필요, 3차 선행 규칙 구현 완료·최종 승인 보류, 3차 기획 확정
> 
...
V9는 V8의 발표 시간·분량 보강 방향을 유지하면서, 2026-07-10 실사용 검증에서 재현된 정확한 장수 생성 실패와 화면 내 동일 내용 중복 문제를 기본 생성 계약에 추가한다. 3차 선행 규칙의 코드 구현 여부와 실제 사용자 결과의 승인 여부를 분리하고, 두 결함이 해결되기 전에는 3차 선행 단계를 완료로 판정하지 않는다.
...
다만 시간 배분과 speakerNotes는 기능 존재만 확인됐을 뿐 다음 품질 문제가 남아 있다.
...
따라서 2차는 핵심 기능 완료 상태로 유지하되, 정확한 장수 보장·시간/분량 안정화·구조적 중복 제거는 3차 기능보다 먼저 닫아야 하는 즉시 보강 범위로 분류한다. 3차 선행 규칙은 구현됐지만 이 기본 생성 계약의 실사용 승인이 끝날 때까지 최종 완료 판정을 보류한다.
...
현재 구현은 `발표 시간 × 분당 글자 수`로 전체 대본 분량을 계산한 뒤, cover·summary·data 등 slideType 가중치에 따라 배분한다. LLM은 슬라이드별 필요 시간을 독립적으로 판단하지 않으며, 코드가 지정한 목표 글자 수에 맞춰 문장을 작성한다. 목표 하한보다 짧으면 repair가 대본을 늘린다.

2026-07-10 실사용 검증에서는 `slideCountRange.min == slideCountRange.max`인 8장·10장·15장 design-pack 요청이 모두 `LLM returned fewer slides than the requested minimum`으로 실패했다. 요청 payload와 `Slide count` prompt는 정상 전달됐지만, LLM 응답의 `slides` 배열에 목표 장수를 구조적으로 강제하지 않고 출력 후 최소 장수 미달을 즉시 예외로 처리한다. 현재 경로에는 장수 재요청, 누락 beat 확장 또는 결과 병합이 없다.

따라서 이 문제는 사용자의 발표 시간이나 장수 입력 오류가 아니라, 비결정적인 LLM 출력에 결정론적 생성 계약을 맡긴 구현 결함이다. 성공한 10장 fixture가 있더라도 같은 조건의 후속 요청이 실패할 수 있으므로 단일 성공 결과만으로 장수 제어를 승인하지 않는다.

이 구조는 시간 제약을 반영한다는 장점이 있지만, 발표 제한 시간 100%를 대본으로 채우고 분당 280~440자를 사용하는 현재 기준은 자연스러운 발표의 전환, 호흡, 강조와 청중 반응 시간을 충분히 확보하지 못한다. 리서치 문서에는 정확한 분당 글자 수 기준이 없으며, 문서 B 13은 한 장당 약 1~2분을 출발점으로 삼고 리허설을 통해 조정하도록 권장한다.
...
- 8장·10장·15장 exact-count fixture를 반복 실행해 모든 결과의 실제 슬라이드 수가 요청값과 일치하는지 검증한다.
...
- [ ]  요청 최소 슬라이드 수 미달 자동 보정과 정확한 장수 보장
- [ ]  `message`와 `contentItems`의 구조적 중복 제거 및 visible text 중복 회귀 검증
- [x]  발표 시간 기반 speakerNotes 생성 및 보정 경로 구현
- [ ]  발화 예산, 전환 시간과 역할별 허용 범위를 반영한 대본 밀도 안정화
...
**판정: 2차 핵심 기능 완료, 정확한 장수·시간/분량·구조적 중복 즉시 보강 필요**
...
- 8장·10장·15장 exact-count 요청에서 LLM 출력이 부족해도 제한된 count repair 후 요청 장수로 생성된다.
...
**현재 판정: profile·narrative·typography·grid·Worker/Editor 검증은 구현됐으나, exact-count와 구조적 중복 회귀가 닫히지 않아 최종 승인 보류**
...
**상태: 핵심 기능 완료, 기본 생성 계약 보강 필요**

### 2차 즉시 보강: 정확한 장수, 내용 중복과 대본 밀도
...
- 8장·10장·15장 exact-count 및 5분·10분·15분 timing 회귀 fixture

**상태: 미완료, 3차 기능 착수 전 필수**
...
**상태: 규칙 구현 완료, exact-count·구조적 중복·시간/분량 보강 완료 전 최종 승인 보류**
...
3차 기능 구현보다 기본 생성 계약과 발표 품질 규칙 기반을 먼저 닫는다.

1. exact-count 생성 계약과 제한된 count repair 구현, 8장·10장·15장 반복 회귀 테스트
2. message와 contentItems의 구조적 중복 제거, cover·overview·insight·closing visible text 회귀 테스트
3. 발화 예산 75~85%, 초기 발표 속도와 장표별 시간 범위 적용
4. speakerNotes 과밀 compact와 반복 없는 repair 적용
5. 본 문서의 R-01~R-22 코드 기준과 fixture 최종 대조
6. narrative selector와 compact prompt 회귀 검증
7. presentationIssues와 역할별 typography·timing validator 최종 검증
8. Worker·에디터 검증 기준 동기화 확인
9. 발표 유형별 회귀 fixture와 렌더링 재승인
10. Saved Design Pack schema와 규칙 저장 범위 확정
11. Saved Design Pack CRUD, 이미지 asset, Brand Kit과 기본 QA 구현

exact-count, 구조적 중복과 시간·분량 기본 생성 계약이 통과하기 전에는 3차 본 기능을 시작하지 않는다. 3차 선행 규칙 기반이 통과하기 전에는 Saved Design Pack에 임의의 QA 필드나 해제 가능한 품질 옵션을 추가하지 않는다. 2차 회귀 suite는 모든 후속 단계에서 유지하며 design-pack과 legacy/template 생성의 분리를 계속 보장한다.
