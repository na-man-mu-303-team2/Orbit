...
V9는 V8의 발표 시간·분량 보강 방향을 유지하면서, 2026-07-10 실사용 검증에서 재현된 정확한 장수 생성 실패와 화면 내 동일 내용 중복 문제를 기본 생성 계약에 추가한다. 3차 선행 규칙의 코드 구현 여부와 실제 사용자 결과의 승인 여부를 분리했으며, 2026-07-11 자동화 회귀와 대표 profile 브라우저·PPTX 재승인을 통과해 기본 생성 계약과 3차 선행 단계를 완료했다.
...
초기 발화 시간 비율: 75~85%
발표 속도 기준: 공백 제외 400자/분
```

위 수치는 리서치 문서의 직접 인용값이 아니라 제품 기준값이다. 실제 한국어 발표 fixture와 리허설 데이터로 보정한다.
...
- [x]  요청 최소 슬라이드 수 미달 자동 보정과 정확한 장수 보장
...
- 발표 속도는 공백 제외 400자/분을 공통 기준으로 사용한다.
...
**현재 판정: 완료. profile·narrative·typography·grid·Worker/Editor 검증과 exact-count·구조적 중복·발화 예산 회귀가 모두 통과했으며 3차 본 작업 착수 가능**
...
기본 생성 계약과 3차 선행 발표 품질 규칙이 최종 승인됐으므로 다음 작업은 3차 재사용·실제 asset 단계로 전환한다.

1. Saved Design Pack schema와 규칙 저장 범위 확정
2. Saved Design Pack CRUD와 사용자·조직 소유권 계약 구현
3. Session override와 저장 Design Pack의 우선순위 및 병합 규칙 구현
4. Brand Kit schema, logo·font·palette asset 연결
5. 실제 AI 및 공개 이미지 생성·검색·저장 provider 경계 구현
6. 이미지 비용·실패·재시도·대체 placeholder 정책 구현
7. 기본 의미 기반 QA와 profile별 golden deck 평가 도입
8. 실제 asset을 포함한 Editor·PPTX export 정합성 검증
