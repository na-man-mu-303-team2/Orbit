# AI PPT 생성 고도화 기획서 V10

> 기준일: 2026-07-11
>
>
> 상태: 1차 완료, 2차 및 기본 생성 계약 완료, 3차 선행 규칙 최종 승인 완료, 장수 상한·Side AI 이미지 정책 안내 보정 필요, 3차 착수 가능
>
...
V10은 첨부된 V9와 저장소의 최신 구현을 다시 대조해 현재 동작과 다른 설명을 수정한다. 2026-07-11 자동화 회귀와 대표 profile 브라우저·PPTX 재승인으로 기본 생성 계약과 3차 선행 규칙은 완료됐지만, 이후 `/ai-ppt`의 사용자 직접 입력 장수는 exact-count가 아니라 목표값 기준 `±2장` 허용 범위로 변경됐다. 이 범위의 상한이 shared schema 최대 20장으로 clamp되지 않은 경계 결함과 Side AI가 현재 capability보다 앞선 이미지 삽입을 약속할 수 있는 안내 문제를 즉시 보정 대상으로 기록한다. 실제 AI·공개 이미지 asset은 여전히 3차 범위다.
...
| 사용자 직접 입력 상한 | shared 최대 20장, Web `N+2` 미보정 | 19장·20장 입력 버그, 즉시 보정 |
...
| Side AI 이미지 안내 | fallback은 placeholder를 안내하지만 provider 답변에는 capability 제한 누락 | 2차 안내 버그, 즉시 보정 |
...
- [ ]  Side AI provider와 fallback의 현재 이미지 capability 안내 동기화
...
**판정: 2차 및 3차 착수 전 기본 생성 계약 완료, 장수 상한과 Side AI 이미지 안내 문구 보정 필요**
...
**현재 판정: 완료. profile·narrative·typography·grid·Worker/Editor 검증, Worker exact-count, Web 허용 범위, 구조적 중복과 발화 예산 회귀가 통과했다. Web 장수 상한과 Side AI capability 안내 보정을 먼저 닫고 3차 본 작업을 병행할 수 있다.**
...
- 참고자료와 웹 리서치 품질 gate
- Side AI 지원
- Side AI 이미지 capability 안내 보정
- repair, validation, contrast와 export 정합성
- legacy 영향 차단

**상태: 핵심 기능 완료, 사용자 장수 상한과 이미지 capability 안내 보정 필요**
...
기본 생성 계약과 3차 선행 발표 품질 규칙이 최종 승인됐으므로 장수 상한과 Side AI capability 안내를 먼저 바로잡고 3차 재사용·실제 asset 단계로 전환한다.

1. 사용자 입력 slide range의 max를 shared 최대 20장으로 clamp하고 경계 테스트 추가
2. Side AI provider instructions와 fallback의 이미지 capability 문구 동기화
3. Saved Design Pack schema와 규칙 저장 범위 확정
4. Saved Design Pack CRUD와 사용자·조직 소유권 계약 구현
5. Session override와 저장 Design Pack의 우선순위 및 병합 규칙 구현
6. Brand Kit schema, logo·font·palette asset 연결
7. 실제 AI 및 공개 이미지 생성·검색·저장 provider 경계 구현
8. 이미지 비용·실패·재시도·대체 placeholder 정책 구현
9. 기본 의미 기반 QA와 profile별 golden deck 평가 도입
10. 실제 asset을 포함한 Editor·PPTX export 정합성 검증
11. 대표 profile 및 Saved Design Pack 브라우저 승인
