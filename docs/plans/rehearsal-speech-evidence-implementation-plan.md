...
- 성공 record: `fileId` 필수, `purpose=rehearsal-evidence-audio`, 정확한 7일 만료, retention state는 `retained`, `delete-pending`, `deleted`, `delete-exhausted` 중 하나
...
| Clip DB/schema            | 기반만 구현 | migration, 최대 12초·7일·Owner-only schema 존재                                                        | raw audio 절단, object 저장, row 생성, observation별 1개 보장                                      |
...
- observation 기반 조회, 12초 상한, 정확한 7일 만료, Owner-only, Signed URL 15분 이하 응답 계약을 고정한다.
...
최대 12초와 7일 보관 규칙을 schema 경계에서 검증
```
...
- 생성일부터 7일 직전은 available, 정확히 7일은 재생 불가·삭제 대상이다.
...
- 성공 Clip의 `createdAt`과 `expiresAt` 차이는 정확히 7일이며 실패 Clip에는 expiry가 없다.
...
7일 만료와 조기 삭제를 기존 storage deletion outbox로 통합
...
- [ ] Clip은 observation당 최대 1개, 최대 12초, AAC-LC M4A이며 성공 Clip만 정확히 7일 보관된다.
