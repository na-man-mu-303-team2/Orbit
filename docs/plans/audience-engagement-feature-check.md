...
- Audience Playwright smoke covers join/session copy, Q&A fallback/escalation action, poll response, reaction send, quiz submit, after-close quiz answer reveal, post-session survey, presenter prepared interaction selection/order, AI reference selection excluding slide snapshots, presenter results summary/CSV link, per-question manual result exposure, and Deck JSON slide fallback rendering when snapshots are unavailable.

## Remaining Gaps

- **Slide snapshot lifecycle still needs full browser proof.** Snapshot jobs are queued during draft session preparation, worker completion is persisted into the durable session map, and URLs are frozen at session start, but E2E proof for the complete prepared snapshot job-to-browser lifecycle remains pending.
...
- **E2E coverage remains incomplete.** The hardened audience smoke now covers the core audience join, Q&A, poll, quiz, quiz reveal, reaction, survey, presenter preparation, presenter result summary, manual result exposure, and slide fallback paths, but the completion-plan Playwright matrix for slide sync, AI answer privacy/recovery, and richer poll/quiz/survey edge cases remains partial.
...
- `PLAYWRIGHT_BASE_URL=http://127.0.0.1:5174 pnpm exec playwright test tests/e2e/audience-engagement.spec.ts --reporter=list` (9 passed)
