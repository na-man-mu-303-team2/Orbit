...
| D11 | 대본 패널 | 3줄 자동 프롬프터. speaker notes 명시 줄바꿈을 우선 추적 단위로 쓰고, 줄바꿈이 없으면 기존 문장 분할을 fallback으로 사용. 읽은 줄은 작고 회색, 현재 줄은 크고 검은색, 다음 줄은 작고 검은색으로 표시하며 현재 줄을 자동 스크롤한다. 사용자가 수동 스크롤하면 자동 따라가기를 멈추고 `따라가기`로 복귀한다. 어절/음절 karaoke highlighting은 범위 밖이다. |
...
- **대본:** 3줄 자동 프롬프터로 렌더한다. 명시적인 speaker notes 줄바꿈이 있으면 줄 단위로 추적하고, 없으면 기존 문장 단위 분리를 유지한다. `sentence-covered`/`paraphrased` 상태는 읽은 줄로 작고 회색 처리하며, 현재 줄은 큰 검은색, 다음 줄은 작은 검은색으로 표시한다. 전체 대본은 스크롤 가능하고 자동 따라가기는 수동 스크롤 시 멈춘 뒤 `따라가기`로 복귀한다. 어절/음절 karaoke highlighting은 구현하지 않는다(D11).
