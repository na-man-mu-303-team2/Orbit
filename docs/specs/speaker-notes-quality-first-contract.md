...
1. 사용할 수 있는 비어 있지 않은 대본 생성
2. 문장 완결성
3. 핵심 메시지 보존
4. 근거와 출처 정합성
5. 슬라이드 내부 및 슬라이드 간 논리 흐름
6. 의미 중복 제거
7. 자연스러운 발표 말투
8. 발표 시간과 글자 수

하위 조건을 만족시키기 위해 상위 조건을 훼손하면 안 된다. 예를 들어 발표 시간을 맞추기 위해 핵심 문장을 삭제하거나 문장 중간을 자르는 처리는 금지한다.
자연스러움이 다소 부족하더라도 완결되고 근거가 안전한 대본이 있으면 Job을
실패시키지 않는다. 발표 말투는 LLM 지시문과 결정적 fallback 표현을 개선하는
방식으로 적용하고 별도의 blocking 조건으로 사용하지 않는다.
...
예시는 `오늘은 코딩테스트에서 트리 탐색을 실제로 활용할 때 필요한 기준부터
짚어보겠습니다. 트리 판별부터 시작해서 DFS와 BFS를 선택하고 예제에 적용하는
방법까지 이어가 보겠습니다.`와 같은 형태다.
...
- `message_refs` 선언뿐 아니라 문장 본문에도 이전·현재 메시지의 구체적인
  핵심어가 각각 포함된다.
- 미래 메시지를 참조하지 않는다.
- 키워드를 `A·B`처럼 기계적으로 결합하거나 `정리합니다`, `확인하겠습니다`,
  `살펴보겠습니다`만 반복하지 않는다.

발표 말투 위반은 생성 실패 조건이 아니다. Worker는 원래 문장이 완결되고 안전하면
보존하며, fallback이 필요한 경우에만 내용 중심의 자연스러운 문장으로 대체한다.
