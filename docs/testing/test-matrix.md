...
| 모든 PR | Environment Contract CI: `node infra/scripts/check-env.mjs`, `node --test infra/scripts/personal-staging-env.test.mjs` | 환경 예시 파일의 키 누락·불일치·중복·필수값 공백과 staging source/delivery 정책·Compose 전달 누락을 빠르게 차단 |
...
| `develop` push | Environment Contract CI와 TypeScript CI 자동 실행 | merge 후 환경 계약과 base branch TypeScript 조합을 검증하며 외부 staging 배포는 시작하지 않음 |
...
| ORBIT-139 | ORBIT-27 AI suggestions | shared validation, API approval/apply/reject, web panel/API helper tests |
