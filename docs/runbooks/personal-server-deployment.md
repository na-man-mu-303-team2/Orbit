...
1. GitHub에서 이 저장소 전용 fine-grained personal access token을 준비한다.
...
GitHub App installation token은 이 직접 연결 방식에 사용하지 않는다. installation token은 짧은 수명의 token이므로 GitHub App을 사용하려면 Doppler와 GitHub API 사이에 매 호출마다 새 token을 발급하는 relay를 별도로 둔다.
