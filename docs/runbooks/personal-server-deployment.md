...
`develop`에 merge되면 `Environment Contract CI`가 환경 예시 파일 계약을 다시 검사한다. 성공한 push job만 같은 commit의 `.github/workflows/deploy-personal-staging.yml` reusable workflow를 호출하며, 개인 서버 self-hosted runner는 검증된 commit SHA를 배포 wrapper에 전달한다.
