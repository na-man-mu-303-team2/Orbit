...
> 2026-07-27: 개인 서버 staging은 폐기됐다. 아래 계획의 개인 staging 관련 내용은 과거 구현 기록이며 현재 workflow나 운영 절차가 아니다. 현재 이미지 게시 대상은 `main`의 AWS production backend뿐이다.

## 목표

backend 이미지(`api`·`worker`·`python-worker`)를 **CI에서 1회 빌드 → 컨테이너 레지스트리 push → AWS 운영이 pull**하도록 유지한다. 배포 시점의 **온박스 빌드를 제거**하여 배포 단계를 "빌드 수 분" → "pull 수십 초"로 단축한다.
...
- **레지스트리: GHCR (GitHub Container Registry)**
  - `api`·`worker`·`python-worker`는 env-agnostic이므로 production release SHA의 동일 이미지를 pull한다.
...
- ✅ **Phase 3(플래그)**: `deploy-aws-ec2.sh`에 `DEPLOY_USE_REGISTRY` 분기 추가 — `true`면 GHCR 로그인 후 `pull`, 아니면 기존 `build`.
...
- ✅ **멀티아키 이미지(amd64 + arm64)**: 운영 EC2가 **arm64(Graviton/t4g)** 라, amd64 전용 이미지는 `no matching manifest for linux/arm64/v8`로 pull이 실패했다(→ 온박스 build fallback). `build-images.yml`의 backend 빌드를 **아키텍처별 네이티브 러너**(amd64=`ubuntu-latest`, arm64=`ubuntu-24.04-arm`)로 나눠 digest로 push하고, `merge` 잡이 이를 **단일 manifest list**(`:<sha>`·`:<branch>`)로 합친다.
...
3. AWS 스모크 테스트가 실패하면 `DEPLOY_USE_REGISTRY=false`로 즉시 롤백
...
- [x] `main` push에서 AWS용 backend 이미지만 생성
...
- [ ] AWS 운영 전환 후 스모크 테스트 (로그인·주요 페이지·AI PPT 흐름)
