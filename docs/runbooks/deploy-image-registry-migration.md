...
- ⏳ **남음**: AWS Phase 0(GHCR 토큰을 SSM 준비), AWS 컷오버(경쟁 조건 해결 + 플래그 ON), Phase 5(정리)
...
> ⚠️ **선결 과제 (경쟁 조건)**: `build-images.yml`와 `deploy-aws-production.yml`이 둘 다 `push: main`에서 동시에 트리거된다. 플래그를 켜면 배포가 이미지 빌드보다 먼저 `pull`을 시도해 실패할 수 있다. 컷오버 전에 **"빌드 완료 후 배포" 순서를 보장**할 것 — 예: `deploy-aws-production`을 `workflow_run`(build-images 성공)에 연결하거나, 배포 스크립트의 `pull`에 재시도/백오프 추가. (개인 staging은 배포가 `workflow_dispatch` 수동이라 위험이 낮다.)
