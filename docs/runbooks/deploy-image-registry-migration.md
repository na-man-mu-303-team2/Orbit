...
- **web 처리**
  ...
  - 개인 staging `web`: staging↔운영 간에는 공유 불가하지만, **staging 배포끼리는 build args(`APP_ENV`/`API_BASE_URL`/`WEB_PORT`)가 상수라 동일 이미지 재사용 가능**. CI에서 staging args로 빌드해 GHCR `orbit-web:<branch>`로 push하고 배포는 pull한다(tag 없으면 온박스 build로 fallback).
...
- ✅ **개인 staging `web`도 pull 전환**: `build-images.yml`에 `build-web` 잡 추가(develop 전용, staging args로 `orbit-web:<sha>`·`:<branch>` push). `docker-compose.staging.yml` web에 `image:` 추가, `deploy-personal-server.sh`는 `pull web`(실패 시 온박스 build fallback). 이로써 staging 배포에서 온박스 web 빌드(~2분)가 제거된다.
