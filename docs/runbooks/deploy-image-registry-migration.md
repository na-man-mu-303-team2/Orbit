...
- ✅ **AWS 경쟁 조건 해결(코드)**: `deploy-aws-ec2.sh`가 이제 배포 커밋의 **`<sha>` 태그로 pull**한다(브랜치 태그 아님). SHA 태그는 stale일 수 없고, 아직 없으면 build-images 완료를 짧게 재시도한 뒤 온박스 build로 fallback. 이로써 `push:main` 동시 트리거로 인한 stale/실패가 방지되어, `workflow_run` 재구성 없이도 안전하다.
