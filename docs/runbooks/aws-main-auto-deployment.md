...
4. `apps/web/dist`를 Static Web S3 bucket에 sync
5. SSM `AWS-RunShellScript`로 EC2의 `/usr/local/sbin/orbit-deploy-aws-production` 실행
6. CloudFront invalidation
7. CloudFront `/`, `/api/health`, `/socket.io/` 확인

EC2 wrapper는 `/opt/orbit/source` 전용 checkout만 사용한다.

- 최초 실행: read-only deploy key로 `git clone --branch main`
- 이후 실행: `git fetch origin main`, `git switch main`, `git pull --ff-only origin main`
