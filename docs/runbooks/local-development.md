...
로컬 기본값은 `AI_MOTION_PLANNER_MODE=shadow`와 dated `OPENAI_MOTION_PLANNER_MODEL`을 사용한다. 실제 proposal routing을 확인할 때만 격리된 internal/demo 환경에서 mode를 `on`으로 바꾸며 snapshot, activity/activity-results, imported `partial | unknown` coverage의 거부 gate는 변경하지 않는다.
