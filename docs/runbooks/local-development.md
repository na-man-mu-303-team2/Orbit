...
로컬 latency 측정이 필요할 때만 DevTools에서
`localStorage.setItem("orbit.liveStt.debugRealtime", "true")`를 설정한다. 새 세션부터
transcript가 없는 bounded 진단 이벤트에 onset→first partial, silence commit→final,
onset→final 시간이 기록된다. 측정 후에는 key를 삭제한다.
