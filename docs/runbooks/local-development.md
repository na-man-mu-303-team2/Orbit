...
첫 `migration:run`과 마지막 재적용 후에는 `stages`와 `artifacts`가 모두 보여야 한다. `revert` 직전 최신 migration 이름은 정확히 `CreateAiDeckReferenceExtractionArtifacts2026071504000`이어야 한다. 한 번 되돌린 뒤에는 `stages`만 남고 `artifacts`는 `null`이어야 한다.
