...
- Do not create or execute a Change Set with an AWS account root session. Configure separate least-privilege GitHub OIDC roles in repository variables `AWS_INFRA_PLAN_ROLE_ARN` and `AWS_INFRA_APPLY_ROLE_ARN`; the apply job remains behind the `production` environment approval, and the Apply role trust must accept only the repository's `production` environment OIDC subject.
- Run `Plan AWS infrastructure` only from `refs/heads/main`. The Plan role trust must require `aud=sts.amazonaws.com` and exact subject `repo:na-man-mu-303-team2/Orbit:ref:refs/heads/main`; never allow `repo:na-man-mu-303-team2/Orbit:*`. Pull-request static validation has no `id-token: write` permission and cannot assume the Plan role.
...
2. After the reviewed release commit is on `main`, run `Plan AWS infrastructure` from `refs/heads/main`; record the successful workflow run and use only the Change Set ARN produced by that run. Confirm the change set contains no replacement or deletion of `AWS::RDS::DBInstance`, `AWS::S3::Bucket`, `AWS::CloudFront::Distribution`, or `AWS::EC2::Instance`.
...
1. Let `Build and push images` copy the exact `main` SHA manifest to ECR and record immutable `@sha256` digests. A rerun for an existing immutable SHA tag must reuse its recorded ECR digest instead of pushing the tag again. Supply only those digests to the ECS compute change set.
...
3. Keep `S3_PRIVATE_AUDIO_BUCKET` empty on EC2 and set it only on ECS tasks after the existing policy is attached. New raw audio keys start with `raw/`, evidence derivatives start with `evidence/`, and prefixless legacy audio remains readable from the assets bucket. Reads and cleanup for prefixed keys check the private-audio bucket first and fall back to the assets bucket so objects written before activation remain available. The API/Worker task role grants `s3:ListBucket` on the private-audio bucket so a missing HEAD is distinguishable from AccessDenied; do not replace that permission by treating 403 as a miss. Verify the existing 14-day raw and 7-day evidence lifecycle rules before traffic shift.
...
5. Deploy the ECS compute stack with ALB weights EC2 `100`, ECS `0`. Confirm the ECS target group and container health checks use the API's direct `/health` endpoint, while the retained Nginx-backed EC2 target keeps `/api/health`. Confirm the stack created the ECS-client-to-RDS and ALB-to-legacy-EC2 ingress rules, `API_TRUST_PROXY_HOPS=2` matches the CloudFront-to-ALB path, and API runtime config reports slide practice and slide question guides enabled. Use a candidate CloudFront distribution for login, Deck save, OOXML sync, export, AI deck, rehearsal audio, legacy audio read, Socket.IO presence, and AI/STT/OCR job smoke tests.
6. In the retained bootstrap stack Change Set, set `ApplicationOriginDomainName` and `ApplicationOriginVerificationSecretArn` together only after the ALB candidate is healthy. The template rejects a Change Set that supplies only one of them. The API, auth, and Socket.IO behaviors then use CloudFront `AllViewerExceptHostHeader`, so the origin receives `origin.tryorbit.site` while existing direct-EC2 operation keeps `AllViewer`.
