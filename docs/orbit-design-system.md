...
- 생성 전 wizard는 `내용 입력 → Style & Color`의 2단계로 표시하고 다음 route의 Story Review로 이어진다.
- 내용 입력 화면은 발표 주제, 발표 내용, 청중, 복수 참고자료 첨부를 한 면에 둔다. 발표 시간과 슬라이드 수는 노출하지 않는다.
- Style & Color 화면은 발표 톤 4개와 기본 팔레트 9개를 먼저 제공하고, 열 번째 AI 타일에서 선택 팔레트의 일부 변경 또는 새 분위기 추천을 자연어로 요청한다.
- AI 팔레트 요청이 실패하면 선택 표시와 기존 색상은 유지하고 타일 가까이에 오류를 표시한다.
- 최종 Primary action은 `스토리 만들기` 하나다.
