...
슬라이드별로 "발표자가 꼭 말해야 하는 의미 단위(context item)"를 LLM으로 추출하고, 실시간 STT 발화를 슬라이딩 윈도우에 누적한 뒤 세 가지 매칭 방식(substring → Dice → semantic)으로 커버 여부를 판정한다.
...
| `apps/web/src/features/rehearsal/RehearsalWorkspace.tsx` | context 상태·판정 로직 통합 |
| `apps/web/src/features/rehearsal/panel/RehearsalPanel.tsx` | SlideContextChecklist 삽입 |
| `apps/web/src/styles.css` | 체크리스트 CSS 추가 |
