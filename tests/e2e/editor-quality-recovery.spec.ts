...
async function openValidationPanel(page: Page) {
  ...
  const validationTabLocator = page.getByRole("tab", {
    name: "검사",
    exact: true,
  });
  const validationTab =
    (await validationTabLocator.count()) > 0 ? validationTabLocator : null;
  if (validationTab) {
...
test.describe("P1-3 quality recovery", () => {
  ...
  test("declining validation navigation preserves a dirty speaker note draft", async ({
    page,
  }) => {
    const { project } = await createAuthenticatedProject(page, {
      deck: createQualityRecoveryDeck(),
    ...
    await page
      ...
      .click();
    await page.getByRole("button", { name: "메모 편집", exact: true }).click();
    ...
    expect(dialogMessage).toContain("저장하지 않은 발표 메모 수정 내용");
    await expect(notesEditor).toHaveValue(dirtyDraft);
    await expectQualityDebug(page, { currentSlideId: "slide_1" });
    expect(probe.patchRequests).toEqual([]);
    expect(probe.deckPuts).toEqual([]);
  });
