...
async function armSaveShortcutDefaultProbe(page: Page) {
  await page.evaluate(() => {
    ...
    Event.prototype.preventDefault = function recordPreventDefault() {
      if (
        this instanceof KeyboardEvent &&
        (this.metaKey || this.ctrlKey) &&
        this.key.toLowerCase() === "s"
      ) {
        document.documentElement.dataset.e2eSaveShortcutPrevented = "true";
...
test.describe("P1-2 canvas productivity", () => {
  test("keeps zoom local while 100%, 200%, internal scrolling, and Fit remain deterministic", async ({
    ...
    expect(fitGeometry?.centerDeltaY).toBeLessThanOrEqual(24);
  ...
  test("handles Page navigation and prevents the browser save shortcut for an owner", async ({
    ...
    const lastSavedAt = page.locator(".editor-save-time");
