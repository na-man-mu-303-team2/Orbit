...
- `deck/generate-deck.schema.ts`: `topic`, `prompt`, `targetDurationMinutes`, `slideCountRange`, `template`, `metadata`, `references` 요청 계약을 관리한다. `references`는 `{ fileId }[]`이고 비어 있으면 topic-only generation이다.
