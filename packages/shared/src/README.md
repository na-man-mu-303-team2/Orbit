...
- `deck/generate-deck.schema.ts`: `topic`, `prompt`, `targetDurationMinutes`, `slideCountRange`, `template`, `metadata`, `design`, `references` 요청 계약을 관리한다. `design`은 `visualRhythm`, `densityTarget`, `mediaPolicy`, `layoutDiversity`를 선택적으로 받고, `references`는 `{ fileId }[]`이고 비어 있으면 topic-only generation이다.
