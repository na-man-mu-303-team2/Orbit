...
- `deck/generate-deck.schema.ts`: `topic`, `prompt`, `designPrompt`, `targetDurationMinutes`, `slideCountRange`, `template`, `metadata`, `design`, `references`, `designReferences` 요청 계약을 관리한다. `designPrompt`는 선택적인 시각 스타일 지시이며, `design`은 `visualRhythm`, `densityTarget`, `mediaPolicy`, `layoutDiversity`를 선택적으로 받고, `references`는 내용 참조, `designReferences`는 PPTX 디자인 참조 `{ fileId }[]`이다.
