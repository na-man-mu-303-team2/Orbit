...
describe("animationTimeline", () => {
  it("stable-sorts by order, source index, and animation id without treating duplicate order as concurrency", () => {
    ...
    expect(plan.clickSteps[0]?.effects.map((effect) => effect.animationId)).toEqual([
      "anim_z",
    ]);
  });

  it("computes all four modes from their explicit base references and real timing", () => {
    const plan = createAnimationTimeline({
      animations: [
    ...
    expect(plan.clickSteps[0]?.durationMs).toBe(810);
    expect(plan.clickSteps[0]?.effects[0]?.baseReference).toEqual({
      kind: "click",
      timeMs: 0,
    });
  ...
  it("uses the destination transition as the implicit previous event for an orphan after-previous", () => {
    const plan = createAnimationTimeline({
      animations: [
  ...
  it("keeps missing-target diagnostics bounded while retaining a deterministic plan", () => {
    ...
    expect(plan.diagnostics[0]).toEqual({
      animationId: "anim_missing_000",
      code: "missing-target",
    });
  });

  it("preserves legacy action-referenced order groups until shared migration materializes startMode", () => {
    const plan = createAnimationTimeline({
      animations: [
  ...
  it("keeps a missing legacy follower attached to an explicit group root", () => {
    const plan = createAnimationTimeline({
      animations: [
        createAnimation("anim_explicit_root", {
          order: 1,
          startMode: "on-click",
        }),
        createAnimation("anim_legacy_follower", {
          order: 1,
          startMode: undefined,
        }),
      ],
