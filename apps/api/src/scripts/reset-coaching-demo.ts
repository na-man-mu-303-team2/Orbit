  ...
  try {
    const counts = await AppDataSource.transaction(
      async (manager: EntityManager) => {
        ...
        await manager.query(
          `INSERT INTO rehearsal_runs (run_id,project_id,created_by_user_id,deck_id,status,error,created_at,updated_at,transcript_retained,meta_json,deck_version,evaluation_snapshot_json,semantic_evaluation_mode,analysis_revision,analysis_finalized_at,report_json)
