...
describe("PptxImportsService", () => {
  ...
  it("creates a PPTX import job and enqueues the worker payload", async () => {
    ...
    const result = await new PptxImportsService(
      services.jobsService,
      services.projectsService,
      services.filesService,
      enqueueJob
    ).createImport("project-a", { fileId: "file_template" });
  ...
  it("rejects non-PPTX uploaded files", async () => {
    ...
    await expect(
      new PptxImportsService(
        services.jobsService,
        services.projectsService,
        services.filesService,
        vi.fn(async () => undefined)
      ).createImport("project-a", { fileId: "file_pdf" })
  ...
  it("rejects missing file ids before enqueue", async () => {
    ...
    await expect(
      new PptxImportsService(
        ...
        services.filesService,
        vi.fn(async () => undefined)
      ).createImport("project-a", { fileId: "file_missing" })
