...
describe("processChallengeQnaAnswerJob",()=>{
  ...
  it("maps missing private evidence to the shared analysis error code",async()=>{
    const query=vi.fn(async(sql:string,parameters?:unknown[])=>{
      if(sql.includes("FROM challenge_qna_answer_attempts attempts"))return [answerAttemptRow()];
      if(sql.includes("UPDATE jobs")&&parameters?.[1]==="running")return [[jobRow("running",null,null)],1];
      if(sql.includes("UPDATE jobs")&&parameters?.[1]==="failed")return [[jobRow("failed",null,parameters?.[5])],1];
      return [];
    });
    const evidence={take:vi.fn(async()=>null),delete:vi.fn(async()=>undefined)};
    const job=await processChallengeQnaAnswerJob({query} as unknown as DataSource,{} as never,evidence as never,"http://python:8000",{jobId:"job-answer",projectId:"project-a",answerAttemptId:"answer-a"});

    expect(job).toMatchObject({status:"failed",error:{code:"ANALYSIS_FAILED"}});
    expect(query.mock.calls.find(([sql])=>String(sql).includes("UPDATE challenge_qna_answer_attempts SET status='failed'"))?.[1]?.[1]).toBe("ANALYSIS_FAILED");
  });

  it("maps answer provider failures to the shared analysis error code",async()=>{
    const query=vi.fn(async(sql:string,parameters?:unknown[])=>{
      if(sql.includes("FROM challenge_qna_answer_attempts attempts"))return [answerAttemptRow()];
      if(sql.includes("UPDATE jobs")&&parameters?.[1]==="running")return [[jobRow("running",null,null)],1];
      if(sql.includes("UPDATE jobs")&&parameters?.[1]==="failed")return [[jobRow("failed",null,parameters?.[5])],1];
...
function answerAttemptRow(){return {answer_attempt_id:"answer-a",project_id:"project-a",qna_session_id:"qna-a",question_id:"question-a",question_revision:1,input_mode:"text",status:"queued",audio_file_id:null,storage_key:null,mime_type:null,duration_ms:null,answer_guide_json:{},question_text:"근거?",source_snapshot_json:{}};}
function jobRow(status:"running"|"succeeded"|"failed",result:unknown,error:unknown){return {job_id:"job-answer",project_id:"project-a",type:"challenge-qna-answer-analysis",status,progress:status==="running"?20:100,message:"answer",result,error,created_at:"2026-07-11T00:00:00.000Z",updated_at:"2026-07-11T00:00:01.000Z"};}
