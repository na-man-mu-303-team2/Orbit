...
async function main() {
  ...
  const devices = String(args.devices ?? "webgpu,wasm")
    .split(",")
  ...
  const dtype = {
    encoder_model: args.encoderDtype ?? "fp32",
    decoder_model_merged: args.decoderDtype ?? "q4"
  };
  ...
  try {
    ...
    for (const device of devices) {
      ...
      try {
        const measurement = await evaluateMoonshineInBrowser(page, {
          modelId,
          device,
          dtype,
          fixtures,
          ...
          allowRemoteModels: parseBooleanArg(args.allowRemoteModels)
        });
...
async function evaluateMoonshineInBrowser(page, payload) {
  let lastError = null;
  for (let attempt = 0; attempt < 3; attempt += 1) {
    try {
      return await page.evaluate(async (nextPayload) => {
        const module = await import(
          "/src/features/rehearsal/moonshineBrowserEvaluation.ts"
        );
        return module.runMoonshineBrowserEvaluation(nextPayload);
      }, payload);
    } catch (error) {
      lastError = error;
