...
type ActivitySlideInspectorProps = {
  deckId?: string;
...
export function ActivitySlideInspector(props: ActivitySlideInspectorProps) {
  if (props.readOnly) {
    return <ReadOnlyActivitySlideInspector slide={props.slide} />;
  }

  return <EditableActivitySlideInspector {...props} />;
}

function EditableActivitySlideInspector(props: ActivitySlideInspectorProps) {
...
function ReadOnlyActivitySlideInspector(props: { slide: ActivitySlide }) {
  const activity = props.slide.activity;

  return (
    <div className="activity-slide-inspector" data-read-only="true">
      <div className="activity-inspector-heading">
        <span className="orbit-ds-eyebrow">ACTIVITY</span>
        <h3>{templateLabels[activity.template]}</h3>
        <p>보기 권한에서는 문항 설정과 실행 상태를 변경할 수 없습니다.</p>
      </div>
      <dl className="activity-read-only-summary">
        ...
        <div>
          <dt>설명</dt>
          <dd>{activity.description || "설명 없음"}</dd>
        </div>
        <div>
          <dt>문항 수</dt>
          <dd>{activity.questions.length}</dd>
        </div>
      </dl>
      <ActivitySlidePreview role="audience" slide={props.slide} />
      <div aria-label="잠긴 시스템 레이어" className="activity-system-layer-lock">
        <strong>시스템 레이어 · 읽기 전용</strong>
        <span>응답 UI와 Activity 실행 상태를 변경할 수 없습니다.</span>
      </div>
    </div>
  );
}
