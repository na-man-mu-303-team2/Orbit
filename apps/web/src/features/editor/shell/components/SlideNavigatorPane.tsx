...
export function SlideNavigatorPane(props: {
  ...
  const currentSlideId = props.deck.slides[props.currentSlideIndex]?.slideId ?? null;
    ...
    <aside className={`slides-pane ${props.isCollapsed ? "collapsed" : ""}`}>
      ...
      {hasSlides ? (
        <SlideRail
          ...
          onSelect={(slideId) => {
            const index = props.deck.slides.findIndex((slide) => slide.slideId === slideId);
      ...
      {!props.isCollapsed ? (
        <div className="side-footer">
          ...
          {props.canMutate ? <button className="add-slide-button" type="button" onClick={props.onAddSlide}>
