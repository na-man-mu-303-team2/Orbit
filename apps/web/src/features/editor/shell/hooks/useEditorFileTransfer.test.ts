...
describe("useEditorFileTransfer image insert action", () => {
  ...
  it("keeps the natural ratio around the pointer and clamps the frame to canvas", () => {
  ...
  it("centers an image on the canvas when no pointer placement is provided", () => {
    const canvas = createDemoDeck().canvas;
    const frame = getPlacedImageInsertFrame(canvas, {
      height: 400,
      width: 800,
    });
    ...
    expect(frame).toEqual({
      height: 260,
      width: 520,
      x: (canvas.width - 520) / 2,
      y: (canvas.height - 260) / 2,
    });
  });

  it("converts the scaled stage pointer without reading URL, HTML, or text payloads", () => {
  ...
  it("uses the authored image serializer matrix for imported Deck insertion", () => {
    const deck = importedDeck();
    const capability = getImageInsertCapability(deck, deck.slides[0]!.slideId);
  ...
  it("allocates the id and z-index from the latest Deck after an async upload", async () => {
    ...
    expect(commitPatch.mock.calls[0]?.[1]).toBeUndefined();
  });
