import { getGroupChildElements } from "@orbit/editor-core";
import type {
  Chart,
  CustomShapeElementProps,
  Deck,
  DeckElement,
  GroupElementProps,
  ShapeElementProps,
  Slide,
  TextElementProps
} from "@orbit/shared";
import type Konva from "konva";
import {
  Arrow as KonvaArrowComponent,
  Circle as KonvaCircle,
  Group as KonvaGroup,
  Line as KonvaLine,
  Rect as KonvaRect,
  RegularPolygon as KonvaRegularPolygon,
  Shape as KonvaShape,
  Star as KonvaStarComponent,
  Text as KonvaText
} from "react-konva";
import type { ComponentType } from "react";

import { RenderImageElementContent } from "./RenderImageElementContent";
import {
  buildCustomShapePathDataFromNodes,
  drawCustomShapeScene,
  getCustomShapeDataArray,
  getCustomShapeDimension,
  getCustomShapePaint,
  getCustomShapePathData,
  getCustomShapeStrokeWidth
} from "../custom-shape/renderableCustomShape";
import { getGroupedChildPreviewFrame } from "../groupedChildFrame";
import { getTextElementLayout } from "../text/textLayout";

type KonvaComponent = ComponentType<any>;

const Circle = KonvaCircle as unknown as KonvaComponent;
const Group = KonvaGroup as unknown as KonvaComponent;
const KonvaArrow = KonvaArrowComponent as unknown as KonvaComponent;
const KonvaStar = KonvaStarComponent as unknown as KonvaComponent;
const Line = KonvaLine as unknown as KonvaComponent;
const Rect = KonvaRect as unknown as KonvaComponent;
const RegularPolygon = KonvaRegularPolygon as unknown as KonvaComponent;
const Shape = KonvaShape as unknown as KonvaComponent;
const Text = KonvaText as unknown as KonvaComponent;

type CustomShapeEditDraft = {
  closed: boolean;
  elementId: string;
  nodes: CustomShapeElementProps["nodes"];
  selectedNodeIndex: number | null;
};

export function RenderElementContent(props: {
  accentColor: string;
  customShapePreview?: CustomShapeEditDraft | null;
  deck: Deck;
  element: DeckElement;
  frame: {
    x: number;
    y: number;
    width: number;
    height: number;
    rotation: number;
  };
  slide: Slide;
}) {
  const { accentColor, customShapePreview, deck, element, frame, slide } = props;

  if (element.type === "text") {
    const textLayout = getTextElementLayout({
      frame,
      props: element.props as TextElementProps,
      slide,
      theme: deck.theme
    });

    return (
      <Text
        align={element.props.align}
        fill={textLayout.color}
        fontFamily={textLayout.fontFamily}
        fontSize={element.props.fontSize}
        fontStyle={textLayout.fontStyle}
        lineHeight={element.props.lineHeight}
        listening={false}
        padding={0}
        text={element.props.text}
        width={textLayout.width}
        wrap="word"
        x={textLayout.x}
        y={textLayout.y}
      />
    );
  }

  if (element.type === "image") {
    return <RenderImageElementContent frame={frame} imageProps={element.props} />;
  }

  if (element.type === "chart") {
    const chart = element.props as Chart;
    const values = chart.data.map((datum) =>
      "value" in datum ? Math.abs(datum.value) : Math.abs(datum.y)
    );
    const maxValue = Math.max(1, ...values);
    const barWidth = frame.width / Math.max(chart.data.length, 1);

    return (
      <Group listening={false}>
        <Rect
          cornerRadius={18}
          fill="#fff"
          height={frame.height}
          stroke={accentColor}
          strokeWidth={2}
          width={frame.width}
        />
        <Text
          fill="#0f172a"
          fontSize={18}
          fontStyle="bold"
          text={chart.title || `${chart.type} chart`}
          x={14}
          y={12}
        />
        {chart.data.slice(0, 6).map((datum, index) => {
          const value = "value" in datum ? Math.abs(datum.value) : Math.abs(datum.y);
          const height = Math.max(18, ((frame.height - 84) * value) / maxValue);

          return (
            <Group key={`${datum.label ?? "item"}-${index}`}>
              <Rect
                cornerRadius={8}
                fill={chart.style.colors[index] ?? accentColor}
                height={height}
                width={Math.max(18, barWidth - 16)}
                x={14 + index * barWidth}
                y={frame.height - height - 24}
              />
            </Group>
          );
        })}
      </Group>
    );
  }

  if (element.type === "group") {
    const groupProps = element.props as GroupElementProps;
    const childElements = getGroupChildElements(slide, groupProps.childElementIds);

    if (childElements.length === 0) {
      return (
        <Group listening={false}>
          <Rect
            cornerRadius={18}
            dash={[10, 6]}
            fill="rgba(241, 245, 249, 0.7)"
            height={frame.height}
            stroke="#64748b"
            strokeWidth={2}
            width={frame.width}
          />
          <Text
            align="center"
            fill="#334155"
            fontSize={15}
            height={frame.height}
            padding={12}
            text="빈 그룹"
            verticalAlign="middle"
            width={frame.width}
          />
        </Group>
      );
    }

    return (
      <Group listening={false}>
        {childElements.map((childElement) => {
          const childFrame = getGroupedChildPreviewFrame({
            childElement,
            currentGroupFrame: element,
            previewGroupFrame: frame
          });

          return (
            <Group
              key={childElement.elementId}
              rotation={childFrame.rotation}
              x={childFrame.x}
              y={childFrame.y}
            >
              <RenderElementContent
                accentColor={accentColor}
                deck={deck}
                element={childElement}
                frame={{
                  height: childFrame.height,
                  rotation: childFrame.rotation,
                  width: childFrame.width,
                  x: 0,
                  y: 0
                }}
                slide={slide}
              />
            </Group>
          );
        })}
      </Group>
    );
  }

  if (element.type === "customShape") {
    const customShapeProps = element.props as CustomShapeElementProps;
    const isClosed = customShapePreview?.closed ?? customShapeProps.closed;
    const pathData =
      customShapePreview?.nodes.length
        ? buildCustomShapePathDataFromNodes(customShapePreview.nodes, isClosed)
        : getCustomShapePathData(customShapeProps);
    const dataArray = getCustomShapeDataArray(pathData);
    const fill = getCustomShapePaint(customShapeProps, "fill", "#f5edff");
    const stroke = getCustomShapePaint(customShapeProps, "stroke", "#9333ea");
    const strokeWidth = getCustomShapeStrokeWidth(customShapeProps);
    const viewBoxWidth = getCustomShapeDimension(
      customShapeProps,
      "viewBoxWidth",
      frame.width
    );
    const viewBoxHeight = getCustomShapeDimension(
      customShapeProps,
      "viewBoxHeight",
      frame.height
    );

    if (dataArray.length > 0) {
      return (
        <Group listening={false}>
          <Rect fill="transparent" height={frame.height} width={frame.width} />
          <Shape
            fill={isClosed ? fill : "transparent"}
            fillEnabled={isClosed}
            lineJoin="round"
            scaleX={frame.width / viewBoxWidth}
            scaleY={frame.height / viewBoxHeight}
            sceneFunc={(context: Konva.Context, shape: Konva.Shape) =>
              drawCustomShapeScene(context, shape, dataArray)
            }
            stroke={stroke}
            strokeWidth={strokeWidth}
          />
        </Group>
      );
    }

    return (
      <Group listening={false}>
        <Rect
          cornerRadius={18}
          dash={[10, 6]}
          fill={fill}
          height={frame.height}
          stroke={stroke}
          strokeWidth={2}
          width={frame.width}
        />
        <Text
          fill="#6b21a8"
          fontSize={16}
          fontStyle="bold"
          height={frame.height}
          padding={14}
          text="SVG PATH"
          width={frame.width}
        />
      </Group>
    );
  }

  if (element.type === "ellipse") {
    const shapeProps = element.props as ShapeElementProps;
    const strokeWidth = Math.max(1, shapeProps.strokeWidth);
    const radius = Math.max(1, Math.min(frame.width, frame.height) / 2 - strokeWidth / 2);

    return (
      <Group listening={false}>
        <Circle
          fill={shapeProps.fill === "transparent" ? "#eff6ff" : shapeProps.fill}
          radius={radius}
          stroke={
            shapeProps.stroke === "transparent"
              ? "rgba(15, 23, 42, 0.18)"
              : shapeProps.stroke
          }
          strokeWidth={strokeWidth}
          x={frame.width / 2}
          y={frame.height / 2}
        />
      </Group>
    );
  }

  if (element.type === "polygon") {
    const shapeProps = element.props as ShapeElementProps & { sides?: number };
    const strokeWidth = Math.max(1, shapeProps.strokeWidth);
    const radius = Math.max(1, Math.min(frame.width, frame.height) / 2 - strokeWidth / 2);
    const sides = shapeProps.sides ?? 3;

    return (
      <Group listening={false}>
        <RegularPolygon
          fill={shapeProps.fill === "transparent" ? "#eff6ff" : shapeProps.fill}
          radius={radius}
          sides={sides}
          stroke={
            shapeProps.stroke === "transparent"
              ? "rgba(15, 23, 42, 0.18)"
              : shapeProps.stroke
          }
          strokeWidth={strokeWidth}
          x={frame.width / 2}
          y={frame.height / 2}
        />
      </Group>
    );
  }

  if (element.type === "star") {
    const shapeProps = element.props as ShapeElementProps;
    const strokeWidth = Math.max(1, shapeProps.strokeWidth);
    const outerRadius = Math.max(
      1,
      Math.min(frame.width, frame.height) / 2 - strokeWidth / 2
    );

    return (
      <Group listening={false}>
        <KonvaStar
          fill={shapeProps.fill === "transparent" ? "#eff6ff" : shapeProps.fill}
          innerRadius={outerRadius * 0.48}
          numPoints={5}
          outerRadius={outerRadius}
          stroke={
            shapeProps.stroke === "transparent"
              ? "rgba(15, 23, 42, 0.18)"
              : shapeProps.stroke
          }
          strokeWidth={strokeWidth}
          x={frame.width / 2}
          y={frame.height / 2}
        />
      </Group>
    );
  }

  if (element.type === "ring") {
    const shapeProps = element.props as ShapeElementProps;
    const strokeWidth = Math.max(6, shapeProps.strokeWidth * 4 || 12);
    const radius = Math.max(1, Math.min(frame.width, frame.height) / 2 - strokeWidth / 2);

    return (
      <Group listening={false}>
        <Circle
          fill="transparent"
          radius={radius}
          stroke={
            shapeProps.stroke === "transparent"
              ? shapeProps.fill === "transparent"
                ? "#2563eb"
                : shapeProps.fill
              : shapeProps.stroke
          }
          strokeWidth={strokeWidth}
          x={frame.width / 2}
          y={frame.height / 2}
        />
      </Group>
    );
  }

  if (element.type === "arrow") {
    const shapeProps = element.props as ShapeElementProps;
    const stroke = shapeProps.stroke === "transparent" ? "#2563eb" : shapeProps.stroke;
    const strokeWidth = Math.max(2, shapeProps.strokeWidth);
    const pointerLength = Math.max(18, Math.min(42, frame.width * 0.1));
    const pointerWidth = Math.max(14, Math.min(30, frame.height * 1.2));

    return (
      <Group listening={false}>
        <Rect fill="transparent" height={Math.max(20, frame.height)} width={frame.width} />
        <KonvaArrow
          fill={stroke}
          pointerLength={pointerLength}
          pointerWidth={pointerWidth}
          points={[0, frame.height / 2, frame.width, frame.height / 2]}
          stroke={stroke}
          strokeWidth={strokeWidth}
          tension={0}
        />
      </Group>
    );
  }

  if (element.type === "line") {
    const shapeProps = element.props as ShapeElementProps;

    return (
      <Group listening={false}>
        <Rect fill="transparent" height={Math.max(16, frame.height)} width={frame.width} />
        <Line
          points={[0, frame.height / 2, frame.width, frame.height / 2]}
          stroke={shapeProps.stroke === "transparent" ? "#2563eb" : shapeProps.stroke}
          strokeWidth={Math.max(2, shapeProps.strokeWidth)}
          tension={0}
        />
      </Group>
    );
  }

  const shapeProps = element.props as ShapeElementProps;

  return (
    <Group listening={false}>
      <Rect
        cornerRadius={shapeProps.borderRadius}
        fill={shapeProps.fill === "transparent" ? "rgba(49, 87, 245, 0.08)" : shapeProps.fill}
        height={frame.height}
        stroke={
          shapeProps.stroke === "transparent"
            ? "rgba(16, 24, 40, 0.18)"
            : shapeProps.stroke
        }
        strokeWidth={Math.max(1, shapeProps.strokeWidth)}
        width={frame.width}
      />
    </Group>
  );
}
