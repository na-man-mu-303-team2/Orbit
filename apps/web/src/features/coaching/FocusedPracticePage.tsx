  ...
  return <div className="orbit-ds-page focused-practice-page">
    ...
    <section className="focused-practice-shell">
      ...
      <div className={`focused-practice-layout${deck && targetScope && activeSlideId ? "" : " no-preview"}`}>
        {deck && targetScope && activeSlideId ? <FocusedTargetPreview
          activeIndex={rangeSlideIndex}
          deck={deck}
          preview={Boolean(props.preview)}
          slideId={activeSlideId}
          targetScope={targetScope}
          totalSlides={resolvedSlideIds.length}
        /> : null}
...
function FocusedTargetPreview(props: {
  ...
  const label = targetLabel(props.targetScope, props.activeIndex, props.totalSlides);
  if (props.targetScope.type === "sentence") {
    const sentence = resolveFocusedPracticeSentence(props.deck, props.targetScope);
    return <section className="focused-slide-preview focused-sentence-preview" aria-labelledby="focused-sentence-title"><div><small>{label}</small><h2 id="focused-sentence-title">{props.deck.slides.find((slide) => slide.slideId === props.slideId)?.title}</h2></div><blockquote>{sentence ?? "문장 내용을 찾을 수 없습니다."}</blockquote></section>;
  }
  if (props.preview) return <FocusedPreviewSlideCard deck={props.deck} label={label} slideId={props.slideId} />;
  return <Suspense fallback={<FocusedPreviewSlideCard deck={props.deck} label={label} slideId={props.slideId} />}><FocusedSlidePreview deck={props.deck} label={label} slideId={props.slideId} /></Suspense>;
}

function FocusedPreviewSlideCard(props: { deck: Deck; label?: string; slideId: string }) {
  const slide = props.deck.slides.find((candidate) => candidate.slideId === props.slideId);
  if (!slide) return null;
  return <section className="focused-slide-preview focused-slide-preview-fallback" aria-labelledby="focused-slide-title"><div><small>{props.label ?? "현재 장표"}</small><h2 id="focused-slide-title">{slide.title}</h2></div><div><span>{String(slide.order).padStart(2, "0")} · FOCUS</span><strong>{slide.title}</strong><p>이 장표의 핵심 메시지와 근거를 짧게 반복합니다.</p></div></section>;
}
