...
const systemFontPattern =
  /-apple-system|BlinkMacSystemFont|Apple SD Gothic Neo|SFMono-Regular|SF Mono|Menlo|SUIT Variable|ui-monospace|ui-sans-serif|system-ui/;

describe("ORBIT web fonts", () => {
  it("bundles the Pretendard family without a local-font dependency", () => {
    const fontCss = fs.readFileSync(
      path.join(webRoot, "src/fonts.css"),
      "utf8",
    );
