...
def imported_main_sequence_coverage(
    ...
    if motion.coverage == "absent":
        return "absent"
    # Without an authoritative shape-to-element mapping, a populated main
    # sequence cannot be proven round-trip complete.
...
def validate_source_props_update(
    ...
    if prop_names and prop_names.issubset(SUPPORTED_TEXT_PROPS):
        ...
        if declared_capability not in {"full", "style-only"}:
            return "RICH_TEXT_CAPABILITY_UNSAFE"
        if declared_capability != actual_capability:
            return "RICH_TEXT_CAPABILITY_UNSAFE"
        if declared_capability == "style-only":
            ...
            if set(props) != {"text"} and text_props_has_content_projection(props):
                target_paragraphs = canonical_text_paragraphs(props)
                body = first_local_child(shape, "txBody")
                if (
                    body is None
                    or target_paragraphs is None
                    or not style_only_paragraphs_match(body, target_paragraphs)
                ):
                    return "RICH_TEXT_CAPABILITY_UNSAFE"
        return None
...
def valid_text_props(props: dict[str, Any]) -> bool:
    ...
    if props.get("verticalAlign", "top") not in {"top", "middle", "bottom"}:
        return False
    if props.get("writingMode", "horizontal") not in {
        "horizontal",
        "vertical-270",
    }:
        return False
    ...
    if paragraphs is not None and (
        ...
        or any(not valid_text_paragraph(paragraph) for paragraph in paragraphs)
    ):
        return False
    return (
...
def valid_text_paragraph(value: Any) -> bool:
    ...
    for key in ("spaceBefore", "spaceAfter"):
        if key in value and not valid_nonnegative_number(value.get(key)):
            return False
...
def valid_text_bullet(value: Any) -> bool:
    if not isinstance(value, dict) or set(value) - {"enabled", "character", "indent"}:
        return False
    return (
        isinstance(value.get("enabled", False), bool)
    ...
    if isinstance(raw_paragraphs, list):
        ...
        for raw_paragraph in raw_paragraphs:
            if not isinstance(raw_paragraph, dict):
                return None
            paragraph = copy.deepcopy(raw_paragraph)
            ...
            if isinstance(raw_runs, list) and raw_runs:
                ...
                paragraph_text = "".join(str(run.get("text", "")) for run in runs)
                if "text" in paragraph and paragraph.get("text") != paragraph_text:
                    return None
            else:
                paragraph_text = str(paragraph.get("text", ""))
                runs = [{"text": paragraph_text}] if paragraph_text else []
        ...
        value = "\n".join(str(paragraph["text"]) for paragraph in paragraphs)
        if "text" in props and props.get("text") != value:
            return None
        return paragraphs
    ...
    if isinstance(raw_runs, list) and raw_runs:
        ...
        for raw_run in raw_runs:
            if not isinstance(raw_run, dict):
                return None
            pieces = str(raw_run.get("text", "")).split("\n")
